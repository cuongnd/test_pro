...
class plgSystemAdmintoolsCore extends JPlugin
{
	...
	public function __construct(& $subject, $config = array())
	{
		...
		$hash = JFactory::getApplication()->input->getCmd('fetchcombinedfile', null);
		if (!empty($hash))
		{
	...
	public function onCssJsCombine()
	{
		...
		)
		{
			...
			if (!in_array($delivery, array('plugin', 'direct')))
			{
				$delivery = 'plugin';
			}
	...
	private function csscombine()
	{
		...
		foreach ($links as $linkCode)
		{
			...
			if (!empty($matches[1]))
			{
				...
				{
					$medium = trim($medium);
					if (in_array($medium, array('screen', 'all')))
					{
						$skip = false;
					}
				}
	...
	private function deliverFile($hash)
	{
		...
		if ($pass)
		{
			$pass = $pass && JFile::exists($this->combineCache . DIRECTORY_SEPARATOR . $hash . $ext);
		}
	...
	/**
	 * Callback method to remove the JavaScript <script type="text/javascript"> tags
	 *
	 * @param array $matches
	 *
	 * @return string
		...
		if (isset($m[1]) && count($m[1]))
		{
			...
			if (count($this->jsFiles))
			{
				...
				foreach ($this->jsFiles as $file)
				{
					...
					if (($file->file == 'administrator' . DIRECTORY_SEPARATOR . $filename))
					{
						$found = true;
					}
