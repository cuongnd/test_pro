// jQuery Plugin for sprFlatFrontEnd admin template
...
(function($) {
    ...
    // here we go!
    $.sprFlatFrontEnd = function(element, options) {
...
/*
        ...
        plugin.fixedSidebar = function (val) {
            if(val == true) {
                $('#sidebar_front_end').addClass('sidebar-fixed');
                //activate slim scroll
                $('#sidebar_front_end>.sidebar-inner-front-end').slimScroll({
                    position: "right",
                    height: '100%',
                    distance: '0',
                    railVisible: false,
                ...
                $('#sidebar_front_end').removeClass('sidebar-fixed');
                //deactivate slim scroll
                $('.sidebar-inner-front-end').parent().replaceWith($('.sidebar-inner-front-end'));
                $('.sidebar-inner-front-end').attr('style', '');
            }
        }
        //fixed sidebarTool
        plugin.fixedSidebarTool = function (val) {
            if(val == true) {
                $('#right-sidebar-tool').addClass('sidebar-fixed');
                //activate slim scroll
                $('#right-sidebar-tool>.sidebar-inner-front-end').slimScroll({
                    position: "right",
                    height: '100%',
                    distance: '0',
                    railVisible: false,
                ...
                $('#right-sidebar-tool').removeClass('sidebar-fixed');
                //deactivate slim scroll
                $('.right-sidebar-tool .sidebar-inner-front-end').parent().replaceWith($('.sidebar-inner-front-end'));
                $('.right-sidebar-tool .sidebar-inner-front-end').attr('style', '');
            }
        }
        ...
        plugin.toggleSidebar = function() {
            ...
            toggleButton.on("click", function(e){
                e.preventDefault();
                $('#sidebar_front_end').toggleClass('hide-sidebar');
                ...
                if(plugin.settings.sidebar.rememberToggle) {
                    if($('#sidebar_front_end').hasClass('hide-sidebar')) {
        ...
        //toggle sidebarTool
        plugin.toggleRightSidebarTool = function() {
            var toggleButton = $('#toggle-right-sidebar-tool');
            var toggleIcon = toggleButton.find('i');
            var breakpoint = plugin.getBreakPoint();
            toggleButton.on("click", function(e){
                e.preventDefault();
                $('#right-sidebar-tool').toggleClass('hide-sidebar');
                if (breakpoint == 'tablet' || breakpoint == 'phone') {
                    $('#content').toggleClass('full-page offCanvas');
                } else {
                    $('#content').toggleClass('full-page');
                ...
                if(plugin.settings.sidebar.rememberToggle) {
                    if($('#right-sidebar-tool').hasClass('hide-sidebar')) {
                        store.set('sidebarToggle', 1);
                    } else {
                        store.set('sidebarToggle', 0);
                    }
        ...
        plugin.hideLeftSidebar = function() {
            var toggleButton = $('#toggle-sidebar');
            var toggleIcon = toggleButton.find('i');
            $('#sidebar_front_end').addClass('hide-sidebar');
        ...
        plugin.showLeftSidebar = function() {
            var toggleButton = $('#toggle-sidebar');
            var toggleIcon = toggleButton.find('i');
            $('#sidebar_front_end').removeClass('hide-sidebar');
        ...
        plugin.sideBarNav = function() {
            //cache the elements
            var navscroll = $('#sidebar_front_end>.sidebar-inner-front-end');
            var nav = $('.menu-menu-side-nav');
            ...
            if(plugin.settings.sideNav.showArrows) {
                if(!$('.menu-menu-side-nav').hasClass('show-arrows')) {
                    $('.menu-menu-side-nav').addClass('show-arrows');
                }
        ...
        plugin.panels = function () {
            ...
            panelControlsLink.click(function(e) {
                ...
                if (self.hasClass('panel-close')) {
                    _functionCheckClose=self.attr('data-function-checkClose');
                    if(_functionCheckClose.length>0) {

                        if(_functionCheckClose())
                        {
                            setTimeout(function () {
                                thisPanel.remove();
                            }, 500);
        ...
        plugin.breadCrumbs = function () {
            ...
            var navel = $('.menu-menu-side-nav>li a.active');
...
-1*delta}if(orgEvent.wheelDeltaY!==undefined)deltaY=orgEvent.wheelDeltaY/120;if(orgEvent.wheelDeltaX!==undefined)deltaX=-1*orgEvent.wheelDeltaX/120;args.unshift(event,delta,deltaX,deltaY);return($.event.dispatch||$.event.handle).apply(this,args)}})(jQuery);
        ...
        plugin.quickSearch = function () {
            ...
            if ($('.top-search input').length) {
                $('.top-search input').val('').quicksearch('.menu-menu-side-nav li a', {
                    'onBefore': function () {
                        if($(this).val() != '') {
    ...
    $.fn.sprFlatFrontEnd = function(options) {

        // iterate through the DOM elements we are attaching the plugin to
        return this.each(function() {

            // if plugin has not already been attached to the element
            if (undefined == $(this).data('sprFlatFrontEnd')) {

                // create a new instance of the plugin
                // pass the DOM element and the user-provided options as arguments
                var plugin = new $.sprFlatFrontEnd(this, options);

                // store a reference to the plugin object
                // element.data('sprFlatFrontEnd').publicMethod(arg1, arg2, ... argn) or
                // element.data('sprFlatFrontEnd').settings.propertyName
                $(this).data('sprFlatFrontEnd', plugin);
