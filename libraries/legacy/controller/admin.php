...
class JControllerAdmin extends JControllerLegacy
{
	...
	public function __construct($config = array())
	{
		...
		$this->registerTask('isnotsystem', 'issystem');
	...
	public function issystem()
	{
		...
		// Get items to publish from the request.
		$cid = JFactory::getApplication()->input->get('cid', array(), 'array');
		$data = array('issystem' => 1, 'isnotsystem' => 0);
		$task = $this->getTask();
		$value = JArrayHelper::getValue($data, $task, 0, 'int');
		...
		else
		{
			...
			try
			{
				$model->issystem($cid, $value);

				if ($value == 1)
				{
					$ntext = $this->text_prefix . '_N_ITEMS_IS_SYSTEM';
				}
				elseif ($value == 0)
				{
					$ntext = $this->text_prefix . '_N_ITEMS_IS_NOT_SYSTEM';
				}
