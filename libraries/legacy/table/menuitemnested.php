 ...
 * @since       11.1
 */
class JTableMenuItemNested extends JTableNested
{
	/**
	...
	public function moveByReference($referenceId, $position = 'after', $pk = null)
	{
		...
		else
		{
			...
			$query->clear()
				...
				->from($this->_tbl)
				->where('parent_id = '.(int)$this->menu_type_id)
                ->where('menu_type_id='.(int)$this->menu_type_id)
				->order('lft DESC');
	...
	public function store($updateNulls = false)
	{
		...
		if (empty($this->$k))
		{
			...
			if ($this->_location_id >= 0)
			{
				...
				{
					// Error message set in lock method.
					$this->setError('rror message set in lock method');
					return false;
				}
	...
	public function getRootId()
	{
		...
		// Test for a unique record with parent_id = $this->menu_type_id
		$query = $this->_db->getQuery(true)
			->select($k)
			->from($this->_tbl)
            ->where('menu_type_id='.(int)$this->menu_type_id)
