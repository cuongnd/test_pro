...
class JTableCategory extends JTable
{
	...
	public function get_root_by_menu_item_id($menu_item_id=0){
		$query=$this->_db->getQuery(true);
		$sql='select @parent:=parent_id as prnt, title, id from (select @parent:='.$menu_item_id.' ) a join (select * from #__categories order by id desc) b where @parent=id';
		$query->setQuery($sql);
	...
	public function set_website_id($website_id=0)
	{
	...
	public function publish($pks = null, $state = 1, $userId = 0)
	{
		...
		foreach ($pks as $pk)
		{
			...
			if ($node->parent_id)
			{
				...
				$query->clear()
					->select('n.' . $k)
					->from($this->_db->quoteName('#__categories') . ' AS n')
			...
			$query->clear()
				->update($this->_db->quoteName('#__categories'))
