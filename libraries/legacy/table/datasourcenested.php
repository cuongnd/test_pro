 ...
 * @since       11.1
 */
class JTableDataSourceNested extends JTable
{
	/**
	...
	function treeRecurseDelete($id,  $maxLevel = 9999, $level = 0)
	{
		...
		if (count($listId) && $level <= $maxLevel) {
			foreach ($listId as $id) {
				JTableDataSourceNested::treeRecurseDelete($id, $maxLevel, $level + 1);
			}
	...
	public function delete($pk = null, $children = true)
	{
		...
		if ($children)
		{
			...
			JTableDataSourceNested::treeRecurseDelete($pk,99,0);
			// Compress the left values.
			$query->clear()
				->update($this->_tbl)
				->set('lft = lft - ' . (int) $node->width)
	...
	public function getRootId()
	{
		...
		$query = $this->_db->getQuery(true)
			...
			->where('parent_id =id ')
			->where('website_id = '.(int)$this->website_id)
			->where('LOWER(screenSize) = '.strtolower($this->_db->q($this->screensize)))
		;
