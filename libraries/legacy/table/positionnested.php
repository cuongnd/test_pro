 ...
 * @since       11.1
 */
class JTablePositionNested extends JTable
{
	/**
	...
	public function getPath($pk = null, $diagnostic = false)
	{
		...
		$query = $this->_db->getQuery(true)
			...
			->where('n.' . $k . ' = ' . (int) $pk)
			->where('n.website_id='.(int)$this->website_id)
			->where('LOWER(n.screensize)='.$this->_db->q($this->screensize))
			->order('p.lft');
	...
	function treeRecurseDelete($id,  $maxLevel = 9999, $level = 0)
	{
		$db=JFactory::getDbo();
		...
		$db->setQuery($query);
		$listId=$db->loadAssocList();
		if (count($listId) && $level <= $maxLevel) {
			foreach ($listId as $id) {
				JTablePositionNested::treeRecurseDelete($id, $maxLevel, $level + 1);
			}
	...
	public function createRoot($screenSize)
	{
		...
		$query = $this->_db->getQuery(true)
			->update($this->_tbl)
			->set('parent_id = '.(int)$insertId)
			->where('id='.(int)$insertId)
		;
		$this->_db->setQuery($query);
		if(!$this->_db->execute())
		{
			$e = new UnexpectedValueException(sprintf('%s::createRoot-update parent-id', get_class($this)));
			$this->setError($e);
			return false;
		}
	...
	public function rebuild($parentId = null, $leftId = 0, $level = 0, $path = '',$menu_item_id=0,$screeensize='',$only_page=0)
	{
		...
		foreach ($children as $node)
		{
			 ...
			 */
			$rightId = $this->rebuild($node->{$this->_tbl_key}, $rightId, $level + 1, $path . (empty($path) ? '' : '/') . $node->alias,$node->menu_item_id,$node->screeensize,$node->only_page);
