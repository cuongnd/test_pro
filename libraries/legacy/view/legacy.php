...
class JViewLegacy extends JObject
{
	...
	public function loadTemplateByLayout($layout='',$tpl='')
	{
	...
	public function loadTemplateByLayoutOfView($layout='',$tpl='',$view='')
	{
		$this->_path['template'][]=JPATH_COMPONENT.'/views/'.$view.'/tmpl';
		$layout1=$this->getLayout();
		$this->setLayout($layout);
		echo  $this->loadTemplate($tpl);
		$this->setLayout($layout1);
		...
		switch (strtolower($type))
		{
			case 'template':
				...
				if (isset($app))
				{
					...
					$view=$this->getName();
					$fallback = JPATH_THEMES . '/' . $app->getTemplate() . '/html/' . $component . '/' .$view ;
