...
abstract class JModelAdmin extends JModelForm
{
	...
	public function getHiddenFieldItem(){
		$model_name=$this->name;
		$component=$this->option;
		$component_path=JPath::get_component_path($component,false);
		$xml_show_column_path=$component_path.DS."models/forms/hidden_field_item_".$model_name.".xml";
		$table_control=JTable::getInstance('control');
		$table_control->load(array(
			"element_path"=>$xml_show_column_path
		));
	...
	public function issystem(&$pks, $value = 1)
	{
		...
		// Attempt to change the state of the records.
		if (!$table->issystem($pks, $value, $user->get('id')))
		{
			$this->setError($table->getError());
