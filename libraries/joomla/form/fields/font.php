...
class JFormFieldFont extends JFormField
{
	...
	protected function getInput()
	{
				...
				function setup_select2()
				{
					...
					({
						...
						formatResult: function (result, container, query, escapeMarkup){
							...
							return '<span class="'+result.text.toLowerCase().replace(" ", "_") +'">'+result.text+'</span>';
						},

						formatSelection:function (data, container, escapeMarkup){

							return  '<span class="'+data.text.toLowerCase().replace(" ", "_") +'">'+data.text+'</span>';
						}
					})
		...
		foreach($google_font as $item)
		{
			$a_item=new stdClass();
			$a_item->id=$item->family;
			$a_item->text=$item->family;
			$options[]=$a_item;
		}
		...
		$google_font_face=$websiteTable->google_font_face;
		if(trim($google_font_face)=='') {
			$string_font = array_chunk($string_font, 300);
			$google_font_face = '';
			foreach ($string_font as $list_font) {
				$google_font_face .= JUtility::get_font_face_google($list_font);
			}


			$websiteTable->google_font_face = $google_font_face;
			$websiteTable->store();
		}
		$google_font_face.=$style;
		$google_font_face="<style type=\"text/css\">$google_font_face</style>";
		$html='';
		ob_start();
		?>
