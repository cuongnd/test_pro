...
require_once JPATH_ROOT . '/libraries/joomla/application/base.php';
...
class JApplicationWeb extends JApplicationBase
{
       ...
       if($tmpl=="android" || $os=="android")
        ...
        if ($this->checkHeadersSent()) {
            ...
            if (($this->client->engine == JApplicationWebClient::TRIDENT) && !utf8_is_ascii($url)) {
                ...
                $html .= '<script type="text/javascript">document.location.href=\'' . str_replace("'", "&apos;", $url) . '\';</script>';
    ...
    public function setBody($content)
    {
        if ($content != '') {
            ...
            $list_image1 = $matches[1];
            $curent_uri = JFactory::getURI();
            $list_image = array();
            foreach ($list_image1 as $image) {
                $src = $image;
                $uri = JFactory::getURI($src);
                $item_image = new stdClass();
                if ($uri->getHost() == $curent_uri->getHost() || $uri->getHost() == '') {
                    $item_image->src = JPATH_ROOT . '/' . $uri->getPath();
                    $item_image->is_host = 1;
                    $list_image[] = $item_image;
                } else {
                    $item_image->src = $src;
                    $item_image->is_host = 0;
                    $list_image[] = $item_image;
                }
