	...
	public function store($updateNulls = false)
	{
		...
		if (empty($this->$k))
		{
			...
			if ($this->_location_id >= 0)
			{
				...
				if(!$this->website_id)
				{
					throw new Exception("there no website_id in row please check agian");
				}
				...
				if ($this->_location_id == 0)
				{
					...
					// Get the last root node as the reference node.
					$query = $this->_db->getQuery(true)
						->select('user_group.'.$this->_tbl_key . ', user_group.parent_id, user_group.level, user_group.lft, user_group.rgt')
						->from($this->_tbl.' AS user_group')
						->leftJoin('#__user_group_id_website_id AS user_group_id_website_id ON user_group_id_website_id.user_group_id=user_group.id')
						->where('user_group_id_website_id.website_id='.(int)$this->website_id)
						->order('lft DESC');
					$this->_db->setQuery($query, 0, 1);
					$reference = $this->_db->loadObject();
					if(!$reference)
					{
						throw new Exception("there no root group in row please check agian");
					}
					// @codeCoverageIgnoreStart
					if ($this->_debug)
					{
