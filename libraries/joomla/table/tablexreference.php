 ...
 */
abstract class Jtablexreference extends JObject implements JObservableInterface
{
	/**
	...
	public static function getInstance($type, $prefix = 'JTable', $config = array())
	{
		...
		if (!class_exists($tableClass))
		{
			...
			$path = JPath::find(self::addIncludePath(), strtolower($type) . '.php',$tableClass);
		...
		$table_key = isset($config['key']) ? $config['key'] : '';

		// Instantiate a new table class and return it.
		if(strtolower($type)=='updatetable')
		{
			return new $tableClass($db,$table,$table_key);
		}
	...
	public function issystem($pks = null, $state = 1, $userId = 0)
	{
