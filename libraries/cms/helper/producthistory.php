 ...
 * @since       3.2
 */
class JHelperproducthistory extends JHelper
{
	/**
	 ...
	 * Constructor
	 *
	 * @param   string  $typeAlias  The type of product to be versioned (for example, 'com_product.article').
	 *
	 * @since   3.2
	 */
	...
	public function deleteHistory($table)
	{
		...
		$id = $table->$key;
		$typeTable = JTable::getInstance('producttype', 'JTable');
	...
	/**
	 * Method to save a version snapshot to the product history table.
	 *
	 * @param   JTable  $table  JTable object being versioned
	 *
	 * @return  boolean  True on success, otherwise false.
	...
	public function store($table)
	{
		...
		$historyTable = JTable::getInstance('producthistory', 'JTable');
		$typeTable = JTable::getInstance('producttype', 'JTable');
		...
		if ($maxVersions = JComponentHelper::getParams('com_product')->get('history_limit', 0))
		{
