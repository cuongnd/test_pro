 ...
 * @since       1.6
 */
class JFormFieldGridToolbar extends JFormField
{
    /**
     * The form field type.
     *
    ...
    function create_html_list_template($nodes,$list_data_type,$list_data_source,$list_editor,$level)
    {
        ...
        for ($i=0;$i<count($nodes);$i++) {
            ...
            <div class="dd-handle">
                ...
                <?php echo $column->id ?>
                <button onclick="gridtoolbar.remove_item_nestable(this)" class="dd-handle-remove pull-right"><i class="fa-remove"></i></button>
            </div>
            <div>
                <button class="add_node" onclick="gridtoolbar.add_node(this,'template')">add node</button>
                <button style="display: none" class="add_sub_node">add sub node</button>
            </div>
            <label>Column title<input class="form-control" style="width: 200px" type="text"  onchange="gridtoolbar.update_data_column(this,'column_title')"
                                    value="<?php echo $column->column_title ?>"/></label>
            <label>Column name <input class="column_name form-control"  style="width: 200px" type="text"  onchange="gridtoolbar.update_data_column(this,'column_name')"
                                     value="<?php echo $column->column_name ?>"/></label>

            <label>show command <input data-tags="true" class="show_command form-control"  style="width: 200px" type="text"  onchange="gridtoolbar.update_data_column(this,'show_command')"
                                     value="<?php echo $column->show_command ?>"/></label>
            <label>Type
                <select style="width: 200px" name="data_type1" onchange="gridtoolbar.update_data_column(this,'type')"
                        class="data_type">
            ...
            <br/>
            <label>Max character<input class="form-control" style="width: 100px" type="text"  onchange="gridtoolbar.update_data_column(this,'max_character')"
                                      value="<?php echo $column->max_character ?>"/></label>


            <label>Column width<input type="text" class="form-control" onchange="gridtoolbar.update_data_column(this,'column_width')"
                                      value="<?php echo $column->column_width ?>"/></label>
            <label>Link key<input type="text" class="link_key form-control" onchange="gridtoolbar.update_data_column(this,'link_key')"
                                  value="<?php echo $column->link_key ?>"/></label>
            <label>editor
                <select onchange="gridtoolbar.update_data_column(this,'editor_type');">
            ...
            <label>datasource
                <select style="width: 200px" onchange="gridtoolbar.update_data_column(this,'data_source_id');gridtoolbar.change_data_source(this)">
            ...
            <label>key datasource<input type="text" class="form-control" onchange="gridtoolbar.update_data_column(this,'key_data_source')"
                                        value="<?php echo $column->key_data_source ?>"/></label>
            <label>text datasource<input type="text" class="form-control" onchange="gridtoolbar.update_data_column(this,'text_data_source')"
            ...
            <label>Show<input <?php echo $column->show == 1 ? 'checked' : '' ?>  type="checkbox"
                                                                                 onchange="gridtoolbar.update_data_column(this,'show','checkbox')"
                                                                                 value="1"/></label>
            <label>filterable<input <?php echo $column->filterable == 1 ? 'checked' : '' ?>  type="checkbox"
                                                                                 onchange="gridtoolbar.update_data_column(this,'filterable','checkbox')"
                                                                                 value="1"/></label>
            <label>sortable<input <?php echo $column->sortable == 1 ? 'checked' : '' ?>  type="checkbox"
                                                                                 onchange="gridtoolbar.update_data_column(this,'sortable','checkbox')"
                                                                                 value="1"/></label>
            <label>menu<input <?php echo $column->menu == 1 ? 'checked' : '' ?>  type="checkbox"
                                                                                 onchange="gridtoolbar.update_data_column(this,'menu','checkbox')"
                                                                                 value="1"/></label>
            <label>button checked<input <?php echo $column->button_checked == 1 ? 'checked' : '' ?>  type="checkbox"
                                                                                 onchange="gridtoolbar.update_data_column(this,'button_checked','checkbox')"
                                                                                 value="1"/></label>
            ...
            if (count($childNodes) > 0) {
                $level=$level+1;
                JFormFieldGridToolbar::create_html_list_template($childNodes,$list_data_type,$list_data_source,$list_editor,$level);
            }
            echo "</li>";
        }
        echo '</ol>';
    ...
    protected function getInput()
    {
        ...
        $lessInput = JPATH_ROOT . '/libraries/cms/form/field/gridtoolbar.less';
        $cssOutput = JPATH_ROOT . '/libraries/cms/form/field/gridtoolbar.css';
        $db = JFactory::getDbo();
        JHtml::_('jquery.framework');
        JHtml::_('bootstrap.framework');
        JUtility::compileLess($lessInput, $cssOutput);
        $doc->addStyleSheet(JUri::root() . "/libraries/cms/form/field/gridtoolbar.css");
        ...
        $doc->addScript(JUri::root() . "/libraries/cms/form/field/gridtoolbar.js");
        ...
        $gridtoolbar =$params->get('gridtoolbar');
        $object_mode_select_column_template = up_json_decode($gridtoolbar, false, 512, JSON_PARSE_JAVASCRIPT);
...
/*        foreach($list_icon as $key=>$value)
        ...
        <script type="text/javascript">
            function <?php echo $scriptId ?>() {
                gridtoolbar.data_command =<?php echo json_encode($list_command) ?>;
                gridtoolbar.data_column =<?php echo json_encode($list_key) ?>;
                gridtoolbar.list_icon =<?php echo json_encode($list_icon) ?>;
                gridtoolbar.list_menu =<?php echo json_encode($list_menu) ?>;

                gridtoolbar.init_gridformarttoolbar();
            }
        </script>
        <?php
        ...
        <div class="cf nestable-lists">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="dd " id="gridformartheader">
                        <?php if(count((array)$object_mode_select_column_template)){
                            $level=1;
                            JFormFieldGridToolbar::create_html_list_template($object_mode_select_column_template,$list_data_type,$list_data_source,$list_editor,$level);
                        }else{ ?>
        ...
        <input type="hidden" name="<?php echo $this->name ?>" value="<?php echo $mode_select_column ?>" name="" value="" id="gridformartoolbar-output"/>
