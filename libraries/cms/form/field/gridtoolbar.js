jQuery(document).ready(function ($) {
    gridtoolbar = {
        ...
        column_name_option: {
            data: function () {
                data_post = gridtoolbar.data_column;
        ...
        show_command_option: {
            tags: "true",
            data: function () {
                data_post = gridtoolbar.data_command;
        ...
        option_droppable: {
            ...
            drop: function (ev, ui) {
                ...
                if (uiDraggable.hasClass('configupdate-item-table')) {
                    gridtoolbar.render_table_fields(uiDraggable, droppable);
                } else if (uiDraggable.hasClass('configupdate-item-field')) {
                    gridtoolbar.render_table_field(uiDraggable, droppable);
                }
            }
        },
        option_nestable1: {
        ...
        init_config_nestable: function () {
            $('#gridformartheader1').nestable(gridtoolbar.option_nestable1)
                .on('change', gridtoolbar.updateOutput);
            $('#gridformartheader2').nestable(gridtoolbar.option_nestable2)
                .on('change', gridtoolbar.updateOutput);


        },
        init_gridformarttoolbar: function () {


            gridtoolbar.set_auto_complete();
            // activate Nestable for list 1
            gridtoolbar.init_config_nestable();
            $(".configupdate-item-table").draggable(gridtoolbar.option_draggable);
            gridtoolbar.update_nestable();
            $('.dd-list-droppable').droppable(gridtoolbar.option_droppable);
            $('.configupdate-item-table a.plus').click(function () {
                gridtoolbar.get_list_field_table($(this));
            });
            list_editor = {};
            $('.column_template_texarea').each(function (index) {
                ...
                gridtoolbar.set_code_mirror(self,key);
            });
        ...
        set_code_mirror: function (self,key) {
            ...
            $.each(gridtoolbar.list_menu, function( index, value ) {
                list_field.push(value);
            });


            $.each(gridtoolbar.data_column, function( index, value ) {
            ...
            CodeMirror.registerHelper("hint","list_icon",function(cm,options){
                ...
                function scan(dir) {
                    ...
                    $.each(gridtoolbar.list_icon, function( index, value ) {
            ...
            editor.on("changes", function(cm, change) {
                ...
                gridtoolbar.update_data_column_template(cm,self,cm.getValue());
            });
        ...
        add_node: function (self, type) {
            ...
            li_clone.find("input.show_command").select2(gridtoolbar.show_command_option);
            li_clone.find("input.column_name").select2(gridtoolbar.column_name_option);
            li_clone.find("input.post_name").select2(gridtoolbar.post_name_option).select2("data", gridtoolbar.data_post, true);
            ...
            if (type == 'template') {
                ...
                var random_number = gridtoolbar.makeid();
                column_template_texarea.attr('id', random_number);
                gridtoolbar.set_code_mirror(column_template_texarea);
            }
            gridtoolbar.update_nestable();
        },
        add_sub_node: function (self) {
            ...
            li_clone.find("input.table_name").select2(gridtoolbar.table_name_option);
            li_clone.find("input.column_name").select2(gridtoolbar.column_name_option);

            //set select 2 post name
            li_clone.find("input.post_name").select2(gridtoolbar.post_name_option);
            gridtoolbar.update_nestable();
        ...
        call_on_change: function (self) {
            ...
            $('input[type="radio"][name="' + name + '"]').each(function () {
                gridtoolbar.update_data_column(this, 'primary_key', 'radio');
            });
        },
        set_auto_complete: function () {
            $(".show_command").select2(gridtoolbar.show_command_option);
            $(".column_name").select2(gridtoolbar.column_name_option);
            $(".link_key").select2(gridtoolbar.link_key_option);


            //$(".post_name").select2(gridtoolbar.post_name_option);

        },
        update_nestable: function () {
            gridtoolbar.updateOutput($('#gridformartoolbar').data('output', $('#gridformartoolbar-output')));
        },
        ...
        render_table_field: function (uiDraggable, droppable) {
            ...
            //gridtoolbar.update_nestable();
            dd_item.droppable(gridtoolbar.option_droppable);
        },
        ...
        update_data_type: function () {
            ...
            gridtoolbar.updateOutput($('#gridtoolbar').data('output', $('#gridformartoolbar-output')));
        },
