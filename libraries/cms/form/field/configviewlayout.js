(function ($) {
    ...
    // here we go!
    $.field_configviewlayout = function (element, options) {
        ...
        plugin.init = function () {
            ...
            document.title='config menu item params';
            ...
            $element.find('.save-configviewlayout-property').click(function(){
                var post=$element.find('select:not(.disable_post),textarea:not(.disable_post), input:not([readonly],.disable_post)').serializeObject();
                var menu_item_id =$('input[name="menu_item_id"]').val();

                ajaxSavePropertyModule=$.ajax({
                    type: "POST",
                    dataType: "json",
                    url: this_host+'/index.php?enable_load_component=1&option=com_menus&task=item.ajax_save_property_menu_item_of_component&menu_item_id='+menu_item_id,
                    data: post,
                    beforeSend: function () {
                        $('.div-loading').css({
                            display: "block"
    ...
    $.fn.field_configviewlayout = function (options) {

        // iterate through the DOM elements we are attaching the plugin to
        return this.each(function () {
            // if plugin has not already been attached to the element
            if (undefined == $(this).data('field_configviewlayout')) {
                var plugin = new $.field_configviewlayout(this, options);
                $(this).data('field_configviewlayout', plugin);
