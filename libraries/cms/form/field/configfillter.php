 ...
 * @since       1.6
 */
class JFormFieldConfigFillter extends JFormField
{
    /**
     * The form field type.
     *
    ...
    function create_html_list($nodes, $list_data_type, $level)
    {
        ...
        foreach ($nodes as $column) {
            ...
            <div class="dd-handle">
                ...
                <?php echo $column->id ?>
                <button onclick="config_fillter.remove_item_nestable(this)" class="dd-handle-remove pull-right"><i
            ...
            <label>table name<input class="table_name" style="width: 200px" type="text"
                                    onchange="config_fillter.update_data_column(this,'table_name')"
                                    value="<?php echo $column->table_name ?>"/></label>
            <label>Column name<input class="column_name" style="width: 200px" type="text"
                                     onchange="config_fillter.update_data_column(this,'column_name')"
                                     value="<?php echo $column->column_name ?>"/></label>
            <label>Type
                <select style="width: 200px" name="data_type1" onchange="config_fillter.update_data_column(this,'type')"
                        class="data_type">
            ...
            <label>Post name
                <input class="post_name" style="width: 200px" type="text"
                       onchange="config_fillter.update_data_column(this,'post_name')"
                       value="<?php echo $column->post_name ?>"/>
            </label>
            <label>Editable<input <?php echo $column->editable == 1 ? 'checked' : '' ?> type="checkbox"
                                                                                        onchange="config_fillter.update_data_column(this,'editable','checkbox')"
                                                                                        value="1"/></label>
            <label>Primary key<input <?php echo $column->primary_key == 1 ? 'checked' : '' ?> class="primary-key"
                                                                                              data-level="<?php echo $level ?>"
                                                                                              name="primary_key_<?php echo $level ?>"
                                                                                              type="radio"
                                                                                              onchange="config_fillter.primary_key_update_value(this);config_fillter.call_on_change(this)"
                                                                                              value="<?php echo $column->primary_key == 1 ? 1 : 0 ?>"/></label>
            <?php
            echo ob_get_clean();
            if (count($childNodes) > 0) {
                $level = $level + 1;
                JFormFieldConfigfillter::create_html_list($childNodes, $list_data_type, $level);
            }
            echo "</li>";
        }
        echo '</ol>';
    ...
    protected function getInput()
    {
        ...
        $doc->addLessStyleSheet(JUri::root() . "/libraries/cms/form/field/configfillter.less");
        ...
        $doc->addScript(JUri::root() . "/libraries/cms/form/field/configfillter.js");
        ...
        $scriptId = "lib_cms_form_fields_Configfillter" . '_' . JUserHelper::genRandomPassword();
        ob_start();
        ?>
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                config_fillter.init_config_fillter();
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <div class="cf nestable-lists">
                    <div class="row">
                        <div class="col-md-2">
                            <input class="form-control" onchange="config_fillter.filter_table(this)">
                            <ul class="config-upate-table">
                                <?php foreach ($tables as $table) { ?>
                                    ...
                                    ?>
                                    <li class="table">
                                        <div class="input-group Configfillter-item-table">
                                            <div class="input-group-btn"><a data-table="<?php echo $table ?>"
                                                                            class="btn btn-primary plus"><i
                                                        class="im-plus"></i></a></div>
                                            <div class="input-group-btn">
                                                <a data-table="<?php echo $table ?>"
                                                   class="btn btn-danger "><?php echo $table ?></a>
                                            </div>
                                        </div>
                        ...
                        <div class="col-md-10">
                            ...
                            <div class="dd " id="config_fillter1">
                                <?php if (count((array)$object_mode_select_column)) {
                                    $level = 1;
                                    JFormFieldConfigfillter::create_html_list($object_mode_select_column, $list_data_type, $level);
                                } else { ?>
                ...
                <input type="hidden" name="<?php echo $this->name ?>" value="<?php echo $this->value ?>" name=""
                       id="config_fillter1-output"/>
