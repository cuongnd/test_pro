...
abstract class JPluginHelper
{
	...
	protected static function import($plugin, $autocreate = true, JEventDispatcher $dispatcher = null)
	{
        ...
        $only_t3=$input->get('only_t3',0,'string');
        if($only_t3&&$plugin->name!='t3')
            return;
		static $paths = array();
        if($plugin->name!='t3')
        {
            $enable_only_plugin_except_t3=$input->get('enable_only_plugin_except_t3','','string');
            if($enable_only_plugin_except_t3&&$enable_only_plugin_except_t3!=$plugin->name)
            {
                return;
            }
        }
