...
abstract class JHtmlJGrid
{
	...
	public static function is_system($value, $i, $prefix = '', $enabled = true, $checkbox = 'cb', $is_system = null, $is_not_system = null)
	{
		if (is_array($prefix))
		{
			$options = $prefix;
        ...
        $states = array(1 => array('isnotsystem', 'J_IS_SYSTEM', 'JLIB_HTML_IS_NOT_SYSTEM_ITEM', 'J_IS_SYSTEM', true, 'issystem', 'issystem'),
            0 => array('issystem', 'J_IS_NOT_SYSTEM', 'JLIB_HTML_IS_SYSTEM_ITEM', 'J_IS_NOT_SYSTEM', true, 'isnotsystem', 'isnotsystem')
           );
		// Special state for dates
		if ($is_system || $is_not_system)
		{
			$nullDate = JFactory::getDbo()->getNullDate();
			$nowDate = JFactory::getDate()->toUnix();
			...
			$is_system = ($is_system != $nullDate) ? JFactory::getDate($is_system, 'UTC')->setTimeZone($tz) : false;
			$is_not_system = ($is_not_system != $nullDate) ? JFactory::getDate($is_not_system, 'UTC')->setTimeZone($tz) : false;
			...
			if ($is_system)
			{
				$tips[] = JText::sprintf('JLIB_HTML_PUBLISHED_START', $is_system->format(JDate::$format, true));
			}

			if ($is_not_system)
			{
				$tips[] = JText::sprintf('JLIB_HTML_PUBLISHED_FINISHED', $is_not_system->format(JDate::$format, true));
			}
			...
			foreach ($states as $key => $state)
			{
				...
				if ($key == 1)
				{
					...
					if ($is_system > $nullDate && $nowDate < $is_system->toUnix())
					{
						$states[$key][2] = $states[$key][3] = 'JLIB_HTML_PUBLISHED_PENDING_ITEM';
						$states[$key][5] = $states[$key][6] = 'pending';
					}

					if ($is_not_system > $nullDate && $nowDate > $is_not_system->toUnix())
					{
						$states[$key][2] = $states[$key][3] = 'JLIB_HTML_PUBLISHED_EXPIRED_ITEM';
						$states[$key][5] = $states[$key][6] = 'expired';
					}
