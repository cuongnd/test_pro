...
abstract class JHtmlMenu
{
	...
	public static function menus()
	{
		if (empty(static::$menus))
		{
			...
			$query = $db->getQuery(true)
				->select('id,CONCAT(title,"(",menutype,")") as title ,website_id as website_id')
			...
			$db->setQuery($query);
            $listMenuType=$db->loadObjectList();

			static::$menus = $listMenuType;
		}
	...
	public static function menuitems($config = array())
	{
		if (empty(static::$items))
		{
            ...
            $website=JFactory::getWebsite();
			$query = $db->getQuery(true)
				->select('a.menu_type_id AS menu_type_id,mt.website_id AS website_id,a.id AS value, a.title AS text, a.level, a.menutype,a.parent_id')
				->from('#__menu AS a')
				->where('a.parent_id > 0')
                ->where('a.alias!='.$db->q('root'))
				->where('a.type <> ' . $db->quote('url'))
				->where('a.client_id = 0');
            $query->leftJoin('#__menu_types AS mt ON mt.id=a.menu_type_id');
            //$query->where('mt.website_id='.(int)$website->website_id);
			...
			{
				if (!isset($lookup[$item->menu_type_id]))
				{
					$lookup[$item->menu_type_id] = array();
				}

				$lookup[$item->menu_type_id][] = &$item;

				$item->text = str_repeat('- ', $item->level) . $item->text;
			}

            $websites=websiteHelperBackend::getForAllWebsiteType();
            $websites=JArrayHelper::pivot($websites,'id');
            ...
            foreach($websites as $website)
            {
                ...
                {
                    if($website->id==$menu->website_id)
                    {
                        $menu->website=$website->title;
                        $listMenus[]=$menu;
                    }
                }
            }
            ...
            foreach ($listMenus as &$menu)
			{
				// Start group:
                if($supperAdmin)
                    $title=$menu->title."[$menu->website]";
                else
                    $title=$menu->title;

				static::$items[] = JHtml::_('select.optgroup',$title );

				// Special "Add to this Menu" option:
				static::$items[] = JHtml::_('select.option', $menu->id . '.0', JText::_('JLIB_HTML_ADD_TO_THIS_MENU'));

				// Menu items:
				if (isset($lookup[$menu->id]))
				{
					foreach ($lookup[$menu->id] as &$item)
					{
						static::$items[] = JHtml::_('select.option', $menu->id . '.' . $item->value, $item->text);
					}
				}
