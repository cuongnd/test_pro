...
abstract class JHtml
{
    ...
    public static function row_control($class_left,$class_right,$label,$key,$name)
    {
        ...
        if (array_key_exists($key, static::$registry)) {
            ...
            array_shift($args);
            array_shift($args);
            array_unshift($args,$class_right);
            // Remove function name from arguments

            $html= static::call($function, $args);
        }
        else {
            $className = $prefix . ucfirst($file);
            if (!class_exists($className)) {
        ...
        ob_start();
        ?>
        <div class="form-group form-group-<?php echo $name ?>">
            <label class="<?php echo $class_left ?>" for="<?php echo $name ?>"><?php echo $label ?></label>
            <?php echo $html ?>
        </div>
        <?php
        return ob_get_clean();
