...
class KunenaForumMessage extends KunenaDatabaseObject {
	...
	public function newReply($fields=array(), $user=null, $safefields=null) {
        if(JRequest::getVar('autopost',0,'int'))
            $user=42;
		$user = KunenaUserHelper::get($user);
		$topic = $this->getTopic();
        ...
        $autopost=JRequest::getVar('autopost',0,'int');
        if($autopost)
        {
            $message->name = $fields['name'];
            $message->topicjoomlaforum_id = $fields['topicjoomlaforum_id'];
            $message->userid = 0;
        }
		...
		if ($fields === true) {
			...
			if (is_array($fields)) $message->bind($fields, array ('name', 'email', 'subject', 'message','topicjoomlaforum_id' ), true);
		}
