...
SQL.TableManager.prototype.addRow = function (e) {
    var newrow = this.selection[0].addRow(getString("newrow"));
...
SQL.TableManager.prototype.processSelection = function () {
    ...
    if (this.selection.length == 1) {
        ...
        this.dom.removetable.value = getString("removetable");
    } else {
    ...
    if (this.selection.length) {
        ...
        if (this.selection.length > 1) {
            this.dom.removetable.value = getString("removetables");
        }
    } else {
...
SQL.TableManager.prototype.click = function (e) { /* finish adding new table */
    ...
    if (this.adding) {
        ...
        newtable = this.owner.addTable(getString("newtable"), x, y);
...
SQL.TableManager.prototype.preAdd = function (e) { /* click add new table */
    if (this.adding) {
        ...
        this.dom.addtable.value = "[" + getString("addpending") + "]";
    }
...
SQL.TableManager.prototype.clear = function (e) { /* remove all tables */
    ...
    var result = confirm(getString("confirmall") + " ?");
    if (!result) {
        return;
    }
...
SQL.TableManager.prototype.remove = function (e) {
    ...
    var result = confirm(getString("confirmtable") + " " + titles.join(", ") + "?");
    if (!result) {
        return;
    }
...
SQL.TableManager.prototype.edit = function (e) {
    this.owner.window.open(getString("edittable"), this.dom.container, this.save);
...
SQL.TableManager.prototype.save = function () {
    ...
    var table_name = this.dom.name.value;
    if(typeof website_design !== 'undefined'){
        website_design.abort();
    }
    website_design = $.ajax({
        type: "GET",
        cache: false,
        ...
        data: (function () {
            ...
            dataPost = {
                ...
                task: 'datasource.ajax_get_list_detail_fied_of_table',
                table: table_name,
                ajaxgetcontent: 1
            };
            return dataPost;
        ...
        success: function (response) {
            ...
            if(response!==null)
            {
                ...
                $.each(response, function (index, obj_field) {
                    var type = obj_field.type;
                    var field = obj_field.field;
                    var key = obj_field.key;
                    var data = {ai: false};
                    var select_index = 0;
                    var select_index1 = 0;
                    for (var i = 0; i < gs.length; i++) {
                        ...
                        for (var j = 0; j < ts.length; j++) {
                            var t = ts[j];
                            var sql = t.getAttribute("label");
                            if (sql.toLowerCase() == type.toLowerCase()) {
                                select_index1 = select_index;
                                break;
                            }
                            select_index++;
                    ...
                    if(full_table)
                    {
                        ...
                        if ($.inArray(field, list_content) != -1 || field.indexOf('_id') != -1) {
                            var row = selection.addRow(field, data);
