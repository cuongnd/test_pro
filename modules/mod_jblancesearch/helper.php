...
class ModJblanceSearchHelper {	
	...
	function getListJobCateg(){
		$select = JblanceHelper::get('helper.select');		// create an instance of the class selectHelper
		$attribs = "class='inputbox jbl_dropdown' size='1'";
		$lists = $select->getSelectCategoryTree('id_categ', 0, 'MOD_JBLANCE_SEARCH_ALL', $attribs, '');
		return $lists;
	}
	
	function getSelectProjectStatus(){
		...
		$lists 	 = JHTML::_('select.genericlist', $types, 'status', 'class="inputbox jbl_dropdown" size="1"', 'value', 'text', '');
