 ...
 * @since       1.5
 */
class mod_templatebestsaller_helper
{

    public static function get_list_product_best_sale()
    {
        $db=JFactory::getDbo();
        $query=$db->getQuery(true);
        $query->select('products_en_gb.*')
            ->from('#__webtempro_products AS products')
            ->where('products.product_sales=1')
        ...
        $list_product_best_sale=$db->loadObjectList();
        return $list_product_best_sale;
    }
}
