...
(function ($) {
    ...
    // here we go!
    $.ui_textarea = function (element, options) {
        ...
        plugin.validate=function(){
            ...
            var $wapper_textarea=$element.closest('.wapper_textarea');
            ...
            $error_alert=$('<div class="alert textarea-alert alert-danger" '+(random_string!=''?'random-string="'+random_string+'"':'') +' data-block-id="'+block_id+'"><i class="im-notification"></i>'+required_message+'</div>');
            ...
            if(!required)
            {
                ...
                if(value.trim()=='')
                {
                    ...
                    {
                        $error_alert.insertAfter($wapper_textarea);
                    }
                    return false;
                }else{
                    $element_alert.remove();
    ...
    $.fn.ui_textarea = function (options) {

        // iterate through the DOM elements we are attaching the plugin to
        return this.each(function () {
            // if plugin has not already been attached to the element
            if (undefined == $(this).data('ui_textarea')) {
                var plugin = new $.ui_textarea(this, options);
                $(this).data('ui_textarea', plugin);
