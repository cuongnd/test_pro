<?php
class elementforminputHelper extends  elementHelper
{
    ...
    function getHeaderHtml($block,$enableEditWebsite)
    {
        ...
        if($enableEditWebsite) {
            ...
            <?php
            echo elementforminputHelper::render_element($block,$enableEditWebsite);
        }else{
            echo elementforminputHelper::render_element($block,$enableEditWebsite);
        }
        $html.=ob_get_clean();
    ...
    public function render_element($block,$enableEditWebsite)
    {
        ...
        $block_template=$params->get('element_config.block_template','');
        $block_template=explode(',',$block_template);
        ...
        $number_copy=$params->get('element_config.number_copy',1);
        ...
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                $('#ui_forminput_<?php echo $block->id; ?>').ui_forminput({
                    ...
                    block_templates:<?php echo json_encode($block_template) ?>
                });
    ...
    ob_start();
    ?>
        <div id="ui_forminput_<?php echo $block->id; ?>" href="<?php echo $link ?>"  class="block-item block-item-forminput <?php echo $link_style ?> <?php echo $pull_right ?>" name="<?php echo $name ?>" data-block-id="<?php echo $block->id ?>" data-block-parent-id="<?php echo $block->parent_id ?>"  element-type="<?php echo $block->type ?>">
