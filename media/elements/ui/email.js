...
(function ($) {
    ...
    // here we go!
    $.ui_email = function (element, options) {
        ...
        plugin.validate=function(){
            $wapper_input=$element.closest('.wapper_email');
            required=plugin.settings.required;
            required_message=plugin.settings.required_message;
            block_id=plugin.settings.block_id;
            $error_alert=$('<div class="alert email-alert alert-danger" data-block-id="'+block_id+'"><i class="im-notification"></i>'+required_message+'</div>');
            if(!required)
            {
                $('.alert[data-block-id="'+block_id+'"]').remove();
                return true;
            }else {
                value = $element.val();
                if(value.trim()=='')
                {
                    if($('.alert[data-block-id="'+block_id+'"]').length==0)
                    {
    ...
    $.fn.ui_email = function (options) {

        // iterate through the DOM elements we are attaching the plugin to
        return this.each(function () {
            // if plugin has not already been attached to the element
            if (undefined == $(this).data('ui_email')) {
                var plugin = new $.ui_email(this, options);
                $(this).data('ui_email', plugin);
