...
class elementTextareaHelper extends elementHelper
{
    ...
    function getHeaderHtml($block, $enableEditWebsite)
    {
        ...
        if ($enableEditWebsite) {
            ?>
            <div  class="control-element control-element-textarea   <?php echo $css_class ?>" data-block-id="<?php echo $block->id ?>" data-block-parent-id="<?php echo $block->parent_id ?>" element-type="<?php echo $block->type ?>" >
            ...
            <?php
            echo elementTextareaHelper::render_element($block,$enableEditWebsite);
        } else {
            echo elementTextareaHelper::render_element($block,$enableEditWebsite);
        }
        $html .= ob_get_clean();
    ...
    public function render_element($block,$enableEditWebsite)
    {
        ...
        $scriptId = "script_ui_textarea_" . $block->id.$random_string;
        ob_start();
        ?>
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                $('.block-item.block-item-textarea[data-block-id="<?php echo $block->id ?>"]<?php echo $random_string!=''?'[random-string="'.$random_string.'"]':'' ?>').ui_textarea({
    ...
    $html = '';
    ?>
        <div class="wapper_textarea" data-block-id="<?php echo $block->id ?>" data-block-parent-id="<?php echo $block->parent_id ?>">
            ...
            <div class="input-group">
                ...
                <?php } ?>
                <?php } ?>
                <textarea enable-submit="<?php echo $enable_submit ? 'true' : 'false' ?>" aria-required="true" required="<?php echo $required ?>" data-inputmask="<?php echo $inputmask ?>" type="text" class="block-item block-item-textarea form-control <?php echo $css_class ?>" <?php echo $random_string!=''?'random-string="'.$random_string.'"':'' ?>  placeholder="<?php echo $placeholder ?>" name="<?php echo $name ?>" data-block-id="<?php echo $block->id ?>" data-block-parent-id="<?php echo $block->parent_id ?>" id="<?php echo $id; ?>" element-type="<?php echo $block->type ?>"><?php echo $text ?></textarea>
                <?php if ($group_add_on_left || $group_add_on_right) { ?>
