...
class elementEmailHelper extends  elementHelper
{
    ...
    function getHeaderHtml($block,$enableEditWebsite)
    {
        ...
        <?php
            echo elementEmailHelper::render_element($block,$enableEditWebsite);
        }else{
            echo elementEmailHelper::render_element($block,$enableEditWebsite);
        }
        $html.=ob_get_clean();
    ...
    public function render_element($block,$enableEditWebsite)
    {
        ...
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                $('.block-item.block-item-email[data-block-id="<?php echo $block->id ?>"]<?php echo $random_string!=''?'[random-string="'.$random_string.'"]':'' ?>').ui_email({
        ...
        ob_start();
        ?>
        <div class="wapper_email" data-block-id="<?php echo $block->id ?>" data-block-parent-id="<?php echo $block->parent_id ?>">
            ...
            <div class="input-group">
                ...
                <?php } ?>
                <?php } ?>
                <input enable-submit="<?php echo $enable_submit ? 'true' : 'false' ?>" aria-required="true" required="<?php echo $required ?>" data-inputmask="<?php echo $inputmask ?>" type="text" class="block-item block-item-email form-control <?php echo $css_class ?>" <?php echo $random_string!=''?'random-string="'.$random_string.'"':'' ?> value="<?php echo $text ?>" placeholder="<?php echo $placeholder ?>" name="<?php echo $name ?>" data-block-id="<?php echo $block->id ?>" data-block-parent-id="<?php echo $block->parent_id ?>" id="<?php echo $id; ?>" element-type="<?php echo $block->type ?>"/>
                <?php if ($group_add_on_left || $group_add_on_right) { ?>
