...
class FD31_ClosureCompiler {
    ...
    public function min($js)
    {
        ...
        if (preg_match('/^Error\(\d\d?\):/', $response)) {
            if (is_callable($this->_fallbackFunc)) {
                ...
                throw new FD31_Minify_JS_ClosureCompiler_Exception($response);
            }
        }
        if ($response === '') {
            $errors = $this->_getResponse($this->_buildPostBody($js, true));
            throw new FD31_Minify_JS_ClosureCompiler_Exception($errors);
        }
        return $response;
    }
    ...
    protected function _getResponse($postBody)
    {
        ...
        if ($allowUrlFopen) {
            ...
            throw new FD31_Minify_JS_ClosureCompiler_Exception(
    ...
    {
        require_once(FD31_FOUNDRY_LIB . '/jsmin.php');
        return FD31_JSMinPlus::minify($js);
    }
}

class FD31_Minify_JS_ClosureCompiler_Exception extends Exception {}
