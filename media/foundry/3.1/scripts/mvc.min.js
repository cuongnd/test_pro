...
}lastPart=parts[parts.length-1],"[]"==lastPart?current.push(value):current[lastPart]=value}return data}})}(),function(){$.fn.triggerAsync=function(type,data,success,prevented){if("function"==typeof data&&(success=data,data=void 0),this[0]){var event=$.Event(type),old=event.preventDefault;event.preventDefault=function(){old.apply(this,arguments),prevented&&prevented(this)},$.event.trigger({type:type,_success:success},data,this[0])}else success.call(this);return this};var types={},rnamespaces=/\.(.*)$/,$event=$.event;
$event.special["default"]={add:function(handleObj){types[handleObj.namespace.replace(rnamespaces,"")]=!0},setup:function(){return!0}};var oldTrigger=$event.trigger;$event.trigger=function(event,data,elem,onlyHandlers){var type=event.type||event,event="object"==typeof event?event[$.expando]?event:new $.Event(type,event):new $.Event(type),res=oldTrigger.call($.event,event,data,elem,onlyHandlers);return onlyHandlers||event.isDefaultPrevented()||0===event.type.indexOf("default")||(oldTrigger("default."+event.type,data,elem),event._success&&event._success(event)),event.isPaused&&event.isPaused()&&(event.isDefaultPrevented=event.pausedState.isDefaultPrevented,event.isPropagationStopped=event.pausedState.isPropagationStopped),res
}}(),function(){var oldClean=$.cleanData;$.cleanData=function(elems){for(var elem,i=0;void 0!==(elem=elems[i]);i++)$(elem).triggerHandler("destroyed");oldClean(elems)}}(),function(){"$:nomunge";function get_fragment(url){return url=url||window[str_location][str_href],url.replace(/^[^#]*#?(.*)$/,"$1")}var fake_onhashchange,jq_event_special=$.event.special,str_location="location",str_hashchange="hashchange",str_href="href",browser=$.browser,mode=document.documentMode,is_old_ie=browser.msie&&(void 0===mode||8>mode),supports_onhashchange="on"+str_hashchange in window&&!is_old_ie;
$[str_hashchange+"Delay"]=100,jq_event_special[str_hashchange]=$.extend(jq_event_special[str_hashchange],{setup:function(){return supports_onhashchange?!1:($(fake_onhashchange.start),void 0)},teardown:function(){return supports_onhashchange?!1:($(fake_onhashchange.stop),void 0)}}),fake_onhashchange=function(){function init(){set_history=get_history=function(val){return val},is_old_ie&&(iframe=$('<iframe src="javascript:0"/>').hide().insertAfter("body")[0].contentWindow,get_history=function(){return get_fragment(iframe.document[str_location][str_href])
},set_history=function(hash,history_hash){if(hash!==history_hash){var doc=iframe.document;doc.open().close(),doc[str_location].hash="#"+hash}},set_history(get_fragment()))}var timeout_id,iframe,set_history,get_history,self={};return self.start=function(){if(!timeout_id){var last_hash=get_fragment();set_history||init(),navigator.userAgent.match(/Rhino/)||function loopy(){var hash=get_fragment(),history_hash=get_history(last_hash);hash!==last_hash?(set_history(last_hash=hash,history_hash),$(window).trigger(str_hashchange)):history_hash!==last_hash&&(window[str_location][str_href]=window[str_location][str_href].replace(/#.*/,"")+"#"+history_hash),timeout_id=setTimeout(loopy,$[str_hashchange+"Delay"])
}()}},self.stop=function(){iframe||(timeout_id&&clearTimeout(timeout_id),timeout_id=0)},self}()}(),function(){var keymap={},reverseKeyMap={};$.event.key=function(map){$.extend(keymap,map);for(var name in map)reverseKeyMap[map[name]]=name},$.event.key({"\b":"8","	":"9","\r":"13",shift:"16",ctrl:"17",alt:"18","pause-break":"19",caps:"20",escape:"27","num-lock":"144","scroll-lock":"145",print:"44","page-up":"33","page-down":"34",end:"35",home:"36",left:"37",up:"38",right:"39",down:"40",insert:"45","delete":"46"," ":"32",0:"48",1:"49",2:"50",3:"51",4:"52",5:"53",6:"54",7:"55",8:"56",9:"57",a:"65",b:"66",c:"67",d:"68",e:"69",f:"70",g:"71",h:"72",i:"73",j:"74",k:"75",l:"76",m:"77",n:"78",o:"79",p:"80",q:"81",r:"82",s:"83",t:"84",u:"85",v:"86",w:"87",x:"88",y:"89",z:"90",num0:"96",num1:"97",num2:"98",num3:"99",num4:"100",num5:"101",num6:"102",num7:"103",num8:"104",num9:"105","*":"106","+":"107","-":"109",".":"110","/":"111",";":"186","=":"187",",":"188","-":"189",".":"190","/":"191","`":"192","[":"219","\\":"220","]":"221","'":"222","left window key":"91","right window key":"92","select key":"93",f1:"112",f2:"113",f3:"114",f4:"115",f5:"116",f6:"117",f7:"118",f8:"119",f9:"120",f10:"121",f11:"122",f12:"123"}),$.Event.prototype.keyName=function(){var event=this,test=/\w/,key_Key=reverseKeyMap[(event.keyCode||event.which)+""],char_Key=String.fromCharCode(event.keyCode||event.which),key_Char=event.charCode&&reverseKeyMap[event.charCode+""],char_Char=event.charCode&&String.fromCharCode(event.charCode);
return char_Char&&test.test(char_Char)?char_Char.toLowerCase():key_Char&&test.test(key_Char)?char_Char.toLowerCase():char_Key&&test.test(char_Key)?char_Key.toLowerCase():key_Key&&test.test(key_Key)?key_Key.toLowerCase():"keypress"==event.type?event.keyCode?String.fromCharCode(event.keyCode):String.fromCharCode(event.which):!event.keyCode&&event.which?String.fromCharCode(event.which):reverseKeyMap[event.keyCode+""]}}(),function(){var event=$.event,findHelper=function(events,types,callback,selector){var t,type,typeHandlers,all,h,handle,namespaces,namespace,match;
for(t=0;t<types.length;t++)for(type=types[t],all=type.indexOf(".")<0,all||(namespaces=type.split("."),type=namespaces.shift(),namespace=new RegExp("(^|\\.)"+namespaces.slice(0).sort().join("\\.(?:.*\\.)?")+"(\\.|$)")),typeHandlers=(events[type]||[]).slice(0),h=0;h<typeHandlers.length;h++)handle=typeHandlers[h],match=all||namespace.test(handle.namespace),match&&(selector?handle.selector===selector&&callback(type,handle.origHandler||handle.handler):null===selector?callback(type,handle.origHandler||handle.handler,handle.selector):handle.selector||callback(type,handle.origHandler||handle.handler))
};event.find=function(el,types,selector){var events=($._data(el)||{}).events,handlers=[];return events?(findHelper(events,types,function(type,handler){handlers.push(handler)},selector),handlers):handlers},event.findBySelector=function(el,types){var events=$._data(el).events,selectors={},add=function(selector,event,handler){var select=selectors[selector]||(selectors[selector]={}),events=select[event]||(select[event]=[]);events.push(handler)};return events?(findHelper(events,types,function(type,handler,selector){add(selector||"",type,handler)
},null),selectors):selectors},event.supportTouch="ontouchend"in document,$.fn.respondsTo=function(events){return this.length?event.find(this[0],$.isArray(events)?events:[events]).length>0:!1},$.fn.triggerHandled=function(event,data){return event="string"==typeof event?$.Event(event):event,this.trigger(event,data),event.handled},event.setupHelper=function(types,startingEvent,onFirst){onFirst||(onFirst=startingEvent,startingEvent=null);var add=function(handleObj){var bySelector,selector=handleObj.selector||"";
selector?(bySelector=event.find(this,types,selector),bySelector.length||$(this).delegate(selector,startingEvent,onFirst)):event.find(this,types,selector).length||event.add(this,startingEvent,onFirst,{selector:selector,delegate:this})},remove=function(handleObj){var bySelector,selector=handleObj.selector||"";selector?(bySelector=event.find(this,types,selector),bySelector.length||$(this).undelegate(selector,startingEvent,onFirst)):event.find(this,types,selector).length||event.remove(this,startingEvent,onFirst,{selector:selector,delegate:this})
};$.each(types,function(){event.special[this]={add:add,remove:remove,setup:function(){},teardown:function(){}}})}}(),function(){var returnFalse=function(){return!1},returnTrue=function(){return!0};$.Event.prototype.isPaused=returnFalse,$.Event.prototype.pause=function(){this.pausedState={isDefaultPrevented:this.isDefaultPrevented()?returnTrue:returnFalse,isPropagationStopped:this.isPropagationStopped()?returnTrue:returnFalse},this.stopImmediatePropagation(),this.preventDefault(),this.isPaused=returnTrue},$.Event.prototype.resume=function(){var handleObj=this.handleObj,currentTarget=this.currentTarget,origType=$.event.special[handleObj.origType],origHandle=origType&&origType.handle;
origType||($.event.special[handleObj.origType]={}),$.event.special[handleObj.origType].handle=function(ev){ev.handleObj===handleObj&&ev.currentTarget===currentTarget&&(origType?$.event.special[handleObj.origType].handle=origHandle:delete $.event.special[handleObj.origType])},delete this.pausedState,this.isPaused=this.isImmediatePropagationStopped=returnFalse,this.isPropagationStopped()||$.event.trigger(this,[],this.target)}}(),function(){var timer,resizers=$(),resizeCount=0,win=$(window),windowWidth=0,windowHeight=0;
$(function(){windowWidth=win.width(),windowHeight=win.height()}),$.event.special.resize={setup:function(){return this!==window&&(resizers.push(this),$.unique(resizers)),this!==window},teardown:function(){return resizers=resizers.not(this),this!==window},add:function(handleObj){var origHandler=handleObj.handler;handleObj.origHandler=origHandler,handleObj.handler=function(ev,data){var isWindow=this===window;if(isWindow&&ev.originalEvent){var width=win.width(),height=win.height();return(width!=windowWidth||height!=windowHeight)&&(windowWidth=width,windowHeight=height,clearTimeout(timer),timer=setTimeout(function(){win.trigger("resize")
},1)),void 0}if(0===resizeCount){resizeCount++;var where=data===!1?ev.target:this;if($.event.handle.call(where,ev),ev.isPropagationStopped())return resizeCount--,void 0;for(var child,sub,index=resizers.index(this),length=resizers.length;++index<length&&(child=resizers[index])&&(isWindow||$.contains(where,child));)if($.event.handle.call(child,ev),ev.isPropagationStopped())for(;++index<length&&(sub=resizers[index]);)if(!$.contains(child,sub)){index--;break}ev.stopImmediatePropagation(),resizeCount--}else handleObj.origHandler.call(this,ev,data)
}}},$([document,window]).bind("resize",function(){})}(),function(){var oldClosest=$.fn.closest;$.fn.closest=function(selectors,context){var res,result,i,selector,selector,rooted={},rootedIsEmpty=!0,selectorsArr=selectors;if("string"==typeof selectors&&(selectorsArr=[selectors]),$.each(selectorsArr,function(i,selector){if(0==selector.indexOf(">")){if(-1!=selector.indexOf(" "))throw" closest does not work with > followed by spaces!";rooted[selectorsArr[i]=selector.substr(1)]=selector,"string"==typeof selectors&&(selectors=selector.substr(1)),rootedIsEmpty=!1
...
if(cookie.substring(0,name.length+1)==name+"="){cookieValue=decodeURIComponent(cookie.substring(name.length+1));break}}if(jQuery.evalJSON&&cookieValue&&cookieValue.match(/^\s*\{/))try{cookieValue=jQuery.evalJSON(cookieValue)}catch(e){}return cookieValue}options=options||{},null===value&&(value="",options.expires=-1),"object"==typeof value&&jQuery.toJSON&&(value=jQuery.toJSON(value));var expires="";if(options.expires&&("number"==typeof options.expires||options.expires.toUTCString)){var date;"number"==typeof options.expires?(date=new Date,date.setTime(date.getTime()+1e3*60*60*24*options.expires)):date=options.expires,expires="; expires="+date.toUTCString()
}var path=options.path?"; path="+options.path:"",domain=options.domain?"; domain="+options.domain:"",secure=options.secure?"; secure":"";document.cookie=[name,"=",encodeURIComponent(value),expires,path,domain,secure].join("")}}(),function(){$.fn.compare=function(element){try{element=element.jquery?element[0]:element}catch(e){return null}if(window.HTMLElement){var s=HTMLElement.prototype.toString.call(element);if("[xpconnect wrapped native prototype]"==s||"[object XULElement]"==s||"[object Window]"===s)return null
}if(this[0].compareDocumentPosition)return this[0].compareDocumentPosition(element);if(this[0]==document&&element!=document)return 8;var number=(this[0]!==element&&this[0].contains(element)&&16)+(this[0]!=element&&element.contains(this[0])&&8),docEl=document.documentElement;if(this[0].sourceIndex)number+=this[0].sourceIndex<element.sourceIndex&&4,number+=this[0].sourceIndex>element.sourceIndex&&2,number+=(this[0].ownerDocument!==element.ownerDocument||this[0]!=docEl&&this[0].sourceIndex<=0||element!=docEl&&element.sourceIndex<=0)&&1;
else{var compare,range=document.createRange(),sourceRange=document.createRange();range.selectNode(this[0]),sourceRange.selectNode(element),compare=range.compareBoundaryPoints(Range.START_TO_START,sourceRange)}return number}}(),function(){var getComputedStyle=document.defaultView&&document.defaultView.getComputedStyle,rupper=/([A-Z])/g,rdashAlpha=/-([a-z])/gi,fcamelCase=function(all,letter){return letter.toUpperCase()},getStyle=function(elem){return getComputedStyle?getComputedStyle(elem,null):elem.currentStyle?elem.currentStyle:void 0
},rfloat=/float/i,rnumpx=/^-?\d+(?:px)?$/i,rnum=/^-?\d/;$.curStyles=function(el,styles){if(!el)return null;for(var oldName,val,left,rsLeft,camelCase,name,currentS=getStyle(el),style=el.style,results={},i=0;i<styles.length;i++)name=styles[i],oldName=name.replace(rdashAlpha,fcamelCase),rfloat.test(name)&&(name=$.support.cssFloat?"float":"styleFloat",oldName="cssFloat"),getComputedStyle?(name=name.replace(rupper,"-$1").toLowerCase(),val=currentS.getPropertyValue(name),"opacity"===name&&""===val&&(val="1"),results[oldName]=val):(camelCase=name.replace(rdashAlpha,fcamelCase),results[oldName]=currentS[name]||currentS[camelCase],!rnumpx.test(results[oldName])&&rnum.test(results[oldName])&&(left=style.left,rsLeft=el.runtimeStyle.left,el.runtimeStyle.left=el.currentStyle.left,style.left="fontSize"===camelCase?"1em":results[oldName]||0,results[oldName]=style.pixelLeft+"px",style.left=left,el.runtimeStyle.left=rsLeft));
return results},$.fn.curStyles=function(){return $.curStyles(this[0],$.makeArray(arguments))}}(),function(){$.fn.range=function(){return $.Range(this[0])};var convertType=function(type){return type.replace(/([a-z])([a-z]+)/gi,function(all,first,next){return first+next.toLowerCase()}).replace(/_/g,"")},reverse=function(type){return type.replace(/^([a-z]+)_TO_([a-z]+)/i,function(all,first,last){return last+"_TO_"+first})},getWindow=function(element){return element?element.ownerDocument.defaultView||element.ownerDocument.parentWindow:window
...
},scrollOffset=function(win){var win=win||window;return doc=win.document.documentElement,body=win.document.body,{left:(doc&&doc.scrollLeft||body&&body.scrollLeft||0)+(doc.clientLeft||0),top:(doc&&doc.scrollTop||body&&body.scrollTop||0)+(doc.clientTop||0)}};support.moveToPoint=!!$.Range().range.moveToPoint}(),function(){var withinBox=function(x,y,left,top,width,height){return y>=top&&top+height>y&&x>=left&&left+width>x};$.fn.within=function(left,top,useOffsetCache){var ret=[];return this.each(function(){var q=$(this);
if(this==document.documentElement)return ret.push(this);var offset=useOffsetCache?$.data(this,"offsetCache")||$.data(this,"offsetCache",q.offset()):q.offset(),res=withinBox(left,top,offset.left,offset.top,this.offsetWidth,this.offsetHeight);res&&ret.push(this)}),this.pushStack($.unique(ret),"within",left+","+top)},$.fn.withinBox=function(left,top,width,height,cache){var ret=[];return this.each(function(){var q=$(this);if(this==document.documentElement)return this.ret.push(this);var offset=cache?$.data(this,"offset")||$.data(this,"offset",q.offset()):q.offset(),ew=q.width(),eh=q.height();
res=!(offset.top>top+height||offset.top+eh<top||offset.left>left+width||offset.left+ew<left),res&&ret.push(this)}),this.pushStack($.unique(ret),"withinBox",$.makeArray(arguments).join(","))}}(),function(){var initializing=!1,makeArray=$.makeArray,isFunction=$.isFunction,isArray=$.isArray,extend=$.extend,getObject=$.String.getObject,concatArgs=function(arr,args){return arr.concat(makeArray(args))},fnTest=/xyz/.test(function(){})?/\b_super\b/:/.*/,inheritProps=function(newProps,oldProps,addTo){addTo=addTo||newProps;
...
return{processor:processor,parts:parts,delegate:arr?convertedName[0]:void 0}},processors:{},listensTo:[],defaults:{},hostname:"parent"},{setup:function(elem,options){var instance=this,Class=instance[STR_CONSTRUCTOR],prototype=instance[STR_PROTOTYPE];instance.instanceId=$.uid(_fullName+"_");var instanceOptions=instance.options=extend(!0,{},Class.defaults,Class.defaultOptions,options),element=instance.element=$(elem);Class.protoFactory&&(prototype=Class.protoFactory.apply(Class,[instance,instanceOptions,element]),extend(!0,instance,prototype));
var _fullName=Class._fullName;if((data(elem)||data(elem,{}))[_fullName]=instance,!element.data("directSelector")){var selector=$.uid("DS");element.addClass(selector).data("directSelector","."+selector)}for(name in instanceOptions)if(name.match(/^\{.+\}$/)){var key=name.replace(/^\{|\}$/g,""),val=instanceOptions[name];instance[key]=isString(val)?function(instance,selector,funcName){selector=/^(\.|\#)$/.test(selector)?selector+funcName:selector;var selectorFunc=function(filter){var elements=instance.element.find(selector);
return filter&&(elements=elements.filter(filter)),elements};return selectorFunc.selector=selector,selectorFunc.css=function(){var cssRule=selectorFunc.cssRule;if(!cssRule){var directSelector=element.data("directSelector"),ruleSelector=$.map(selector.split(","),function(selector){return directSelector+" "+selector});cssRule=selectorFunc.cssRule=$.cssRule(ruleSelector),cssRule.important=!0}return arguments.length?cssRule.css.apply(cssRule,arguments):cssRule},selectorFunc}(instance,val,key):val}var views=instanceOptions.view,__view=instance.view;
instance.view=function(){return __view.apply(this,arguments)},each(views||{},function(name){instance.view[name]=function(useHtml){var args=makeArray(arguments);return $.isBoolean(useHtml)?args=args.slice(1):useHtml=!1,instance.view.apply(instance,[useHtml,name].concat(args))}}),$.extend(instance,instanceOptions.controller),instance.pluginInstances={},instance.called="init",instance._bind();var __init=instance.init||$.noop;return instance.init=function(){return instance.init=__init,result=__init.apply(instance,arguments),instance.trigger("init."+Class.fullName.toLowerCase(),[instance]),result
},[element,instanceOptions].concat(makeArray(arguments).slice(2))},on:function(eventName){var args=makeArray(arguments),element=this.element,length=args.length;return 2==length?this._binder(element,eventName,args[1]):3==length&&isString(args[1])?this._binder(element,eventName,args[2],args[1]):this._binder(args[1],eventName,args[2])},_bind:function(){var instance=this,Class=instance[STR_CONSTRUCTOR],actions=Class.actions,bindings=instance._bindings=[],element=instance.element;each(actions||{},function(name){if(actions.hasOwnProperty(name)){var ready=Class.actions[name]||Class._action(name,instance.options);
$.isControllerInstance(ready.delegate)&&(ready.delegate=ready.delegate.element),bindings.push(ready.processor(ready.delegate||element,ready.parts[2],ready.parts[1],name,instance))}});var destroyCB=shifter(this,"destroy");return element.bind("destroyed",destroyCB),bindings.push(function(el){$(el).unbind("destroyed",destroyCB)}),bindings.length},_binder:function(el,eventName,func,selector){return"string"==typeof func&&(func=shifter(this,func)),this._bindings.push(binder(el,eventName,func,selector)),this._bindings.length
},_unbind:function(){var el=this.element[0];each(this._bindings,function(key,value){value(el)}),this._bindings=[]},trigger:function(name){var el=this.element;if(el){var event=$.Event(name);return el.trigger.apply(el,[event].concat($.makeArray(arguments).slice(1))),event}},delegate:function(element,selector,eventName,func){return"string"==typeof element&&(func=eventName,eventName=selector,selector=element,element=this.element),this._binder(element,eventName,func,selector)},update:function(options){extend(this.options,options),this._unbind(),this._bind()
},destroy:function(){if(!this._destroyed){var fname=this[STR_CONSTRUCTOR]._fullName;for(pname in this.pluginInstances)this.removePlugin(pname);this._destroyed=!0,this.element.removeClass(fname),this._unbind(),delete this._actions,delete this.element.data("controllers")[fname],$(this).triggerHandler("destroyed"),this.element=$()}},find:function(selector){return this.element.find(selector)},view:function(){var name,args=makeArray(arguments),options=args,useHtml=!1,context=this[STR_CONSTRUCTOR].component||$,html="",view=this.options.view||{};
...
}),instances},addPlugin:function(name,plugin,options){if(name){(!plugin&&!options||$.isPlainObject(plugin))&&(options=plugin,plugin=[this.Class.root,this.Class.fullName,$.String.capitalize(name)].join(".")),$.isString(plugin)&&(plugin=$.getController(plugin));var isPluginInstance=$.isControllerInstance(plugin);if(isFunction(plugin)||isPluginInstance){options=$.extend(!0,{element:this.element},options,(this.options.plugin||{})[name]||{});var type=isPluginInstance?"instance":$.isController(plugin)?"controller":"function";
this.trigger("addPlugin",[name,plugin,options,type]);var hostname=this.Class.hostname;options["{"+hostname+"}"]=this;var pluginInstance;switch(type){case"instance":pluginInstance=plugin,plugin.update(options),plugin[hostname]=this;break;case"controller":pluginInstance=options.element.addController(plugin,options);break;case"function":pluginInstance=plugin(this,options)}if(pluginInstance){if(this.pluginInstances[name]=pluginInstance,"function"!==type){var hostOptions={};hostOptions["{"+name+"}"]=pluginInstance,this.update(hostOptions)
}return this.trigger("registerPlugin",[name,pluginInstance,options,type]),pluginInstance}}}},removePlugin:function(name){var plugin=this.getPlugin(name);if(plugin)return this.trigger("removePlugin",[name,plugin]),delete this.pluginInstances[name],$.isFunction(plugin.destroy)?plugin.destroy():null},invokePlugin:function(name,method,args){var plugin=this.getPlugin(name);if(plugin&&$.isFunction(plugin[method]))return this.trigger("invokePlugin",[name,plugin,args]),plugin[method].apply(this,args)},getMessageGroup:function(){var messageGroup=$.isFunction(this.messageGroup)?this.messageGroup():this.element.find("[data-message-group]");
return messageGroup.length<1&&(messageGroup=$("<div data-message-group></div>").prependTo(this.element)),messageGroup},setMessage:function(message,type){var defaultOptions={type:"warning",message:"",parent:this.getMessageGroup(),element:$('<div class="alert fade in"><button type="button" class="close" data-foundry-dismiss="alert">×</button></div>')},userOptions={},isDeferred=$.isDeferred(message);userOptions=$.isPlainObject(message)&&!isDeferred?message:{message:message,type:type||"warning"};var options=$.extend({},defaultOptions,userOptions),element=options.element;
if($.isDeferred(message)){var myself=arguments.callee,context=this;message.done(function(message,type){options.message=message,options.type=type||"warning",myself.call(context,options),element.show()})}else element.addClass("alert-"+options.type).append(options.message),$("html").has(element).length<1&&element.appendTo(options.parent);return element},clearMessage:function(){this.getMessageGroup().empty()},_set_called:!0});var processors=$.Controller.Class.processors,basicProcessor=function(el,event,selector,methodName,controller){return binder(el,event,shifter(controller,methodName),selector)
...
},isController=function(controller){return isFunction(controller)&&controller.hasOwnProperty("_fullName")},flattenControllers=function(controllers){return $.map(controllers,function(controller){return isArray(controller)?flattenControllers(controller):getController(controller)})};$.getController=getController,$.isController=function(controller){return!!getController(controller)},$.isControllerInstance=function(instance){return instance&&instance[STR_CONSTRUCTOR]&&isController(instance[STR_CONSTRUCTOR])},$.isControllerOf=function(instance,controllers){if(!controllers)return!1;
isArray(controllers)||(controllers=[controllers]);for(var i=0;i<controllers.length;i++){var controller=getController(controllers[i]);if(instance instanceof controller)return!0}return!1},$.fn.extend({controllers:function(){var candidates=flattenControllers(makeArray(arguments)),instances=[];return this.each(function(){var controllers=$.data(this,"controllers");each(controllers||{},function(_fullName,instance){controllers.hasOwnProperty(_fullName)&&(!candidates.length||$.isControllerOf(instance,candidates))&&instances.push(instance)
})}),instances},controller:function(controller,options){return void 0===options?this.controllers(controller)[0]:(this.addController.apply(this,arguments),this)},hasController:function(controller){var _fullName=(getController(controller)||{})._fullName||(isString(controller)?underscoreAndRemoveController(normalizeController(controller)):"");return _fullName?($(this).data("controllers")||{}).hasOwnProperty(_fullName):!1},addController:function(controller,options,callback){var Controller=getController(controller);
if(Controller){var instances=[];return this.each(function(){if(1===this.nodeType&&"SCRIPT"!=this.nodeName){var existingInstance=$(this).controller(controller);if(existingInstance)return instances.push(existingInstance),void 0;var instance=new Controller(this,options);isFunction(callback)&&callback.apply(instance,[$(this),instance]),instances.push(instance)}}),instances.length>1?instances:instances[0]}},removeController:function(controller){return this.each(function(){for(var instances=$(this).controllers(controller);instances.length;)instances.shift().destroy()
}),this},addControllerWhenAvailable:function(controller){var elements=this,args=arguments,task=$.Deferred();return $.isController(controller)&&(controller=controller.fullName),isString(controller)?($.module("$:/Controllers/"+controller).pipe(function(){var instance=elements.addController.apply(elements,args);task.resolveWith(instance,[elements,instance])},task.reject,task.fail),task):task.reject()},implement:function(){return this.addController.apply(this,arguments),this}})}(),function(){var toId=function(src){return src.replace(/^\/\//,"").replace(/[\/\.]/g,"_")
},makeArray=$.makeArray,id=1,$view=$.View=function(view,data,helpers,callback){"function"==typeof helpers&&(callback=helpers,helpers=void 0);var deferreds=getDeferreds(data);if(deferreds.length){var deferred=$.Deferred();return deferreds.push(get(view,!0)),$.when.apply($,deferreds).then(function(resolved){var result,objs=makeArray(arguments),renderer=objs.pop()[0];if(isDeferred(data))data=usefulPart(resolved);else for(var prop in data)isDeferred(data[prop])&&(data[prop]=usefulPart(objs.shift()));result=renderer(data,helpers),deferred.resolve(result),callback&&callback(result)
}),deferred.promise()}var response,async="function"==typeof callback,deferred=get(view,async);return async?(response=deferred,deferred.done(function(renderer){callback(renderer(data,helpers))})):deferred.done(function(renderer){response=renderer(data,helpers)}),response},checkText=function(text,url){if(!text.match(/[^\s]/))throw"$.View ERROR: There is no template or an empty template at "+url},get=function(url,async){return $.ajax({url:url,dataType:"view",async:async})},isDeferred=function(obj){return obj&&$.isFunction(obj.always)
},getDeferreds=function(data){var deferreds=[];if(isDeferred(data))return[data];for(var prop in data)isDeferred(data[prop])&&deferreds.push(data[prop]);return deferreds},usefulPart=function(resolved){return $.isArray(resolved)&&3===resolved.length&&"success"===resolved[1]?resolved[0]:resolved};$.ajaxTransport("view",function(options,orig){var type,el,id,jqXHR,url=orig.url,suffix=url.match(/\.[\w\d]+$/),response=function(text){var func=type.renderer(id,text);return $view.cache&&($view.cached[id]=func),{view:func}
};if((el=document.getElementById(url))&&(suffix="."+el.type.match(/\/(x\-)?(.+)/)[2]),suffix||(suffix=$view.ext,url+=$view.ext),id=toId(url),url.match(/^\/\//)){var sub=url.substr(2);url="undefined"==typeof steal?url="/"+sub:steal.root.mapJoin(sub)+""}type=$view.types[suffix];var template=$.template()[orig.url];return{send:function(headers,callback){return template?(type=$view.types["."+template.type],callback(200,"success",response(template.content))):$view.cached[id]?callback(200,"success",{view:$view.cached[id]}):(el?callback(200,"success",response(el.innerHTML)):jqXHR=$.ajax({async:orig.async,url:url,dataType:"text",error:function(){checkText("",url),callback(404)
