...
var isIframe=!$.isString(content)&&$(content).is("iframe"),refContent=isIframe?document.createElement("div"):self.stripScript(content);bodyCss.width=isIframe&&"auto"==options.width?content.contents().width():options.width,bodyCss.height=isIframe&&"auto"==options.height?content.contents().height():options.height,refBody.css(bodyCss).toggleClass("type-iframe",isIframe).html(refContent),refBody.width(),refBody.height(),"border-box"!==refBody.css("boxSizing")&&(bodyCss.width=refWidth,bodyCss.height=refHeight),bodyCss.width<bodyCss.minWidth&&(bodyCss.width=bodyCss.minWidth),bodyCss.height<bodyCss.minHeight&&(bodyCss.height=bodyCss.minHeight);
