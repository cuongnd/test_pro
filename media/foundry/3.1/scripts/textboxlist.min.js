...
},filterItem:function(item){var options=self.options,filterItem=options.filterItem;$.isFunction(filterItem)&&(item=filterItem.call(self,item));var items=self.itemsByTitle;if($.isString(item)&&""!==item){var title=item,key=self.getItemKey(title);item=items.hasOwnProperty(key)?self.itemsByTitle[key]:{id:$.uid("item-"),title:title,key:self.getItemKey(title)}}return item.name=item.name||self.options.name,item.html=item.html||self.view.itemContent(!0,item),item},createItem:function(item){item.key=self.getItemKey(item.title),self.items[item.id]=item,self.itemsByTitle[item.key]=item
},deleteItem:function(id){var item=self.items[id];delete self.items[id];var key=self.options.caseSensitive?item.title:item.title.toLowerCase();delete self.itemsByTitle[key]},addItem:function(item){if(item){var options=self.options,max=options.max;if(!(null!==max&&self.item().length>=max)&&(item=self.filterItem(item),$.isPlainObject(item))){var itemEl,existingItemEl=self.item().filterBy("id",item.id);return options.unique&&existingItemEl.length>0&&(itemEl=existingItemEl),itemEl||(itemEl=self.view.item(item).attr("data-id",item.id)),self.createItem(item),itemEl.insertBefore(self.textField()),self.trigger("addItem",[item]),item
}}},removeItem:function(id){var item=self.items[id];self.item().filterBy("id",id).remove(),self.deleteItem(id),self.trigger("removeItem",[item])},clearItems:function(){self.item().each(function(){self.removeItem($(this).data("id"))})},getAddedItems:function(){var addedItems=[];return self.item().each(function(){var item=$(this),id=item.data("id");addedItems.push(self.items[id])}),addedItems},"{self} addItem":function(){self.setLayout()},"{self} removeItem":function(){self.setLayout()},"{itemRemoveButton} click":function(button){var item=button.parents(self.item.selector);
self.removeItem(item.data("id"))},"{textField} keydown":function(textField,event){var keyCode=event.keyCode;textField.data("realEnterKey",keyCode==KEYCODE.ENTER)},"{textField} keypress":function(textField,event){var keydownIsEnter=textField.data("realEnterKey"),keypressIsEnter=event.keyCode==KEYCODE.ENTER;textField.data("realEnterKey",keydownIsEnter&&keypressIsEnter);var keyword=$.trim(self.textField().val());switch(event.keyCode){case KEYCODE.ENTER:if(textField.data("realEnterKey")){var event=self.trigger("useItem",[keyword]),item=event.item;
if(void 0===item&&(item=keyword),item===!1||null===item)return;self.addItem(item),textField.val("")}}},"{textField} keyup":function(textField,event){var item=$.trim(self.textField().val()),canRemoveItemUsingBackspace="canRemoveItemUsingBackspace";switch(event.keyCode){case KEYCODE.BACKSPACE:if(""===item)if(self[canRemoveItemUsingBackspace]){var prevItem=textField.prev(self.item.selector);prevItem.length>0&&self.removeItem(prevItem.data("id"))}else self[canRemoveItemUsingBackspace]=!0;break;default:self[canRemoveItemUsingBackspace]=!1
}},"{self} click":function(el,event){var textField=self.textField();textField.is(event.target)||textField.focus()},"{textField} focusin":function(){self.activated||(self.activated=!0,self.trigger("textboxlistActivate"))},"{self} mousedown":function(){self.focusing=!0},"{self} mouseup":function(){self.focusing=!1},"{self} focusout":function(){self.focusing||(self.activated=!1,self.deactivateTimer=setTimeout(function(){self.activated||self.trigger("textboxlistDeactivate")},1))}}}),$(document).on("click.textboxlist.data-api","[data-textboxlist]",function(){$(this).addController($.Controller.TextboxList).textField().focus()
}).on("focus.textboxlist.data-api","[data-textboxlist] [data-textboxlist-textField]",function(){$(this).parents("[data-textboxlist]").addController($.Controller.TextboxList)}),$.template("textboxlist/menu",'<div class="textboxlist-autocomplete" data-textboxlist-autocomplete><div class="textboxlist-autocomplete-inner" data-textboxlist-autocomplete-viewport><div class="textboxlist-autocomplete-loading" data-textboxlist-autocomplete-loading></div><div class="textboxlist-autocomplete-empty" data-textboxlist-autocomplete-empty></div><ul class="textboxlist-menu" data-textboxlist-menu></ul></div></div>'),$.template("textboxlist/menuItem",'<li class="textboxlist-menuItem" data-textboxlist-menuItem>[%== html %]</li>'),$.template("textboxlist/loadingHint",'<i class="textboxlist-autocomplete-loading-indicator"></i>'),$.template("textboxlist/emptyHint",'<span class="textboxlist-autocomplete-empty-text">No items found.</span>'),$.Controller("Textboxlist.Autocomplete",{defaultOptions:{view:{menu:"textboxlist/menu",menuItem:"textboxlist/menuItem",loadingHint:"textboxlist/loadingHint",emptyHint:"textboxlist/emptyHint"},cache:!0,minLength:1,limit:10,highlight:!0,caseSensitive:!1,exclusive:!1,query:null,position:{my:"left top",at:"left bottom",collision:"none"},filterItem:null,showEmptyHint:!1,showLoadingHint:!1,"{menu}":"[data-textboxlist-menu]","{menuItem}":"[data-textboxlist-menuItem]","{viewport}":"[data-textboxlist-autocomplete-viewport]","{loadingHint}":"[data-textboxlist-autocomplete-loading]","{emptyHint}":"[data-textboxlist-autocomplete-empty]"}},function(self){return{init:function(){if(!self.element.data(self.Class.fullName))return self.destroy(),self.view.menu().appendTo("body").data(self.Class.fullName,!0).addController(self.Class,self.options),void 0;
...
return self.query=function(keyword){return func.call(self,keyword)},void 0}if($.isArray(query)){var dataset=query;return self.query=function(keyword){var task=$.Deferred(),keyword=keyword.toLowerCase();return setTimeout(function(){var result=$.grep(dataset,function(item){return item.title.toLowerCase().indexOf(keyword)>-1});task.resolve(result)},0),task},void 0}},setLayout:function(){self.hidden||self.element.css({opacity:1,width:self.textboxlist.element.outerWidth()}).position(self.options.position)},"{window} resize":$.debounce(function(){self.element.css("opacity",0),self.setLayout()
...
var textboxlist=self.textboxlist,autocomplete=self,element=self.element,options=self.options,menu=self.menu();if(items.length<1)return options.showEmptyHint?(menu.empty(),element.addClass("empty"),textboxlist.trigger("renderMenu",[menu,autocomplete,textboxlist]),self.show()):self.hide(),void 0;element.removeClass("empty"),options.cache&&menu.data("keyword")===keyword||(menu.empty(),$.each(items,function(i,item){textboxlist.trigger("filterItem",[item,autocomplete,textboxlist]);var filterItem=options.filterItem;
if($.isFunction(filterItem)&&(item=filterItem.call(self,item,keyword)),$.isPlainObject(item)&&!item.discard){var html=item.menuHtml||item.title;self.view.menuItem({html:html}).data("item",item).appendTo(menu)}}),menu.data("keyword",keyword));var menuItems=self.menuItem();return textboxlist.trigger("filterMenu",[menu,menuItems,autocomplete,textboxlist]),menuItems.filter(":not(.hidden)").length<1&&(element.addClass("empty"),!options.showEmptyHint)?self.hide():(options.exclusive&&self.menuItem(":not(.hidden):first").addClass("active"),textboxlist.trigger("renderMenu",[menu,autocomplete,textboxlist]),self.show(),void 0)
}),"{textboxlist.textField} keydown":function(textField,event){clearTimeout(self.sleep);var activeMenuItem=self.menuItem(".active:not(.hidden)");switch(activeMenuItem.length<1&&(activeMenuItem=!1),self.textboxlist.textField(),event.keyCode){case KEYCODE.UP:self.menuItem().removeClass("active"),activeMenuItem?activeMenuItem.prev(self.menuItem.selector+":not(.hidden)").addClass("active"):self.menuItem(":not(.hidden):last").addClass("active"),event.preventDefault();break;case KEYCODE.DOWN:self.menuItem().removeClass("active"),activeMenuItem?activeMenuItem.next(self.menuItem.selector+":not(.hidden)").addClass("active"):self.menuItem(":not(.hidden):first").addClass("active"),event.preventDefault();
...
if(exclusive&&(event.item=!1),self.hidden){var lastItem=self.lastItem;return exclusive&&lastItem&&lastItem.keyword==keyword&&(event.item=lastItem.item),void 0}var activeMenuItem=self.menuItem(".active");if(activeMenuItem.length>0){var item=activeMenuItem.data("item");event.item=item}self.hide()},"{menuItem} mousedown":function(){self.textboxlist.focusing=!0},"{menuItem} mouseup":function(){self.textboxlist.focusing=!1},"{menuItem} click":function(menuItem){self.hide();var item=menuItem.data("item");self.textboxlist.addItem(item);
var textField=self.textboxlist.textField().val("");setTimeout(function(){textField.focus()},150)},"{menuItem} mouseover":function(menuItem){self.menuItem().removeClass("active"),menuItem.addClass("active")},"{menuItem} mouseout":function(){self.menuItem().removeClass("active")}}})};exports(),module.resolveWith(exports)})};FD31.module("textboxlist",moduleFactory)}();