...
class NotesControllerNotes extends SocialAppsController
{
	...
	public function form()
	{
		...
		// Load language
		Foundry::language()->loadApp( SOCIAL_APPS_GROUP_USER , 'notes' );
	...
	public function store()
	{
		...
		$comments			= Foundry::comments( $note->id , 'notes' , SOCIAL_APPS_GROUP_USER , array( 'url' => FRoute::apps( array( 'layout' => 'canvas', 'userid' => $my->getAlias() , 'cid' => $note->id ) ) ) );
		$note->comments 	= $comments->getCount();

		// Get the likes count
		$likes 			= Foundry::likes( $note->id , 'notes' );
