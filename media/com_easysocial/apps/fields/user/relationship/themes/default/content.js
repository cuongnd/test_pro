...
?>
EasySocial.module('field.relationship', function($) {
	var module = this;
	...
	.done(function($) {
		...
		EasySocial.Controller('Field.Relationship.Form', {
			...
			return {
				...
				// '{parent} relationshipRejected': function() {
				// 	var input = self.input().controller('Textboxlist');

				// 	var items = input.getAddedItems();

				// 	if(items.length > 0) {
				// 		input.removeItem(items[0].id);
				// 	}
				...
				// 	self.type().val(self.type().find('option:first').val());

				// 	self.type().trigger('change');
				// }
			}
		});
		...
		EasySocial.Controller('Field.Relationship.Display', {
			...
			return {
				...
				'{approveButton} click': function() {
					EasySocial.dialog({
						width: 500,
						content: '<?php echo JText::_( 'PLG_FIELDS_RELATIONSHIP_APPROVE_CONFIRM' ); ?>',
						selectors: {
						...
						buttons: '<button data-cancel-button type="button" class="btn btn-es"><?php echo JText::_( 'COM_EASYSOCIAL_CANCEL_BUTTON' ); ?></button><button data-approve-button type="button" class="btn btn-es-primary"><?php echo JText::_( 'PLG_FIELDS_RELATIONSHIP_ACTION_APPROVE' ); ?></button>'
					})
				},
...
EasySocial.module('field.relationship/<?php echo $field->id; ?>', function($) {
	this.resolve(<?php echo Foundry::json()->encode( $types ); ?>);
});
