 ...
 * Requires $jMaQma 1.3.2, jquery 1.2.6 or jquery 1.2.x plus the jquery dimensions plugin
 *
...
$jMaQma(document).ready(function () {
    $jMaQma('#myList').listnav({
        includeAll:false,
        includeOther:true,
        flagDisabled:true,
        noMatchText:'...',
        showCounts:true,
        cookieName:'helpdesk-glossary',
        onClick:function (letter) {
        },
        prefixes:['the', 'a']
    });
    if ($jMaQma("#term").length){
        $jMaQma("#term").focus();
    }
});
...
function submitbutton(pressbutton) {
	...
	if (pressbutton == 'insert') {
		if (form.term.value == "") {
			$jMaQma("#alertMessage .modal-body p").html('<img src="'+SITEURL+'components/com_maqmahelpdesk/images/alert.png" align="absmiddle" /> '+MQM_GLOSSARY_TERM);
...
function Cancel() {
	if (confirm(MQM_GLOSSARY_CANCEL)) {
