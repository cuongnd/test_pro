...
return form.isValid;},validateField:function(e){var elem=e.target||e;if(elem.form===undefined){return null;}
...
elem.validityState={valueMissing:isMissing,patterMismatch:isPatternMismatched,valid:(!isMissing&&!isPatternMismatched&&!isDisabled)};if(elem.validityState.valueMissing){$elem.addClass(self.options.requiredClass);}
else{$elem.removeClass(self.options.requiredClass);}
if(elem.validityState.patterMismatch){$elem.addClass(self.options.patternClass);}
else{$elem.removeClass(self.options.patternClass);}
if(!elem.validityState.valid){$elem.addClass(self.options.invalidClass);var $labelref=self.findLabel($elem);$labelref.addClass(self.options.invalidClass);$labelref.attr('aria-invalid','true');}
