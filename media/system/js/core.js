...
Joomla.replaceTokens = function (c) {
    var b = document.getElementsByTagName("input"), a;
...
Joomla.checkAll = function (b, f) {
    if (!f) {
        f = "cb"
    }
    if (b.form) {
        var g = 0, a, d;
        for (a = 0, n = b.form.elements.length; a < n; a++) {
            d = b.form.elements[a];
            if (d.type == b.type) {
                if ((f && d.id.indexOf(f) == 0) || !f) {
                    d.checked = b.checked;
                    g += (d.checked == true ? 1 : 0)
                }
            }
        }
        if (b.form.boxchecked) {
            b.form.boxchecked.value = g
        }
        return true
    }
    return false
};
Joomla.renderMessages = function (d) {
    var e = jQuery.noConflict(), g, b, a, f, c;
    Joomla.removeMessages();
    g = e("#system-message-container");
    e.each(d, function (h, j) {
        b = e("<div/>", {id: "system-message", "class": "alert alert-" + h});
        g.append(b);
        a = e("<h4/>", {"class": "alert-heading", text: Joomla.JText._(h)});
        b.append(a);
        f = e("<div/>");
...
Joomla.isChecked = function (a, d) {
    if (typeof(d) === "undefined") {
        d = document.getElementById("adminForm")
    }
    if (a == true) {
        d.boxchecked.value++
    } else {
...
Joomla.popupWindow = function (e, k, j, c, f) {
    var g = (screen.width - j) / 2, b, a, d;
    b = (screen.height - c) / 2;
    a = "height=" + c + ",width=" + j + ",top=" + b + ",left=" + g + ",scrollbars=" + f + ",resizable";
    d = window.open(e, k, a);
    d.window.focus()
};
Joomla.tableOrdering = function (a, c, b, d) {
    if (typeof(d) === "undefined") {
        d = document.getElementById("adminForm")
    }
...
function writeDynaList(e, g, d, h, a) {
    ...
    for (x in g) {
        if (g[x][0] == d) {
            f = "";
            if ((h == d && a == g[x][1]) || (b == 0 && h != d)) {
                f = 'selected="selected"'
            }
            c += '\n     <option value="' + g[x][1] + '" ' + f + ">" + g[x][2] + "</option>"
        }
        b++
    }
    c += "\n </select>";
...
function listItemTask(g, b) {
    var d = document.adminForm, c, e, a = d[g];
    if (a) {
        for (c = 0; true; c++) {
            e = d["cb" + c];
            if (!e) {
