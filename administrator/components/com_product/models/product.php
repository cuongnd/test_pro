...
class productModelproduct extends JModelAdmin
{
	...
	/**
	 * The type alias for this product type (for example, 'com_product.product').
	 *
	 * @var      string
	 * @since    3.2
	 */
	public $typeAlias = 'com_product.product';
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			// Check if the product was featured and update the #__product_frontpage table
			if ($featured == 1)
			{
				$db = $this->getDbo();
	...
	public  function batchCopyAllproductOfWebsiteToOtherWebsite($website_id)
	{
        $db=JFactory::getDbo();
        $query=$db->getQuery(true);
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_product.product.' . (int) $record->id);
		}
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_product.product.' . (int) $record->id);
		}
	...
	protected function prepareTable($table)
	{
		...
		// Reorder the products within the category so the new product is first
		if (empty($table->id))
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_product.product');
			}
		}
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_product.product.' . (int) $id))
	...
	protected function loadFormData()
	{
		...
		$app = JFactory::getApplication();
		$data = $app->getUserState('com_product.edit.product.data', array());
		...
		$this->preprocessData('com_product.product', $data);
