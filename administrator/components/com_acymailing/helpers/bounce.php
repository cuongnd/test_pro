...
class bounceHelper{
	...
	function _handleRule(&$oneRule){
		...
		$message = JText::_('BOUNCE_RULES').' ['.$oneRule->ruleid.'] '.$oneRule->name.' : ';
	...
	function _actionuser(&$oneRule){
		...
		if(isset($oneRule->action_user['block'])){
			$message .= ' | user '.$this->_message->subemail.' blocked';
	...
	function _actionmessage(&$oneRule){
		...
		if(isset($oneRule->action_message['save']) && !empty($this->_message->subid) && empty($oneRule->action_user['delete'])){
			$data = array();
			$data[] = 'SUBJECT::'.htmlentities($this->_message->subject, ENT_COMPAT | ENT_HTML401, 'UTF-8');
			...
			if(!empty($this->_message->html)) $data[] = 'HTML_VERSION::'.htmlentities($this->_message->html, ENT_COMPAT | ENT_HTML401, 'UTF-8');
			if(!empty($this->_message->text)) $data[] = 'TEXT_VERSION::'.nl2br(htmlentities($this->_message->text, ENT_COMPAT | ENT_HTML401, 'UTF-8'));
		...
		if(!empty($oneRule->action_message['forwardto'])){
			...
			$this->mailer->AddAddress($oneRule->action_message['forwardto']);
			...
			$this->mailer->setFrom(trim($this->_message->header->from_email,'<> '),$this->_message->header->from_name);
			if($this->mailer->send()){
