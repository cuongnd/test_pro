...
class listHelper{
	...
	function subscribe($subid,$listids){
		...
		if($this->sendConf AND !$app->isAdmin()){
			...
			$db = JFactory::getDBO();
			$db->setQuery('SELECT DISTINCT `welmailid` FROM '.acymailing_table('list').' WHERE `listid` IN ('.implode(',',$listids).')  AND `published` = 1 AND `welmailid` != 0');
			...
			if(!empty($messages)){
				...
				foreach($messages as $mailid){
					if($mailid == '-1'){
						$db->setQuery('SELECT l.mailid FROM #__acymailing_listmail as l JOIN #__acymailing_mail as m ON l.mailid = m.mailid WHERE l.`listid` IN ('.implode(',',$listids).') AND m.published = 1 AND m.type = \'news\' ORDER BY m.senddate DESC LIMIT 1');
