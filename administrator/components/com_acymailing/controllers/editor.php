...
class EditorController extends acymailingController{
			...
			$js="
				...
				function validateImage(){
					...
					if(urlInput!='http://' && selectedImageName.trim()==urlImageName.trim()){
							var tag = '<img ' + imgSize + ' src=\"' + urlInput + '\" alt=\"' + selectedImageAlt + '\" ".$sizeAndAlignTags." />';
					}else{
							var tag = '<img ' + imgSize + ' src=\"' + selectedImageUrl + '\" alt=\"' + selectedImageAlt + '\" ".$sizeAndAlignTags." />';
					}
	...
	private function _displayHTML(){
		...
		$imageZone = JRequest::getVar( 'image_zone', array(), '', 'array' );
		if($imageZone){
			echo '<script type="text/javascript">checkSelected(true);</script>';
		}else{
			echo '<script type="text/javascript">checkSelected();</script>';
		}
		if(isset($uploadMessage) && $uploadMessage=='success'){
			$imageSize = getimagesize(ACYMAILING_LIVE.$defaultFolder. $this->imageName);
			echo  '<script type="text/javascript"> displayImageFromUrl(\''.ACYMAILING_LIVE.$defaultFolder. $this->imageName.'\',\'success\', \''.$this->imageName.'\','.$imageSize[0].','.$imageSize[1].');</script>';
		}
	}
	...
	private function _generateSpecificFolders($folders, $default=false){

		$my = JFactory::getUser();
		$newFolders = array();

		if($default) $folders[0] = $default;
