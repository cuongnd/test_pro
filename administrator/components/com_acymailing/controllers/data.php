...
class DataController extends acymailingController{
	...
	function doexport(){
		...
		for($i=0,$a=count($allData);$i<$a;$i++){
			...
			if(!empty($allData[$i]['confirmed_date'])) $allData[$i]['confirmed_date'] = acymailing_getDate($allData[$i]['confirmed_date'],'%Y-%m-%d %H:%M:%S');
			if(!empty($allData[$i]['lastopen_date'])) $allData[$i]['lastopen_date'] = acymailing_getDate($allData[$i]['lastopen_date'],'%Y-%m-%d %H:%M:%S');
			if(!empty($allData[$i]['lastclick_date'])) $allData[$i]['lastclick_date'] = acymailing_getDate($allData[$i]['lastclick_date'],'%Y-%m-%d %H:%M:%S');
