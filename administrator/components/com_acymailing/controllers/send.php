...
class SendController extends acymailingController{
	...
	function scheduleone(){
		...
		if($status) acymailing_display(JText::sprintf('AUTOSEND_DATE','<b><i>'.$myNewsletter->subject.'</i></b>',acymailing_getDate($realSendDate)),'success');
		else{ acymailing_display(array(JText::_('ERROR_SAVING'),$db->getErrorMsg()),'error'); return $this->addqueue();}
	}
	...
	function spamtest(){
		...
		$receiver->subid = 0;
		$receiver->email = 'acymailing-'.base64_encode(ACYMAILING_LIVE).'-'.time().'@mail-tester.com';
		...
		$app = JFactory::getApplication();
		$app->redirect('http://www.mail-tester.com/check.php?id='.urlencode($receiver->email));
