...
class CpanelViewCpanel extends acymailingView
{
	function display($tpl = null)
	{
		...
		if(acymailing_level(1)){
			if($config->get('website') != ACYMAILING_LIVE){
		...
		if(acymailing_level(1)){
			...
			$cronreportval = array();
			$cronreportval[] = JHTML::_('select.option', '0',JText::_('JOOMEXT_NO'));
			$cronreportval[] = JHTML::_('select.option', '1',JText::_('EACH_TIME'));
			$cronreportval[] = JHTML::_('select.option', '2',JText::_('ONLY_ACTION'));
			$cronreportval[] = JHTML::_('select.option', '3',JText::_('ONLY_SOMETHING_WRONG'));
			...
			$cronsave = array();
			$cronsave[] = JHTML::_('select.option', '0',JText::_('JOOMEXT_NO'));
			$cronsave[] = JHTML::_('select.option', '1',JText::_('SIMPLIFIED_REPORT'));
			$cronsave[] = JHTML::_('select.option', '2',JText::_('DETAILED_REPORT'));
			...
			$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=cpanel&amp;task=cleanreport';
			$elements->deleteReport = '<a class="modal" href="'.$link.'" rel="{handler: \'iframe\', size: {x: 400, y: 100}}"><button class="btn" onclick="return false">'.JText::_('REPORT_DELETE').'</button></a>';

			$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=cpanel&amp;task=seereport';
			$elements->seeReport = '<a class="modal" href="'.$link.'" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><button class="btn" onclick="return false">'.JText::_('REPORT_SEE').'</button></a>';

			$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=email&amp;task=edit&amp;mailid=report';
			$elements->editReportEmail = '<a class="modal" href="'.$link.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><button class="btn" onclick="return false">'.JText::_('REPORT_EDIT').'</button></a>';
			...
			$urlCron = 'http://www.acyba.com/index.php?option=com_updateme&ctrl=launcher&task=edit&cronurl='.urlencode($elements->cron_url);
			$elements->cron_edit = '<a class="modal" href="'.$urlCron.'" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><button class="btn" onclick="return false">'.JText::_('CREATE_CRON').'</button></a>';
		}
