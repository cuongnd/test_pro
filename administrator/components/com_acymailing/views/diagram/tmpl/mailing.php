...
<tr><td width="45%">
	<ul style="margin:0px;">
		...
		if(acymailing_isAllowed($this->config->get('acl_subscriber_view','all'))){
			...
			if(!$app->isAdmin()) $link = 'newsletter&task=detailstats&listid='.JRequest::getInt('listid').'&mailid='.$this->mailing->mailid;
		...
		<?php
			...
			if(acymailing_isAllowed($this->config->get('acl_subscriber_view','all'))){
				...
				if(!$app->isAdmin()) $link = 'newsletter&task=statsclick&listid='.JRequest::getInt('listid').'&mailid='.$this->mailing->mailid;
