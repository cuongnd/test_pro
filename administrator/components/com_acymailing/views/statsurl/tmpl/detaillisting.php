...
<form action="index.php?option=<?php echo ACYMAILING_COMPONENT ?>&amp;ctrl=<?php echo JRequest::getCmd('ctrl'); ?>" method="post" name="adminForm" id="adminForm" >
	<fieldset>
		...
		<div class="toolbar" id="toolbar" style="float: right;">
			<table><tr>
			<?php $app = JFactory::getApplication(); if($app->isAdmin()){ ?>
				<?php if(acymailing_isAllowed($this->config->get('acl_subscriber_export','all'))){ ?>
				<td><a onclick="javascript:submitbutton('export')" href="#" ><span class="icon-32-acyexport" title="<?php echo JText::_('ACY_EXPORT',true); ?>"></span><?php echo JText::_('ACY_EXPORT'); ?></a></td>
				<?php } ?>
			<?php }else{ ?>
				<td><a href="<?php echo acymailing_completeLink('newsletter&task=stats&listid='.JRequest::getInt('listid').'&mailid='.JRequest::getInt('mailid'),true) ?>" ><span class="icon-32-cancel" title="<?php echo JText::_('ACY_CANCEL',true); ?>"></span><?php echo JText::_('ACY_CANCEL'); ?></a></td>
			<?php } ?>
