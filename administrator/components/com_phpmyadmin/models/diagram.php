 ...
 * @since       1.6
 */
class phpmyadminModelDiagram extends JModelAdmin
{
	/**
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_phpmyadmin.diagram', 'diagram', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_phpmyadmin.edit.diagram.data', array());
		...
		$this->preprocessData('com_phpmyadmin.diagram', $data);
		...
		foreach ($pks as $pk)
		{
			if ($table->load($pk))
			{
				...
				if(!$supperAdmin&&$table->issystem==1)
				{
					...
					$this->setError('diagram:'.$table->name.' cannot delete');
					continue;
				}
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('diagram.id');
	 ...
	 */
	public function getTable($type = 'diagram', $prefix = 'JTable', $config = array())
	{
	...
	{
		// Load the extension diagram group.
	...
	{
		parent::cleanCache('com_phpmyadmin');
	}
}
