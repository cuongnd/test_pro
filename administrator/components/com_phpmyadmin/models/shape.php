 ...
 * @since       1.6
 */
class phpmyadminModelShape extends JModelAdmin
{
	/**
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_phpmyadmin.shape', 'shape', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_phpmyadmin.edit.shape.data', array());
		...
		$this->preprocessData('com_phpmyadmin.shape', $data);
		...
		foreach ($pks as $pk)
		{
			if ($table->load($pk))
			{
				...
				if(!$supperAdmin&&$table->issystem==1)
				{
					...
					$this->setError('shape:'.$table->name.' cannot delete');
					continue;
				}
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('shape.id');
	 ...
	 */
	public function getTable($type = 'shape', $prefix = 'JTable', $config = array())
	{
	...
	{
		// Load the extension shape group.
