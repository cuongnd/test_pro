...
class WebsiteModelWebsite extends JModelAdmin
{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		if ($this->getState('website.id'))
		{
			$id = $this->getState('website.id');
			// Existing record. Can only edit in selected categories.
			$form->setFieldAttribute('catid', 'action', 'core.edit');
			// Existing record. Can only edit own websites in selected categories.
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_website.website.' . (int) $id))
		...
		)
		{
			...
			// The controller has already verified this is an website you can edit.
		...
		// Prevent messing with website language and category when editing existing website with associations
		$app = JFactory::getApplication();
		$assoc = JLanguageAssociations::isEnabled();

		// Check if website is associated
		if ($id && $app->isSite() && $assoc)
		{
			$associations = JLanguageAssociations::getAssociations('com_website', '#__website', 'com_website.item', $id);
