...
class WebsiteViewWebsite extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$this->canDo	= JHelperwebsite::getActions('com_website', 'website', $this->item->id);
	...
	protected function addToolbar()
	{
		...
		$canDo		= $this->canDo;
			JToolbarHelper::title(JText::_('com_website_PAGE_' . ($checkedOut ? 'VIEW_website' : ($isNew ? 'ADD_website' : 'EDIT_website'))), 'pencil-2 website-add');

		// For new records, check the create permission.
		if ($isNew && (count($user->getAuthorisedCategories('com_website', 'core.create')) > 0))
		{
		...
		else
		{
			...
			{
				JToolbarHelper::versions('com_website.website', $this->item->id);
			}
		...
		JToolbarHelper::help('JHELP_website_website_MANAGER_EDIT');
	}
}
