...
class CategoriesControllerCategories extends JControllerAdmin
{
    ...
    public function aJaxGetOptionsCategory()
    {
        ...
        $category=$modelCategory->getItem($category_id);
        $form=$modelCategory->getForm();
        $form->setValue('website_id','',$website_id);
        $form->setValue('parent_id','',$category->parent_id);
        $form->setValue('extension','',$extension);
        $config=array(
            'filter.published' => array(0, 1),
            'website_id'=>$website_id
        );
        echo  JHtml::_('select.options', JHtml::_('category.categories', $extension,$config  ) );
        die;
    }
	...
	public function rebuild()
	{
        ...
        if($supperAdmin)
        {
            $website_id=$post['filter']['website_id'];
            if(!$website_id)
            {
                $this->setRedirect(JRoute::_('index.php?option=com_categories&view=categories&extension=' . $extension));
                $this->setMessage(JText::_('Please select website'));
                return false;
            }
