 ...
 * @since       1.6
 */
class MenusControllerMenus extends JControllerAdmin
{
	/**
	 * Display the view
	 *
    ...
    function quick_assign_website()
    {
        ...
        if($copy)
        {
            try {
                if (empty($pks))
                {
                    throw new Exception(JText::_('COM_MENU_ERROR_NO_MENU_SELECTED'));
                }
                $model = $this->getModel();
                $model->duplicateAndAssign($pks,$website_id);
                $this->setMessage(JText::plural('COM_MENUS_N_MENU_DUPLICATED', count($pks)));
            } catch (Exception $e)
            {
                JError::raiseWarning(500, $e->getMessage());
            }
            $this->setRedirect(JRoute::_('index.php?option=com_menus&view=menus'));
        }
        else {
            try {
                if (empty($pks)) {
                    throw new Exception(JText::_('COM_MENU_ERROR_NO_MODULE_SELECTED'));
                }
                $model = $this->getModel();
                $model->quick_assign_website($pks, $website_id);
                $this->setMessage(JText::plural('COM_MENU_N_QUICK_ASSIGN', count($pks)));
            } catch (Exception $e) {
                JError::raiseWarning(500, $e->getMessage());
            }
	...
	public function rebuild()
	{
        ...
        try {
            if (empty($pks))
            {
                throw new Exception(JText::_('COM_PLUGINS_ERROR_NO_MENU_TYPE_SELECTED'));
            }
            $model = $this->getModel('Item');
            if ($model->rebuild($pks))
            {
                // Reorder succeeded.
