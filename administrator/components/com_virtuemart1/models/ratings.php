...
// 		if(!Permissions::getInstance()->check('admin')){
		...
		if ( !empty($data['virtuemart_product_id']) && !empty($userId)){
			...
			//$data['virtuemart_rating_vote_id'] =0;// empty($vote->virtuemart_rating_vote_id)? 0: $vote->virtuemart_rating_vote_id;
	...
	* @return int
	*/
	public function countReviewsForProduct($pid,$wherein=array()) {
        static $listTotal=null;
		$db = JFactory::getDBO();
        ...
        if(!empty($wherein))
        {
            ...
            if(is_null($listTotal))
            {
                $str_wherein=implode(',',$wherein);
                $q = "SELECT COUNT(virtuemart_rating_review_id) AS total,virtuemart_product_id
                FROM #__virtuemart_rating_reviews
                WHERE virtuemart_product_id IN (".$str_wherein.") GROUP BY virtuemart_product_id";
                $listTotal=$this->_db->loadObjectList('virtuemart_product_id');

            }
            $result=$listTotal[$pid]->total;
