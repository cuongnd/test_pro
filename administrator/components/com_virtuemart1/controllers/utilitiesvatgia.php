 ...
 */
class VirtuemartControllerUtilitiesvatgia extends VmController {
    ...
    public function setcategoryproduct()
    {
        ...
        $db=JFactory::getDbo();
        $query=$db->getQuery(true);
        $query->select('products_en_gb.virtuemart_product_id, products_en_gb.params')
            ->from('#__virtuemart_products_en_gb AS products_en_gb')
            ->leftJoin('ueb3c_virtuemart_product_categories AS product_categories USING(virtuemart_product_id)')
            ->where('product_categories.virtuemart_category_id is null')
            ->where('products_en_gb.params!=""')
            ;
        $product=$db->setQuery($query)->loadObject();
        $query=$query->clear('select')
            ->select('count(*)')
            ;
        $else_total=$db->setQuery($query)->loadResult();
        if(!$product)
        {
            $response->e=1;
            $response->r='there are no product select';
            echo json_encode($response);
            die;
        }
        $params = new JRegistry;
        $params->loadString($product->params);
        $cat_id=$params->get('vatgia.cat_id',0);
        require_once JPATH_ROOT.'/libraries/simplehtmldom_1_5/simple_html_dom.php';

        $html=$params->get('vatgia.html','');
        $html = str_get_html($html);
        $href=$html->find('div.name a',0)->href;
        $href='http://www.vatgia.com'.$href;
        
        $data=JUtility::getCurl($href);

        $uncategory_id=14426;
        if($data=='')
        {
            $response->e=0;
            $response->r='data is null';
            $response->r.='<br/>';
            $response->r.='Link:'.$href;
            $response->r.='<br/>';
            $query=$db->getQuery(true);
            $query->insert('#__virtuemart_product_categories')
                ->set('virtuemart_product_id='.(int)$product->virtuemart_product_id)
                ->set('virtuemart_category_id='.$uncategory_id)
            ;
            $db->setQuery($query)->execute();
            echo json_encode($response);
            die;
        }
        $product_content=str_get_html($data);
        $product_content=$product_content!=''?$product_content->find('div#container_body',0):'';
        $header_navigate=$product_content!=''?$product_content->find('div#header_navigate',0)->plaintext:'';

        $header_navigate=explode('&gt;',$header_navigate);

        $category1= $header_navigate[count($header_navigate)-2];
        $category1=strtolower($category1);
        $category=$product_content!=''?$product_content->find('div#detail_product_exclusive_cart',0):'';
        $text_overflow=$category!=''?$category->find('div.text_overflow a',1)->plaintext:'';
        $vatgia_category_link=$category!=''?$category->find('div.text_overflow a',1)->href:'';

        if($vatgia_category_link!='')
        {
            $vatgia_category_link='http://www.vatgia.com'.$vatgia_category_link;
            $header_navigate=$this->get_header_navigate_vatgia($vatgia_category_link);
        }

        $text_overflow=$text_overflow==''?$category1:$text_overflow;

        if($text_overflow=='')
        {
            $response->e=1;
            $response->r='cannot found category vat gia';
            $response->r.='<br/>';
            $response->r.='Link:'.$href;
            $response->r.='<br/>';
            $response->r.=$product_content;
            ob_start();
            ?>

            <?php
            $response->r.=ob_get_clean();
            echo json_encode($response);
            die;
        }
        $count_up=2;
        for($i=0;$i<count($header_navigate);$i++) {
            $query = $db->getQuery(true);
            $query->select('virtuemart_category_id')
                ->from('#__virtuemart_categories_en_gb')
                ->where('LOWER(category_name) = ' . $query->q("$text_overflow"));
            $virtuemart_category_ids = $db->setQuery($query)->loadColumn();
            if(count($virtuemart_category_ids)>0){
                break;
            }else{
                $text_overflow=$header_navigate[$count_up];
                $count_up++;
            }
        }
        ...
        if(count($virtuemart_category_ids)==0)
        {
            $virtuemart_category_ids[]=$uncategory_id;
        }

        $query->clear()
            ->select('virtuemart_product_id')
            ->from('#__virtuemart_products_en_gb AS products_en_gb')
            ->where('products_en_gb.params LIKE '.$query->q("%\"cat_id\":$cat_id,%"));
        $virtuemart_product_ids=$db->setQuery($query)->loadColumn();
        foreach($virtuemart_category_ids as $virtuemart_category_id)
        {
            foreach($virtuemart_product_ids AS $virtuemart_product_id) {
                $query = $db->getQuery(true);
                $query->insert('#__virtuemart_product_categories')
                    ->set('virtuemart_product_id='.(int)$virtuemart_product_id)
                    ->set('virtuemart_category_id='.(int)$virtuemart_category_id)
                    ;
                $db->setQuery($query)->execute();
                }
        }
        $response->r='done';
        $response->r.='<br/>';
        $response->r.='Total else:'.$else_total;
        $response->r.='<br/>';
        $response->r.=$text_overflow;
        $response->r.='<br/>';
        $detail_product_technical=$product_content!=''?$product_content->find('div#detail_product_technical',0):'';
        $detail_product_technical=$detail_product_technical==''?$product_content->find('div.detail_product_normal_information',0):$detail_product_technical;
        $response->r.=$detail_product_technical;
        echo json_encode($response);
        die;
...
/*        $model_product=VmModel::getModel('product');
    ...
    private function get_header_navigate_vatgia($link)
    {
