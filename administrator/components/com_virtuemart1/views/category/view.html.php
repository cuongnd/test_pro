...
class VirtuemartViewCategory extends VmView {
	...
	function display($tpl = null) {
		...
		if ($layoutName == 'edit') {
            ...
            $category->virtuemart_category_id=$virtuemart_category_id;
            ...
            $xrefTable = $model->getTable ('category_websites');

            $category->virtuemart_website_id = $xrefTable->load ((int)$category->virtuemart_category_id);



            $websiteList=JUtility::getListWebsite();
            $this->websiteList='';
            foreach($websiteList as $website)
            {
                $this->websiteList.='<option '.(in_array($website->id,$category->virtuemart_website_id)?'selected':'').' value="'.$website->id.'">'.$website->title.'</option>';
            }
			...
			//echo $model->getDBO()->getQuery()->dump();
