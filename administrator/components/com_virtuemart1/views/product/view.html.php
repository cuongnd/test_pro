    ...
    function display($tpl = null)
    {
        ...
        switch ($task) {
            ...
            case 'edit':
                ...
                $listMedia=productHelper::getFilesUploadByProductId($virtuemart_product_id);

                $this->assignRef('listMedia', $listMedia);
                //end get list file upload
...
//TODO test if path is ok addpath is now in the constructor
            ...
            default:
                ...
                //echo "<pre>";
                //print_r(JFactory::getDbo()->replacePrefix($model->getDBO()->getQuery()));
                //exit();
                //echo $model->getDBO()->getQuery();
                ...
                foreach ($productlist as $virtuemart_product_id => $product) {
                    ...
                    $product->reviews = $productreviews->countReviewsForProduct($product->virtuemart_product_id, $model->listProductId);
                    ...
                    $product->categoriesList = shopfunctions::renderGuiList('virtuemart_category_id', '#__virtuemart_product_categories', 'virtuemart_product_id', $product->virtuemart_product_id, 'category_name', '#__virtuemart_categories', 'virtuemart_category_id', 'category', 4, 1, $model->listProductId);
    ...
    function displayLinkToChildList($product_id, $product_name)
    {
        ...
        $listProductId = $model_product->listProductId;
        $strProductId = implode(',', $listProductId);
        static $_countProductsByProductParent = null;
        if (is_null($_countProductsByProductParent) && !empty($model_product->listProductId)) {
            $db = JFactory::getDbo();
            $query = $db->getQuery(true);
            $query->from('#__virtuemart_products AS p1');
            $query->select('p1.product_parent_id AS product_parent_id,COUNT(p1.virtuemart_product_id ) AS total');
            $query->where('p1.product_parent_id IN(' . $strProductId . ')');
            $query->group('p1.product_parent_id');
            $db->setQuery($query);
            $_countProductsByProductParent = $db->loadObjectList('product_parent_id');
        }
        if ($_countProductsByProductParent) {
            $result = JText::sprintf('COM_VIRTUEMART_X_CHILD_PRODUCT', $_countProductsByProductParent[$product_id]->total);
            if ($this->frontEdit) $front = "&tmpl=component";
            else $front = "";
            echo JHTML::_('link', JRoute::_('index.php?view=product&product_parent_id=' . $product_id . '&option=com_virtuemart' . $front), '<div class="small">' . $_countProductsByProductParent[$product_id]->total . '</div>', array('class' => 'hasTooltip', 'title' => JText::sprintf('COM_VIRTUEMART_PRODUCT_LIST_X_CHILDREN', $product_name)));
        }
    }
