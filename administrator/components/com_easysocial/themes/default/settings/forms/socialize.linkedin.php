...
echo $settings->renderPage(
	$settings->renderColumn(
		$settings->renderSection(
			...
			$settings->renderSetting( 'LinkedIn Authentication', '', 'custom', Foundry::oauth( 'LinkedIn' )->getLoginButton( 3 , 'config' , FRoute::_( 'index.php?option=com_easysocial&view=settings&layout=closeOauthDialog&tmpl=component' ) , array( 'publish_stream' ) ) )
		)
	)
);
