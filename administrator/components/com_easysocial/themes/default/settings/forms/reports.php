...
echo $settings->renderPage(
	$settings->renderColumn(
		$settings->renderSection(
			...
			//,$settings->renderSetting( 'Maximum reports per IP address', 'reports.maxip', 'input', array( 'help' => true, 'unit' => true , 'class' => 'center input-mini') )
		)
	),
	$settings->renderColumn(
		$settings->renderSection(
			...
			$settings->renderSetting( 'Custom Email addresses', 'reports.notifications.emails', 'input', array( 'class' => 'input-full' , 'help' => true, 'placeholder' => true ) )
		)
	)
);
