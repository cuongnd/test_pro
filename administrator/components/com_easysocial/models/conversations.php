...
class EasySocialModelConversations extends EasySocialModel
{
	...
	public function leave( $conversationId , $userId )
	{
		...
		$participant	= Foundry::table( 'ConversationParticipant' );
	 ...
	 */
	public function getParticipants( $conversationId , $excludeUsers = array() )
	{
		// Ensure excluded users is an array.
		$excludeUsers	= Foundry::makeArray( $excludeUsers );
	...
	public function getLastMessage( $conversationId , $viewerId )
	{
		...
		$sql->column( 'b.*' );
		$sql->join( '#__social_conversations_message' , 'b' );
	...
	public function getMessages( $conversationId , $userId , $options = array() )
	{
		...
		$query[]	= 'FROM ' . $db->nameQuote( '#__social_conversations_message' ) . ' AS a';
		$query[]	= 'LEFT JOIN ' . $db->nameQuote( '#__social_conversations_message_maps' ) . ' AS b';
	...
	public function getConversations( $userId , $options = array() )
	{
		...
		$sql->select( '#__social_conversations' , 'a' );
		$sql->join( '#__social_conversations_message' , 'b' , 'INNER' );
		$sql->on( 'a.id' , 'b.conversation_id' );
		$sql->join( '#__social_conversations_message_maps' , 'c' , 'INNER' );
		$sql->on( 'c.message_id' , 'b.id' );
		...
		{
			$sql->where( 'c.state' , SOCIAL_CONVERSATION_STATE_ARCHIVED );
		}
		else
		{
			$sql->where( 'c.state' , SOCIAL_CONVERSATION_STATE_PUBLISHED );
		}
		...
		if( isset( $options['filter'] ) )
		{
			...
			{
				case 'unread':
					$sql->where( 'c.isread' , SOCIAL_CONVERSATION_UNREAD );
					break;
				case 'read':
					$sql->where( 'c.isread' , SOCIAL_CONVERSATION_READ );
					break;
			}
		}
		...
		{
			$sql->column( 'COUNT(DISTINCT( a.`id` ) )' );
		...
		$sql->column( 'a.*' );
		$sql->column( 'b.message' );
		$sql->column( 'c.isread' );

		$sql->group( 'b.conversation_id' );
	...
	public function addReply( $conversationId , $msg , $creatorId )
	{
		...
		if( $users )
		{
			...
			{
				$map 	= Foundry::table( 'ConversationMessageMap' );
				$map->user_id 			= $user->id;
		...
		$conversation->set( 'lastreplied' , Foundry::date()->toMysQL() );
		$conversation->store();
	...
	public function getAttachments( $messageId )
	{
		...
		foreach( $data as $row )
		{
			...
			$attachment		= Foundry::table( 'ConversationAttachment' );
			$attachment->bind( $row );
