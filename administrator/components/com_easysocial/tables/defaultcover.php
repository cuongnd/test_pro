...
class SocialTableDefaultCover extends SocialTable
{
	...
	public function getSource( $size = SOCIAL_COVER_SMALL , $absolute = true )
	{
		...
		$avatarsPath 	= Foundry::cleanPath( $config->get( 'covers.storage.container' ) );

		// Get the defaults storage path.
		$defaultsPath	= Foundry::cleanPath( $config->get( 'covers.storage.default' ) );

		// Get the types storage path.
		$typesPath		= Foundry::cleanPath( $config->get( 'covers.storage.defaults.' . $this->type ) );
		...
		if( !$exists )
		{
			...
			Foundry::logError( __FILE__ , __LINE__ , 'DEFAULT_COVERS: Unable to load the cover from ' . $storagePath );
	...
	public function upload( $files )
	{
		...
		$coversPath = JPATH_ROOT . '/' . Foundry::cleanPath( $config->get( 'covers.storage.container' ) );

		// Test if the avatars path folder exists. If it doesn't we need to create it.
		if( !Foundry::makeFolder( $coversPath ) )
		{
			Foundry::logError( __FILE__ , __LINE__ , 'DEFAULT_COVERS: Unable to create the path ' . $coversPath );
		...
		$defaultsPath 	= $coversPath . '/' . Foundry::cleanPath( $config->get( 'covers.storage.default' ) );

		// Ensure that the defaults path exist
		if( !Foundry::makeFolder( $defaultsPath ) )
		{
			Foundry::logError( __FILE__ , __LINE__ , 'DEFAULT_COVERS: Unable to create the path ' . $defaultsPath );
		...
		{
			Foundry::logError( __FILE__ , __LINE__ , 'DEFAULT_COVERS: Unable to create the path ' . $storagePath );
		...
		if( !$image->isValid() )
		{
			Foundry::logError( __FILE__ , __LINE__ , 'DEFAULT_COVERS: Image uploaded ' . $file[ 'name' ] . ' is invalid' );
			$this->setError( JText::_( 'COM_EASYSOCIAL_PROFILES_DEFAULT_AVATARS_FILE_NOT_IMAGE' ) );
			return false;
		}

		// Process covers storage.
		$cover 	= Foundry::get( 'Cover' , $image );
		...
		if( $sizes === false )
		{
			Foundry::logError( __FILE__ , __LINE__ , 'DEFAULT_COVERS: Error creating covers at ' . $storagePath );
	...
	private function deleteImage( $folder , $size )
	{
		if( !empty( $this->$size ) )
		{
			...
			{
				return Foundry::get( 'Files' )->delete( $path );
			}
		}
	}
