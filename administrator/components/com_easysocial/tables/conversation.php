...
class SocialTableConversation extends SocialTable
{
	...
	{
		$url 	= FRoute::conversations( array( 'id' => $this->id ) , $xhtml );
	...
	public function getLastMessage( $userId )
	{
		...
		{
			$model 		= Foundry::model( 'Conversations' );
			$messages[ $this->id . $userId ]	= $model->getLastMessage( $this->id , $userId );
		}
	...
	{
		return Foundry::model( 'Conversations' )->archive( $this->id , $userId );
	}
