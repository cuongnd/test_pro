...
class SocialTableConversationParticipantHistory extends SocialTable
{
	...
	public function getCreator()
	{
		...
		{
			$nodes[ $this->created_by ]	= Foundry::get( 'People' , $this->created_by , SOCIAL_PEOPLE_NODE_ID );
		}
	...
	public function getTarget()
	{
		...
		if( !isset( $nodes[ $this->target ] ) )
		{
			...
			$target 	= Foundry::json()->decode( $this->target );
			$targets	= array();

			foreach( $target as $targetId )
			{
				$targets[]	= Foundry::get( 'People' , $targetId , SOCIAL_PEOPLE_NODE_ID );
			}
