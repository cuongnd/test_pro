...
class SocialTableFriend extends SocialTable
{
	...
	public function bind( $data , $ignore = array() )
	{
		...
		{
			$this->created	= Foundry::get( 'Date' )->toMySQL();
		}
	...
	public function sendNotification( $action = '' )
	{
		...
		if( $action == 'approve' )
		{
			...
			$target			= Foundry::user( $this->target_id );
			...
			$params 	= array(
									...
									'friendLink'	=> $target->getPermalink(),
									'friendDate'		=> Foundry::date()->toMySQL(),
			...
			// Email template
			$emailOptions 		= array(
											'title'		=> JText::sprintf( 'COM_EASYSOCIAL_EMAILS_SUBJECT_FRIENDS_REQUEST_APPROVED' , $target->getName() ),
											'template'	=> 'site/friends/accepted',
											'params'	=> $params
										);

			// Add the option to the notification.
			Foundry::notify( 'friends.approve' , $recipient , $emailOptions , $systemOptions );
		}
	...
	public function approve()
	{
		...
		$this->modified 	= Foundry::date()->toMySQL();
		...
		$user 			= Foundry::user( $this->actor_id );
		$target			= Foundry::user( $this->target_id );
		...
		// Add the users to their friend's list accordingly.
	 ...
	 * <?php
	 * $table	= Foundry::table( 'Friend' );
	 * $table->setTargetId( Foundry::user()->id );
	 * ?>
	 * </code>
	 *
	 * @since	1.0
	 ...
	 * <?php
	 * $table	= Foundry::table( 'Friend' );
	 * $table->setSourceId( Foundry::user()->id );
	 * ?>
	 * </code>
	 *
	 * @since	1.0
