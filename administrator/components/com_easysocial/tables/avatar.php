...
class SocialTableAvatar extends SocialTable
{
	...
	public function upload( $file )
	{
		...
		{
			Foundry::logError( __FILE__ , __LINE__ , 'AVATARS: Unable to create the path ' . $avatarsPath );
		...
		$storagePath 	= $avatarsPath . '/' . Foundry::cleanPath( $typePath );

		// Ensure storage path exists.
		if( !Foundry::makeFolder( $storagePath ) )
		{
			Foundry::logError( __FILE__ , __LINE__ , 'AVATARS: Unable to create the path ' . $storagePath );
		...
		if( !$image->isValid() )
		{
			Foundry::logError( __FILE__ , __LINE__ , 'AVATARS: Image uploaded ' . $file[ 'name' ] . ' is invalid' );
			$this->setError( JText::_( 'COM_EASYSOCIAL_PROFILES_DEFAULT_AVATARS_FILE_NOT_IMAGE' ) );
			return false;
		}

		// Process avatar storage.
		$avatar 	= Foundry::get( 'Avatar' , $image );
		...
		if( $sizes === false )
		{
			Foundry::logError( __FILE__ , __LINE__ , 'AVATARS: Error creating avatars at ' . $storagePath );
	...
	public function getPaths( $relative = false )
	{
		...
		$path 	= $path . '/' . Foundry::cleanPath( $config->get( 'avatars.storage.' . $this->type ) );
	...
	public function getPath( $size = SOCIAL_AVATAR_MEDIUM , $absolute = false )
	{
		...
		$location 	= Foundry::cleanPath( $config->get( 'avatars.storage.container' ) );
		$location 	= $location . '/' . Foundry::cleanPath( $config->get( 'avatars.storage.' . $this->type ) );
	...
	public function getSource( $size = SOCIAL_AVATAR_MEDIUM , $absolute = true )
	{
		...
		$typesLocation 		= Foundry::cleanPath( $config->get( 'avatars.storage.' . $this->type ) );
