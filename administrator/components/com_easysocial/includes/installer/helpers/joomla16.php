...
class SocialInstallerJoomla16
{
	...
	{
		$files			= Foundry::get( 'Folders' )->files( $path , '.xml' , self::RECURSIVE_SEARCH , self::RETRIEVE_FULL_PATH );
		$this->source	= $path;

		if (!count($files))
		{
			// Throw errors
			Foundry::get( 'Errors' )->set( 'installer.xml' , self::XML_NOT_FOUND );
			return false;
		}
		...
		foreach( $files as $file )
		{
			...
			{
				Foundry::get( 'Errors' )->set( 'installer' , self::XML_NOT_VALID );
				unset( $xml );
				continue;
			}
