...
class SocialLessc {
	...
	protected function compileProp($prop, $block, $out) {
		...
		case 'assign':
			...
			if ($name[0] == $this->vPrefix) {
				...
				// FOUNDRY HACK
				$line = $this->formatter->property($name,
							$this->compileValue($this->reduce($value)));

				// Certain CSS compressors don't translate paths
				// that begins with ./
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new SocialLessc_parser($this, __METHOD__);
	...
	public function cachedCompile( $in, $force = false) {
		...
		if ($root !== null)
		{
			...
			// @stackideas
			// Added a hack to split multiple block of css codes
			// Split up into multiple files here to avoid the 4096 css selectors issues on IE!
			$out[ 'blocks' ] 	= $this->blockify( $out[ 'compiled' ] );
	...
	/**
	 * Splits out the css file into multiple blocks to fix Internet Explorer mess
	 *
	 * @since	1.0
	 * @access	public
	 * @return	
	 */
	public function blockify( $css )
	{
		$results = array();
		...
		foreach( $matches as $match )
		{
			// Match the selector
			$pattern 	= '/.+?\{/s';
			preg_match( $pattern , $match , $selector );
	...
	protected function makeParser($name) {
		$parser = new SocialLessc_parser($this, $name);
	...
	protected function newFormatter() {
		$className = "Sociallessc_formatter_lessjs";
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
				return $this->formatterName;
			$className = "Sociallessc_formatter_$this->formatterName";
		}
...
class SocialLessc_parser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('SocialLessc', 'preg_quote'),
					array_keys(self::$precedence))).')';

			$commentSingle = SocialLessc::preg_quote(self::$commentSingle);
			$commentMultiLeft = SocialLessc::preg_quote(self::$commentMultiLeft);
			$commentMultiRight = SocialLessc::preg_quote(self::$commentMultiRight);
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("SocialLessc", "preg_quote"), $directives));
	...
	protected function expressionList(&$exps) {
		...
		$exps = SocialLessc::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName = null) {
		...
		$value = SocialLessc::compressList($values, ', ');
		return true;
	}
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("SocialLessc", "preg_quote"), $stop);
	...
	protected function literal($what, $eatWhitespace = null) {
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = SocialLessc::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.SocialLessc::preg_quote($what), $m, !$until)) return false;
...
class Sociallessc_formatter_classic {
	public $indentChar = "  ";
...
class Sociallessc_formatter_compressed extends Sociallessc_formatter_classic {
...
class Sociallessc_formatter_lessjs extends Sociallessc_formatter_classic {
	public $disableSingle = true;
