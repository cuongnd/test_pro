...
class SocialAlertRegistry
{
	...
	public function register( $rule, $email = 0, $system = 0, $options = array() )
	{
		...
		{
			Foundry::logError( __FILE__, __LINE__, 'SocialAlert: No type set yet.' );
			return false;
		}

		$table = Foundry::table( 'alert' );
		$loaded = $table->loadByRule( $this->element, $rule );
		...
		if( !$result )
		{
			Foundry::logError( __FILE__, __LINE__, 'SocialAlert: Unable to store rule.' );
			return false;
		}
	...
	public function registerUser( $user_id = null )
	{
		...
		if( empty( $this->element ) )
		{
			Foundry::logError( __FILE__, __LINE__, 'SocialAlert: No element set yet.' );
			return false;
		}
	...
	public function loadRule( $rulename )
	{
		if( empty( $this->rules[$rulename] ) )
		{
			...
			$rule	= $table->loadByRule( $this->element, $rulename );

			if( !$rule )
			{
				Foundry::logError( __FILE__, __LINE__, 'No such rule.' );
				return false;
			}
