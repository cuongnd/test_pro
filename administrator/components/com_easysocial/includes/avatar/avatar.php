...
class SocialAvatar
{
	...
	public function crop( $top = null , $left = null , $width = null , $height = null )
	{
		...
		$tmpImagePath 	= md5( Foundry::date()->toMySQL() ) . $image->getExtension();
	...
	public function cleanup( $avatarTable )
	{
		...
		$storage = Foundry::storage( $avatarTable->storage );
	 ...
	 * @return
	 */
	public function create( &$avatarTable = null )
	{
	...
	public function store( SocialTablePhoto &$photo, $options = array() )
	{
		...
		$avatarTable->modified 	= Foundry::date()->toMySQL();
	...
	public function getPath( $createFolders = true )
	{
		...
		$container	= Foundry::cleanPath( $config->get( 'avatars.storage.container' ) );
		...
		$containerType	= Foundry::cleanPath( $config->get( 'avatars.storage.' . $this->type ) );
	...
	public function getStoragePath( $uid , $type , $createFolders = true )
	{
		...
		// Append it with the container path
		$storage 	= $storage . '/' . Foundry::cleanPath( $config->get( 'avatars.storage.container' ) );
		...
		// Append it with the type
		$storage 	= $storage . '/' . Foundry::cleanPath( $config->get( 'avatars.storage.' . $type ) );
