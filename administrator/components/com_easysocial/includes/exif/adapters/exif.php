...
class SocialExifLibrary
{
	...
	public function getCreationDate()
	{
		...
		$dt 	= Foundry::date( $this->data[ 'DateTimeOriginal' ] )->toSql();
	...
	public function getLocation()
	{
		...
		$long 		= $this->data[ 'GPSLongitude' ];
		$longR		= $this->data[ 'GPSLongitudeRef' ];

		$lat 		= $this->data[ 'GPSLatitude' ];
		$latR 		= $this->data[ 'GPSLatitudeRef' ];

		$location->longitude 	= $this->toDecimal( $long[ 0 ] , $long[ 1 ] , $long[ 2 ] , $longR );
		$location->latitude 	= $this->toDecimal( $lat[ 0 ] , $lat[ 1 ] , $lat[ 2 ] , $latR );
