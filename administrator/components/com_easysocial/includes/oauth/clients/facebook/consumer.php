...
class SocialConsumerFacebook extends SocialFacebook implements ISocialOAuth
{
	...
	public function addPermission( $permission )
	{
		...
		$permissions	= Foundry::makeArray( $oauthTable->permissions );
		...
		$oauthTable->permissions	= Foundry::makeJSON( $permissions );
	...
	public function removePermissions( $scope )
	{
		...
		$permissions 	= Foundry::json()->decode( $oauthTable->permissions );
	 ...
	 * @return
	 */
	public function push( $message , $placeId = null , $photo = null )
	{
		$options 	= array(
								'message'	=> $message
							);
	...
	public function api()
	{
		try {
			...
			$result = call_user_func_array( 'parent::api', $args );
	...
	public function getUserMeta()
	{
		...
		// Get the first_name , middle_name and last_name
		$names	= explode( ' ' , $data[ 'name' ] );

		if( count( $names ) > 0 )
		{
			$data[ 'first_name' ]	= isset( $names[ 0 ] ) ? $names[ 0 ] : '';
			$data[ 'middle_name' ]	= isset( $names[ 1 ] ) ? $names[ 1 ] : '';
			$data[ 'last_name' ]	= isset( $names[ 2 ] ) ? $names[ 2 ] : '';
		}
		...
		{
			$avatar 	= $this->api( '/me/picture' , array( 'type' => 'large' , 'redirect' => false ));
	...
	public function getAuthorizeURL( $params )
	{
		...
		if( $scopes )
		{
			...
			$scopes	= Foundry::makeArray( $scopes );
	...
	public function getRevokeButton( $callback , $uid = null , $type = SOCIAL_TYPE_USER )
	{
		...
		$uid 	= is_null( $uid ) ? Foundry::user()->id : $uid;
