...
// Foundry::import( 'admin:/inclues/migrators/helpers/info' );
...
class SocialMigratorHelperJomsocial
{
	...
	private function addMessage( $jsMessage, $esConvObj )
	{
		...
		{
			$esMessageMap 	= Foundry::table( 'ConversationMessageMap' );
	...
	private function addConversation( $jsMsg )
	{
		...
		$esConv = Foundry::table( 'Conversation' );
		...
		foreach( $recipients as $recipient )
		{
			$esParty = Foundry::table( 'ConversationParticipant' );
	...
	private function processAvatar()
	{
		...
		foreach( $jsUsers as $jsUser )
		{
			...
			$query = 'select `created` from `#__community_activities` where actor = ' . $db->Quote( $jsUser->userid );
			$query .= ' and app = ' . $db->Quote( 'profile.avatar.upload' );
			...
			$lastAvatarUploadDate = $db->loadResult();

			if( !$lastAvatarUploadDate || $lastAvatarUploadDate == '00-00-00 00:00:00' )
			{
				...
				$query .= ' and `cid` = ' . $db->Quote( '0' );
				$query .= ' and `comment_type` IN (' . $db->Quote( 'profile.status' ) . ',' . $db->Quote( 'profile.avatar.upload' ) . ')';
				$query .= ' order by `created` desc limit 1';
				...
				if( !$lastAvatarUploadDate || $lastAvatarUploadDate == '00-00-00 00:00:00' )
				{
					...
					$query = 'select `registerDate` from `#__users` where `id` = ' . $db->Quote( $jsUser->userid );
	...
	private function processConnection()
	{
		...
		foreach( $jsConns as $jsConn )
		{
			...
			$esFriendTbl = Foundry::table( 'Friend' );
	...
	private function processProfileUsers()
	{
		...
		foreach( $jsUsers as $jsUser )
		{
			...
			if( !$jsUser->profile_id && !$jsUser->esprofileid )
			{
				...
				$userMapPro->state 		= 1;
				$userMapPro->created 	= Foundry::date()->toMySQL();
			...
			else
			{
				...
				else
				{
					...
					{
						$esFieldData 	= Foundry::table( 'FieldData' );
						...
						switch( $fieldElement )
						{
							case 'gender':
								...
								if( strtolower( $jsUserField->value ) == 'male' )
								{
									$gender = 1;
								}
								else if( strtolower( $jsUserField->value ) == 'female' )
								{
									$gender = 2;
								}

								$esFieldData->data 	= $gender;
								$esFieldData->raw 	= $esFieldData->data;
							...
							case 'url':
								...
								$esFieldData->data 	= $jsUserField->value;
								$esFieldData->raw 	= $esFieldData->data;
							...
							case 'checkbox':
								...
								$esFieldData->data 	= $json->encode( $jsArrValues );
								$esFieldData->raw 	= implode( ', ', $jsArrValues );
							...
							case 'birthday':
								...
								{
									$date = Foundry::date( $jsUserField->value );

									$esFieldData->data 	= $jsUserField->value;
									$esFieldData->raw 	= $date->toFormat('Y F d');
								}
							...
							case 'datetime':
								...
								else
								{
									...
									if( strtotime( $jsUserField->value ) !== false && strtotime( $jsUserField->value ) != -1 )
									{
										...
										$data 	= $jsUserField->value;
										$raw 	= $date->toFormat('Y F d');
									}
								}

								$esFieldData->data 	= $data;
								$esFieldData->raw 	= $raw;
							...
							case 'joomla_fullname':
								...
								if( $jsUserField->esFieldId )
								{
									$esFieldData->load( array( 'field_id' => $jsUserField->esFieldId, 'uid' => $jsUser->userid ) );

									if( $esFieldData->raw )
									{
										$default 			= $json->decode( $esFieldData->data );

										$nameObj->first		= $default->first;
										$nameObj->middle	= $default->middle;
										$nameObj->last		= $default->last;
								...
								{
									$nameObj->first	= $jsUserField->value;
								}

								if( strpos( strtolower( $jsUserField->jsfield_fieldcode ) , 'familyname' ) !== false )
								{
									$nameObj->last		= $jsUserField->value;
								}

								// now let reconstruct back again.
								$name 			= '';
								...
								$esFieldData->data 	= $json->encode( $nameObj );
								$esFieldData->raw 	= $name;
						...
						// lets add privacy on this field.
						if( $addPrivacy )
						{
							$this->addItemPrivacy( 'core.view', $jsUserField->esFieldId, 'field', $jsUser->userid, $jsUserField->access );
						}
	...
	private function addItemPrivacy( $command, $esUid, $esUType, $jsUserId, $jsAccess )
	{
		...
		$esPrivacyItem = Foundry::table( 'PrivacyItems' );
	...
	{
		$esProfile = Foundry::table( 'ProfileMap' );

		$esProfile->profile_id  = $esProfileId;
		$esProfile->user_id 	= $jsUserId;
		$esProfile->state 		= 1;
		$esProfile->created 	= Foundry::date()->toMySQL();
	...
	private function createDefaultProfile()
	{
		...
		$jsProfile->created 	= Foundry::date()->toMySQL();
	...
	private function processField( $stepId, $jsField, $esProfileId )
	{
		...
		if( $jsField->type == 'text' && ( strpos( strtolower( $jsField->fieldcode ) , 'givenname' ) !== false || strpos( strtolower( $jsField->fieldcode ) , 'familyname' ) !== false ) )
		{
			...
			$query .= ' and `step_id` = ( select `id` from `#__social_fields_steps` where `uid` = ' . $db->Quote( $esProfileId ) . ' order by `id` ASC limit 1 )';
		...
		switch( $fieldElement )
		{
			...
			case 'textbox':
				...
				if( !$jsField->min && !$jsField->max )
				{
					...
					if( $jsField->params )
					{
						$jsParams = Foundry::json()->decode( $jsField->params );
		...
		$fieldTable->params = ( $params ) ? Foundry::json()->encode( $params ) : '';
	...
	private function createDefaultItems( $profileId )
	{
		...
		$path 		= SOCIAL_ADMIN_DEFAULTS . '/profile_migrator.json';
	...
	{
		$tbl = Foundry::table( 'Migrators' );

		$tbl->oid 		= $oriId;
		$tbl->element 	= $element;
		$tbl->component = $this->name;
		$tbl->uid 		= $newId;
		$tbl->created 	= Foundry::date()->toMySQL();
