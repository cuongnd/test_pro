	...
	*/
	function save($overwrite_mode = ASIDO_OVERWRITE_DISABLED) {
		...
		foreach ($this->__operations as $o)
		{
			...
			{
				trigger_error( sprintf( 'Operation %s::%s() not found', get_class($o->callback[0]), $o->callback[1] ), E_USER_WARNING );
				continue;
			}
			...
			{
				trigger_error( sprintf( 'Failed performing %s::%s()', get_class($o->callback[0]), $o->callback[1] ), E_USER_WARNING );
				$failed = false;
			}
		}
