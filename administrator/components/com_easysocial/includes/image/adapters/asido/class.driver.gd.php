...
if (!defined('ASIDO_GD_JPEG_QUALITY')) {
	...
	$quality = (int) $config->get('photos.quality');

	define('ASIDO_GD_JPEG_QUALITY', $quality);
}
...
Class Asido_Driver_GD Extends Asido_Driver {
	...
	*/
	...
	public function isTransparent( &$tmp )
	{
		return @imagecolortransparent( $tmp ) >= 0;
	}
...
// 	function __resize(&$tmp, $width, $height ) {
//
// // 		$sourceX    = isset( $options[ 'sourceX' ] ) ? $options[ 'sourceX' ] : 0;
// // 		$sourceY    = isset( $options[ 'sourceY' ] ) ? $options[ 'sourceY' ] : 0;
// // 		$destX      = isset( $options[ 'destX' ] ) ? $options[ 'destX' ] : 0;
// // 		$destY      = isset( $options[ 'destY' ] ) ? $options[ 'destY' ] : 0;
// // 		$destWidth  = isset( $options[ 'destWidth' ] ) ? $options[ 'destWidth' ] : 0;
// // 		$destHeight = isset( $options[ 'destHeight'] ) ? $options[ 'destHeight' ] : 0;
// //
//
// 		$_ = imageCreateTrueColor($width, $height);
//
