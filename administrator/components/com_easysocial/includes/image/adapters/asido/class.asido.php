...
Class Asido {
	...
	*/
	...
	function fill(&$image, $width, $height) {

		$size = getimagesize($image->__source);

		$sourceWidth = $size[0];
		$sourceHeight = $size[1];

		$width_ratio = ($width / $sourceWidth);
		$height_ratio = ($height / $sourceHeight);
		...
		if ($width_ratio < $height_ratio)
		{
			...
			asido::_operation(
				$image, 'crop',
				array(
					'x' => (($sourceWidth * $height_ratio) - $width) / 2,
					'y' => 0,
					'width' => $width,
					'height' => $height
					)
			...
			asido::_operation(
				...
				array(
					'x' => 0,
					'y' => (($sourceHeight * $width_ratio) - $height) / 2,
					'width' => $width,
					'height' => $height
					)
			);
	...
	function outerFit(&$image, $width, $height) {
