...
class EasySocialViewSettings extends EasySocialAdminView
{
	...
	public function display( $tpl = null )
	{
		...
		// Set page heading
		$this->setHeading( JText::_( 'Settings' ) );

		// Set page icon.
		$this->setIcon( 'icon-jar jar-web_blog' );
	...
	public function renderSetting( $text, $name, $type = 'boolean', $options = array() )
	{
		...
		if( is_array( $options ) )
		{
			...
			if( isset( $options[ 'rowAttributes' ] ) )
			{
				...
				$options[ 'rowAttributes' ]	= Foundry::makeArray( $options[ 'rowAttributes' ] );
			...
			if( isset( $options['help'] ) )
			{
				...
				else
				{
					...
					$help = $theme->html( 'bootstrap.popover', $popoverTitle, $popoverDescription, 'bottom' , '' , true );
				}
	...
	{
		$selected	= Foundry::config()->get( $name );
		$theme		= $this->getSettingTheme();
