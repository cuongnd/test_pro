 ...
 * @author jseros
 */
class VirtueMartModelShortedCategory extends VirtueMartModelCategory {

    private $_category_tree;
    public $listCategoryRoute ;
    public $list_category_name_slug;
    public $shortCategory;
    public $childCategorie;
    ...
    static  public  function getChildCategoryList($vendorId, $virtuemart_category_id) {
        ...
        $query->from('#__virtuemart_category_categories AS cc');
        $query->select('cc.category_child_id AS virtuemart_category_id');
        $query->leftJoin('#__virtuemart_categories_'.VMLANG.' AS cl ON cc.category_child_id=cl.virtuemart_category_id');
        ...
        $db->setQuery( $query);
        $childList = $db->loadObjectList('virtuemart_category_id');
        return $childList;
	}

    /**
     * @param int $virtuemart_category_id
     * @param bool $thisCategory
     * @param bool $thisChildrenCategory
     * @param bool $thisChildrenProducts
     * @param int $levelProduct
     * @param $maxLevelProduct
     * @internal param $onlyByCategory
     */
    public function getImageByCategoryId($virtuemart_category_id=0)
    {

        $db=JFactory::getDbo();
        //get child_categories
        $query=$db->getQuery(true);
        $query->select('child_categories');
        $query->from('#__virtuemart_categories_en_gb AS cl');
        $query->where('cl.virtuemart_category_id='.(int)$virtuemart_category_id);
        $db->setQuery($query);
        $child_categories=$db->loadResult();
        $child_categories=$child_categories!=''?explode(',',$child_categories):array();
        $child_categories[]=$virtuemart_category_id;
        array_diff($child_categories, array(''));
        ...
        $query->leftJoin('#__virtuemart_category_medias AS cm USING(virtuemart_media_id)');
        $query->where('cm.virtuemart_category_id IN('.$child_categories.')');
        $query->order('RAND()');
        $db->setQuery($query);
        $media=$db->loadObject();
        if($media)
        ...
        else
        {
            ...
            $query->leftJoin('#__virtuemart_product_categories AS pc ON pc.virtuemart_product_id=pm.virtuemart_product_id');
            foreach($child_categories as $key=>$category)
            {
                if(!$category)
                    unset($child_categories[$key]);
            }
            $query->where('pc.virtuemart_category_id IN('.$child_categories.')');
            $query->order('RAND()');
            $db->setQuery($query);
            $media=$db->loadObject();
            if($media)
            {
                $query=$db->getQuery(true);
                $query->insert('#__virtuemart_category_medias');
                $query->columns('virtuemart_media_id,virtuemart_category_id');
                $query->values($media->virtuemart_media_id.','.$virtuemart_category_id);
                $db->setQuery($query);
                $db->execute();
                return $media;
            }else
    ...
    public function setImageForArrayCategoryIdByFirstProductOfPerCategoryId($arrayCategoryID=array(),&$listimage=array(),$level = 1, $maxlevel = 99)
    {

        $listCategoryChildrenId=array();
        $shortCategory=$this->getShortCategoryTree();
        $listAllCategoryId=array();
        foreach($arrayCategoryID as $categoryId)
        {
            $listAllCategoryId[]=$categoryId;
            $listCategoryId=$this->treeReCurseCategories($categoryId,'',array(),$shortCategory);
            foreach($listCategoryId as $key=> $categoryId1)
            {
                $listCategoryChildrenId[$categoryId][]=$key;
                $listAllCategoryId[]=$key;
            }

        }
        unset($shortCategory);
        $str_AllCategoryId=implode(',',$listAllCategoryId);
        //$str_AllCategoryId='23,25';
        unset($listAllCategoryId);
        $db=JFactory::getDbo();
        $query=$db->getQuery(true);
        $query->from('#__virtuemart_product_categories AS pc');
        $query->select('pc.virtuemart_category_id,pc.virtuemart_product_id');
        $query->where('pc.virtuemart_category_id IN('.$str_AllCategoryId.')');
        $db->setQuery($query);
        $listProducts=$db->loadObjectList();

        $listImage1=array();

        foreach($listCategoryChildrenId as $key=> $listCategory)
        {
            foreach($listCategory as $categoryId)
            {
                if($listImage[$categoryId])
                {
                    $listImage1[$key]=$listImage[$categoryId];
                    break;
                }
            }
        }
     ...
     * @return array
     */
    public function getShortCategoryTree($onlyPublished=false,$keyword='')
    {
        if(!$this->shortCategory)
        {
            $app=JFactory::getApplication();
            $input=$app->input;
            $db=JFactory::getDbo();
            ...
            $query->select('cl.category_name,cl.slug');
            $query->leftJoin('#__virtuemart_categories AS c using(virtuemart_category_id)');
            $query->select('c.virtuemart_category_id,c.ordering,c.published,c.created_by,c.shared');
            $query->leftJoin('#__virtuemart_category_categories AS cc ON cl.virtuemart_category_id=cc.category_child_id');
            $query->select('cc.category_child_id,cc.category_parent_id');
            ...
            if( !empty( $keyword ) ) {
                ...
                $query->where('cl.category_name LIKE '.$keyword.' OR cl.category_description='.$keyword);
            }
            $query->order('category_parent_id');
            $db->setQuery($query);
            $cache = JFactory::getCache('_virtuemart','');
            $key='childCategories_'.VMLANG;
            $config=JFactory::getConfig();
            $childCategories=array();

            if($config->get('caching'))
            {
                $childCategories = $cache->get($key);

                if (!$childCategories) {
                    $childCategories=$db->loadObjectList();
                    $cache->store($childCategories, $key);
                }
            ...
            else
            {
                ...
                {
                    $this->childCategories=$db->loadObjectList();
                }
                $childCategories=$this->childCategories;
            }
            $listChildrenCategories = array();
            if(!empty($childCategories)){

                $children = array();

                // First pass - collect children
                foreach ($childCategories as $v)
                {
                    $pt = $v->category_parent_id;
                    $list = @$listChildrenCategories[$pt] ? $listChildrenCategories[$pt] : array();
                    array_push($list, $v);
                    $listChildrenCategories[$pt] = $list;
                }

            }
            $this->shortCategory=$listChildrenCategories;
        }
        return $this->shortCategory;


    }
    public function getArrayCategoryIdByCategoryId($categoryId)
    {
        $arrayCategoryId=array();
        if($this->listCategoryRoute)
        {
            foreach($this->listCategoryRoute as $category)
            {
                if($category->parent==$categoryId)
                    $arrayCategoryId[]=$category->child;
            }
        ...
        $query=$db->getQuery(true);
        $query->from('#__virtuemart_category_categories AS cc');
        $query->select('cc.category_child_id');
        $query->where('cc.category_parent_id ='.(int)$categoryId);
        $db->setQuery($query);
        $arrayCategoryId=$db->loadColumn();
        return $arrayCategoryId;
    }
    function getListchildrencategoryIdByArraycategoryId($arrayCatgoryID)
    {
        $strcategoryIds=implode(',',$arrayCatgoryID);
        $db=JFactory::getDbo();
        $query=$db->getQuery(true);
        $query->from('#__virtuemart_category_categories AS cc');
        $query->select('cc.category_parent_id,cc.category_child_id');
        $query->where('cc.category_parent_id IN('.$strcategoryIds.')');
        $db->setQuery($query);
        $listchildrenCategoryId=$db->loadObjectList('category_child_id');

        return $listchildrenCategoryId;
    }

    function getimageschildproduct($category_product_ids,$cid, $vendorId, $level = 1, $maxlevel = 3)
    {
        if ($level <= $maxlevel) {
            ...
            $productids = $productModel->sortSearchListQuery(TRUE, $cid, FALSE, FALSE, $vendorId);

            //$firstproduct_id=$productids[0];
            $id_random = rand(1, count($productids));
            $firstproduct_id = $productids[$id_random];
            if ($firstproduct_id) {

                $firstproduct = $productModel->getProductSingle($firstproduct_id);
                $productModel->addImages($firstproduct);
                return $firstproduct;

            } else {
                $childs = $cache->call(array('VirtueMartModelCategory', 'getChildCategoryList'), $vendorId, $cid);
                $cids = array();
                foreach ($childs as $category) {
                    $cids[] = $category->virtuemart_category_id;
                }
                unset($childs);
                foreach ($cids as $id) {
                    $a_level = $level + 1;
                    $preturn = $this->getimageschildproduct($category_product_ids,$id, $vendorId, $a_level, $maxlevel);
                    if ($preturn)
                        return $preturn;
                }
            }
...
// 		vmdebug('getCategoryTree $limitStart',$limitStart,$limit);
	...
	public function ReCurseCategories($virtuemart_category_id,$level,$onlyPublished,$keyword,&$sortedCats){
    ...
    public function getCategories2($onlyPublished = true, $parentId = false, $childId = false, $keyword = "") {
        ...
        $query->leftJoin('#__virtuemart_categories AS c using(virtuemart_category_id)');
        $query->select('c.virtuemart_category_id,c.ordering,c.published,c.created_by,c.shared');
        $query->leftJoin('#__virtuemart_category_categories AS cx ON l.virtuemart_category_id=cx.category_child_id');
	...
	public function getCategories($onlyPublished = true, $parentId = false, $childId = false, $keyword = "") {
		...
		$select = ' c.`virtuemart_category_id`,  l.`category_name`, c.`ordering`, c.`published`, c.`created_by`, cx.`category_child_id`, cx.`category_parent_id`, c.`shared` ';
