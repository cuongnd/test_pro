...
//    }
	...
	*/
     ...
     * @param int $id
     */
    function load($oid=null,$overWriteLoadName=0,$andWhere=0,$tableJoins= array(),$joinKey = 0,$wherein=array()){
    	if(empty($this->_skey) ) {
    		vmError( 'No secondary keys defined in VmTableXarray '.$this->_tbl );
    		return false;
    	}
        static $_listdatavmtable=array();
		...
		$q = 'SELECT `'.$this->_skey.'`,`'.$this->_pkey.'` FROM `'.$this->_tbl.'` WHERE `'.$this->_pkey.'` = "'.(int)$oid.'" '.$orderby;
        if(!empty($wherein))
        {

            if(empty($_listdatavmtable))
            {
                $str_wherein=implode(',',$wherein);
                $q = 'SELECT `'.$this->_skey.'` FROM `'.$this->_tbl.'` WHERE `'.$this->_pkey.'` IN('.$str_wherein.') '.$orderby;
                $this->_db->setQuery( $q );
                $a_list=$this->_db->loadAssocList($this->_pkey);
                foreach($a_list as $item)
                {
                    $_listdatavmtable[$item[$this->_pkey]]=$item;
                }
            }
            $result=$_listdatavmtable[$oid];
