...
class Permissions extends JObject{
	...
	public function getUserGroups() {
		if (empty($this->_user_groups)) {
            ...
            $user_groups=$cache->get(array($this->_db, 'loadObjectList'), null);
            $this->_user_groups= JArrayHelper::pivot($user_groups,'group_name');
		}
		...
		if(!$this->_vendorId){
			...
			if(!empty( $user->id)){
                ...
                $query->select('virtuemart_vendor_id')
                    ...
                    ->leftJoin('#__virtuemart_userinfos AS u ON au.virtuemart_user_id = u.virtuemart_user_id')
                    ->where('u.virtuemart_user_id='.(int)$user->id)
                    ->where('au.user_is_vendor=1')
                    ;
				$db->setQuery($query);
                $cache = JFactory::getCache('_virtuemart', 'callback');
                $virtuemart_vendor_id=0;
                try
                {
                    $languages = $cache->get(array($db, 'loadResult'), null, md5($query), false);
                }
                catch (RuntimeException $e)
                {
                    // Fatal error.
                    JLog::add(JText::sprintf('JLIB_APPLICATION_ERROR_LANGUAGE_LOAD', md5($query), $e->getMessage()), JLog::WARNING, 'jerror');
                    return false;
                }
