...
only good for HTML filtering needed in MultiVendor editing Product/category ...
*/
...
class vmFilter{
	...
	private static $hl_Ids=null;

	// begin class
	public static function hl($t, $C=null, $S=array()){
		if ( is_string($C) ) $C = vmFilter::loadconfig($C);
		...
		$C['anti_link_spam'] = (isset($C['anti_link_spam']) && is_array($C['anti_link_spam']) && count($C['anti_link_spam']) == 2 && (empty($C['anti_link_spam'][0]) or vmFilter::hl_regex($C['anti_link_spam'][0])) && (empty($C['anti_link_spam'][1]) or vmFilter::hl_regex($C['anti_link_spam'][1]))) ? $C['anti_link_spam'] : 0;
		...
		if(!is_null(vmFilter::$C)){
			$reC = vmFilter::$C;
		}
		vmFilter::$C = $C;
		$S = is_array($S) ? $S : vmFilter::$hl_spec($S);
		if(!is_null(vmFilter::$S)){
			$reS = vmFilter::$S;
		}
		vmFilter::$S = $S;
		...
		if($C['cdata'] or $C['comment']){
			$t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', 'vmFilter::hl_cmtcd', $t);
		}
		$t = preg_replace_callback('`&amp;([A-Za-z][A-Za-z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', 'vmFilter::hl_ent', str_replace('&', '&amp;', $t));
		if($C['unique_ids'] && is_null(vmFilter::$hl_Ids)){
			vmFilter::$hl_Ids = array();
		}
		if($C['hook']){
			$t = $C['hook']($t, $C, $S);
		}
		if($C['show_setting'] && preg_match('`^[a-z][a-z0-9_]*$`i', $C['show_setting'])){
			vmFilter::$C['show_setting'] = array('config'=>$C, 'spec'=>$S, 'time'=>microtime());
		}
		// main
		$t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', 'vmFilter::hl_tag', $t);
		$t = $C['balance'] ? vmFilter::hl_bal($t, $C['keep_bad'], $C['parent']) : $t;
		$t = (($C['cdata'] or $C['comment']) && strpos($t, "\x01") !== false) ? str_replace(array("\x01", "\x02", "\x03", "\x04", "\x05"), array('', '', '&', '<', '>'), $t) : $t;
		$t = $C['tidy'] ? vmFilter::hl_tidy($t, $C['tidy'], $C['parent']) : $t;
		unset($C, $e);
		if(isset($reC)){
			vmFilter::$C = $reC;
		}
	...
	public static function hl_bal($t, $do=1, $in='div'){
		...
		if(vmFilter::$C['direct_list_nest']){
		...
		for($i=-1, $ci=count($t); ++$i<$ci;){
			...
			if($s){
				...
				if($p == $e){
					array_pop($q); echo '</', $e, '>'; unset($e); continue;
				} // Last open
	...
	public static function hl_prot($p, $c=null){
		...
		if(preg_match('`^([a-z\d\-+.&#; ]+?)(:|&#(58|x3a);|%3a|\\\\0{0,4}3a).`i', $p, $m) && !isset($c[strtolower($m[1])])){
			// Denied prot
			return "{$b}{$d}{$p}{$a}";
		}
		if($C['abs_url']){
	...
	public static function hl_spec($t){
		...
		for($i = count(($t = explode(';', $t))); --$i>=0;){
			...
			foreach(explode(',', $a) as $v){
				...
				if(isset($y[$x]['match']) && !vmFilter::hl_regex($y[$x]['match'])){
					unset($y[$x]['match']);
				}
				if(isset($y[$x]['nomatch']) && !vmFilter::hl_regex($y[$x]['nomatch'])){
					unset($y[$x]['nomatch']);
				}
			}
			if(!count($y) && !count($n)){
	...
	public static function hl_tag($t){
		...
		if($C['make_tag_strict'] && isset($eD[$e])){
			$trt = vmFilter::hl_tag2($e, $a, $C['make_tag_strict']);
			if(!$e){
				return (($C['keep_bad']%2) ? str_replace(array('<', '>'), array('&lt;', '&gt;'), $t) : '');
			}
		}
		...
		foreach($aA as $k=>$v){
			if(((isset($C['deny_attribute']['*']) ? isset($C['deny_attribute'][$k]) : !isset($C['deny_attribute'][$k])) or isset($rl[$k])) && ((!isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) && (isset($aN[$k][$e]) or (isset($aNU[$k]) && !isset($aNU[$k][$e])))){
				...
				if($k == 'style' && !$C['style_pass']){
					...
					$v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS', 'vmFilter::hl_prot', $v);
				...
				if(isset($rl[$k]) && is_array($rl[$k]) && ($v = vmFilter::hl_attrval($v, $rl[$k])) === 0){
					continue;
				}
				$a[$k] = str_replace('"', '&quot;', $v);
			}
		...
		if($C['unique_ids'] && isset($a['id'])){
			if(!preg_match('`^[A-Za-z][A-Za-z0-9_\-.:]*$`', ($id = $a['id'])) or (!is_null(vmFilter::$hl_Ids[$id]) && $C['unique_ids'] == 1)){
				unset($a['id']);
			}else{
				while(!is_null(vmFilter::$hl_Ids[$id])){
					$id = $C['unique_ids']. $id;
				}
				vmFilter::$hl_Ids[($a['id'] = $id)] = 1;
			}
		}
		// xml:lang
	...
	public static function kses($t, $h, $p=array('http', 'https', 'ftp', 'news', 'nntp', 'telnet', 'gopher', 'mailto')){
		...
		$C['hook'] = 'vmFilter::kses_hook';
		$C['schemes'] = '*:'. implode(',', $p);
		return vmFilter::$hl($t, $C, $h);
		// eof
	}
	...
	public static function loadconfig($c) {
		...
		switch($c){
			...
			case 'no_id_style':
				...
				$config = array('safe'=>1,'deny_attribute'=>'id, style');
				break;
			case 'only_link':
				// Permitting only attributes title and href --
				$config = array('safe'=>1,'deny_attribute'=>'* -title -href');
				break;
			case 'remove':
				// Remove bad/disallowed tags altogether instead of converting them to entities --
				$config = array('safe'=>1,'keep_bad'=>0);
				...
				// $spec = 'a=title';
				// $out = vmFilter($in, $config, $spec);
			case 'text':
			default:
				//only text and br--

				$config = array('elements'=>'strong', 'safe'=>1);
