...
class SelectHelper {
	...
	function getSelectPrivacy($var, $default){
		...
		//$put[] = JHTML::_('select.option',  30, JText::_('COM_JBLANCE_NOBODY'));
		$privacy = JHTML::_('select.genericlist', $put, $var, '', 'value', 'text', $default);
		return $privacy;
	}
	...
	function getCheckCategory($default){
		...
		foreach($categs as $categ){
			...
			$html .= "\n\t<input type='checkbox' onclick=\"checkUncheck(this, 'cat')\" class='cat-parent-$categ->parent' alt='$categ->id' id='category_$categ->id' name='id_categ[]' value='$categ->id' $checked>&nbsp;<label class='boldfont'>$categ->category</label><br>";
			
			$subs = $this->getSubcategories($categ->id, $indent, '', 0);
			
			foreach($subs as $sub) {
				$html .= "\n\t<input type='checkbox' onclick=\"checkUncheck(this, 'cat')\" class='cat-parent-$sub->parent' alt='$sub->id' id='category_$sub->id' name='id_categ[]' value='$sub->id' $checked>&nbsp;<label class=''>$sub->category</label><br>";
	...
	function getSelectBudgetRange($var, $default, $title, $attribs, $event){
		...
		$query = "SELECT CONCAT_WS('-', budgetmin, budgetmax) AS value, title, budgetmin, budgetmax FROM `#__jblance_budget` WHERE published=1 ORDER BY ordering";
		$db->setQuery($query);
		$budgets = $db->loadObjectList();
		
		$types[] = JHTML::_('select.option', '', '- '.JText::_($title).' -');
		foreach($budgets as $item){
			$types[] = JHTML::_('select.option', $item->value, sprintf("%s ($currencysym %s - $currencysym %s)", $item->title, number_format($item->budgetmin), number_format($item->budgetmax)));
		}
