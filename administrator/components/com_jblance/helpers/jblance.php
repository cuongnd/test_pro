...
class JblanceHelper {
	...
	public static function getTotalFund($userid){
		...
		$total_fund = number_format($total_fund, 2, '.', '' );
		return $total_fund;
	}
	...
	public static function getAvarageRate($userid, $html = true){
		...
		$query = "SELECT AVG((quality_clarity+communicate+expertise_payment+professional+hire_work_again)/5) AS rating FROM #__jblance_rating ".
				 "WHERE target=".$userid." AND quality_clarity <> 0";
		$db->setQuery($query);
		$avg = $db->loadResult();
		$avg = round($avg, 2);
		...
		?>
		<div class="rating_bar fl hasTip" title="<?php echo JText::sprintf('COM_JBLANCE_RATING_VALUE_TOOLTIP', $avg); ?>">
			<div style="width:<?php echo $avg*10*2; ?>%" class=""><!-- convert the rating into percent --></div>
		</div>
		<div class="fl">(<?php echo JText::sprintf('COM_JBLANCE_COUNT_REVIEWS', $count); ?>)</div>
		<?php
		return $avg;
		}
	}
	...
	public static function whichPlan($userid = null){
		...
		if(!$id_max || $is_expired){	//user has no active plan or it is expired. so choose the default plan (free plan)
			$query = "SELECT * FROM #__jblance_plan WHERE `default_plan` = 1 AND ug_id=".$ug_id;
		}
		else {
			$query = "SELECT * FROM #__jblance_plan WHERE id = (
			SELECT plan_id FROM #__jblance_plan_subscr WHERE id = ".$id_max." )";
		}
		$db->setQuery($query);
		$plan = $db->loadObject();
	...
	public static function formatCurrency($amount, $currencySym, $currencyCode = ''){
		$formatted = $currencySym.' '.number_format($amount, 2, '.', ',');
