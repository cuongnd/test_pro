...
class com_jblanceInstallerScript {
	...
	function uninstall($parent){
		...
		foreach($manifest->modules->module as $module){
			...
			$query = "SELECT id FROM #__extensions WHERE type='module' AND element=".$db->quote($element);
			$db->setQuery($query);
			$id = $db->loadResult();
		...
		foreach($manifest->plugins->plugin as $plugin){
			...
			$query = "SELECT id FROM #__extensions WHERE type='plugin' AND element=".$db->quote($element);
			$db->setQuery($query);
			$id = $db->loadResult();
