...
class JFormFieldDuration extends JFormFieldList
{
	
	protected function getInput() {
		
		$db =& JFactory::getDBO();
		$sql = "SELECT id, title  FROM #__bookpro_category WHERE type=6 ORDER BY title ";
		$db->setQuery($sql);
		$options 	= array();
		$options[] 	= JHTML::_('select.option',  '', JText::_('Select Duration'), 'id', 'title');
