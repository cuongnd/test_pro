...
<div class="table_passenger">
	<table style="width: 100%" class="table table-bordered">
        ...
        <?php
			...
			<td><?php echo ($i++) . '.' . $pass->firstname . ' ' . $pass->lastname . ' (' . ($pass->gender ? "Male" : "Female") . ')'; ?><?php if($pass->leader){ ?><span
			...
			<td><?php echo $pass->passport; ?></td>
                    <?php if (!$input->get('tmpl')) { ?>
                        <td><a
				href="index.php?option=com_bookpro&view=order&layout=passenger&tpl=edit&order_id=<?php echo $this->order->id ?>&passenger_id=<?php echo $pass->id ?>"
				class="btn edit-passenger"><?php echo JText::_('EDIT') ?></a></td>
                        <?php } ?>
                </tr>
