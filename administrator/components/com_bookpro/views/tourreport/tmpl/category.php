...
foreach ($cats AS $cat){
	...
	$count = ReportHelper::getCountToursCat($date_from, $date_to, $tours);
	$cat->total = $total;
	$cat->count = $count;
}
      ...
      function drawTable() {
        ...
        data.addRows([
          ...
          <?php 
             if ($i < count($cats)-1){
           ?>
           ['<?php echo $v->title ?>',  {v: <?php echo $v->count ?>},{v:<?php echo $v->total; ?>,f:'<?php echo CurrencyHelper::formatprice($v->total); ?>'}],
           <?php }else{ ?>
           ['<?php echo $v->title ?>',  {v: <?php echo $v->count ?>},{v:<?php echo $v->total; ?>,f:'<?php echo CurrencyHelper::formatprice($v->total); ?>'}]
           <?php } ?>
          <?php 
