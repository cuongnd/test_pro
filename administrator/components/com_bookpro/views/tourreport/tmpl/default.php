...
AImporter::helper('contants','date','currency','order');
...
?>
<div class="row-fluid">
<div class="span2"><?php echo $this->loadTemplate('menu')?></div>
<div class="span10">


<?php echo $this->loadTemplate('search')?>;</div>
...
<?php 
	...
	for ($i = 0;$i < $num;$i++){
		...
		//$total = BookProHelper::getSummary($date_start,$date_end);				$total = OrderHelper::getTotal($date_start,$date_end,'TOUR');
		if ($total == NULL){
			$total = 0;
		}
		$days[$i+1] = $total;
...
<div>
	...
	for ($i = 1;$i <=12;$i++){
		...
		$end_date = DateHelper::endMonth($num,$i,$y);
				$total = OrderHelper::getTotal($start_date,$end_date,'TOUR');
		if ($total == NULL){
			$total = 0;
		}
		$data[$i] = $total;
