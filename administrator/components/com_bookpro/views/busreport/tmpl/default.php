...
AImporter::helper('contants','date','currency','busreport');
AImporter::model('agents');

$model = new BookProModelAgents();

$model->init($this->lists);
$this->items = $model->getAgentReportDriver();
...
<script>
	jQuery(document).ready(function($) {
     ...
     $( "#end" ).datepicker( "option", $.datepicker.regional['vi']);
     $( "#end" ).datepicker( "option", "dateFormat", "dd-mm-yy" );
     $("input#end").datepicker("setDate","<?php echo $end ?>" );
...
<div>
	...
	<div>
		<form action="index.php" name="adminForm" id="adminForm" method="POST">
			...
			<div class="width-100 fltrt">
				...
				<fieldset class="adminform">
					...
					<legend>
						<?php echo JText::_('COM_BOOKPRO_BUSREPORT_NEW_ORDER') ?>
					</legend>


					<h2 class="titlePage">
						<?php echo JText::_('COM_BOOKPRO_BUSREPORT_ORDER_LIST'); ?>
					</h2>
					<div class="clr"></div>
					<div>
		...
		echo $this->getAgenSelectBox($this->lists['agent_id']);?>
						</div>
						<div class="report-filter-left">
							...
							<tr class="row<?php echo ($i % 2); ?>">
								...
								<td><a href="<?php echo $link; ?>"><?php echo $subject->title; ?>
									...
									<div>
										...
										echo JText::sprintf('COM_BOOKPRO_BUSREPORT_DEPART',BusReportHelper::getAgentBusStop($subject->id));
										?>
									</div>
								</td>
								...
								if ($report->seat == NULL) {
									...
									?>
										<?php 
										echo JText::sprintf('COM_BOOKPRO_BUSREPORT_TICKET_SEAT',$report->ticket,$report->seat);
										?>
										<?php 	 	
									}else{
										echo 0;
