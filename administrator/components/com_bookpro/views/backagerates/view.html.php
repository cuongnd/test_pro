...
AImporter::model('hotels','packagerates','package');
//import custom icons
AHtml::importIcons();



class BookProViewpackageRates extends BookproJViewLegacy
{
    /**
    ...
    function display($tpl = null)
    {
        ...
        $document->setTitle(JText::_('List of packagerates'));
        $model = new BookProModelpackageRates();
        $this->lists = array();
        ...
        if(count($this->items)>0)
        {
            for($i=0; $i<count($this->items); $i++)
            {
                ...
                if($package){
                $item->package_id = $package->package_type;
                }
            }
        }
