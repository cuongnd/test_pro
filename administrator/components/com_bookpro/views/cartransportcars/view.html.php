...
AImporter::model('carcategory');
//import needed assets
AHtml::importIcons();
if (! defined('SESSION_PREFIX')) {
	if (IS_ADMIN) {
		define('SESSION_PREFIX', 'bookpro_car_transport_car_list_');
	}
}


class BookProViewCarTransportCars extends BookproJViewLegacy
{
    /**
    ...
    function display($tpl = null)
    {
        ...
        $model = new BookProModelCarTransportCars();
        ...
        $this->lists['name'] = ARequest::getUserStateFromRequest('name', '', 'string');
        $model->init($this->lists);
        
        $this->pagination = &$model->getPagination();
        
        $this->items = &$model->getData();     //var_dump($this->items); die;
        
        if(count($this->items)>1){
            for($i=0 ;$i<count($this->items);$i++)
            {
                $item = &$this->items[$i];
                $modelCarcategory = new BookProModelCarCategory();
                $modelCarcategory->setId($item->car_category_id);
                $carcategory = $modelCarcategory->getObject();
                $item->car_category_id = $carcategory->title;
            }   
        }
