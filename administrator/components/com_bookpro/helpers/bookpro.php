...
class BookProHelper {
    ...
    static function getPassengerGroup($name, $selected = null) {
        ...
        return AHtml::getFilterSelect($name, JText::_("COM_BOOKPRO_SELECT_GROUP"), $results, $selected, false, 'class="input-medium pBirthday[]"', 'id', 'title');
    }
    ...
    static function getCountryTourBookSelect($select, $field_id = 'country', $field = 'country') {
        ...
        $state->set('list.limit', 0);
        $fullList = $model->getItems();
        return AHtml::getFilterSelect($field, JText::_('COM_BOOKPRO_SELECT_COUNTRY'), $fullList, $select, false, 'class="validate-select" id="' . $field_id . '"', 'id', 'country_name');
    }
