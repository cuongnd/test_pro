...
class TourHelper {
	...
	static function getBookedTour($order_id){
		...
		$query->from('#__bookpro_tour AS tour')
			->select('tour.*')
			->leftJoin('#__bookpro_tour_package AS tour_package ON tour_package.tour_id=tour.id')
			->leftJoin('#__bookpro_packagerate AS packagerate ON packagerate.tourpackage_id=tour_package.id')
			->leftJoin('#__bookpro_orderinfo as orderinfo ON orderinfo.obj_id=packagerate.id')
			->where('orderinfo.order_id='.(int)$order_id)
			->leftJoin('#__bookpro_supplier AS supplier ON supplier.id=tour.supplier_id')
			->select('supplier.company AS supplier_company')
			;
