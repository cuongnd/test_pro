...
class TransportHelper
{
	...
	public static function getTransport($id){
		$db= JFactory::getDbo();
		$query = "SELECT t.* FROM #__bookpro_transport AS t where  t.id = ".$id;
		$db->setQuery($query);
	...
	public static function buildAdminReport($datefrom,$dateto){
		...
		$query->select('info.*, o.user_id AS cus_id, o.total AS total,o.created AS receiveDate, o.notes AS notes, o.order_number AS ordNo,o.order_status AS order_status,o.pay_status AS pay_status');
		...
		foreach ($infos as $info) {
			$tourobj=TransportHelper::getTransport($info->obj_id);
	...
	public static function buildDriverReport($datefrom,$dateto){
		...
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);
		$query->select('info.*, o.user_id AS cus_id, o.total AS total, o.notes AS notes, o.order_number AS ordNo,o.order_status AS order_status,o.pay_status AS pay_status');
		$query->from('#__bookpro_orderinfo AS info');
		$query->leftJoin('#__bookpro_orders AS o ON info.order_id=o.id');
		...
		foreach ($infos as $info) {
			...
			$tModel->setId($info->obj_id);
			$route=$tModel->getObject();
			$info->pickup=$route->tfrom;
			$info->drop=$route->tto;
			$info->depart_time=$tourobj->start_time;
			$cusModel=new BookProModelCustomer();
			$cusModel->setId($info->cus_id);
