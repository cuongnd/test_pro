 ...
 * @version 	$Id: itinerary.php 66 2012-07-31 23:46:01Z quannv $
 **/
...
class BookProControllerItinerary extends AController
{
    ...
    function __construct($config = array())
    {
        ...
        $this->_model = $this->getModel('itinerary');
        $this->_controllerName = CONTROLLER_ITINERARY;
    }
    ...
    function display()
    {
        ...
        JRequest::setVar('view', 'itineraries');
        parent::display();
    }
	...
	function ajaxDest(){
		...
		$input = $app->input;
		$cid = $input->get('cid',0);
		$id = $input->get('dest_id',0);
		AImporter::model('airports','itinerarydests');
		$model = new BookProModelAirports();
		$lists = $model->getDestbyParent($id);
		if ($cid) {
			$imodel = new BookProModelItineraryDests();
			$dest = $imodel->getDestIdByItinerary($cid);
		}else{
			$dest = array();
		}
		if (!empty($lists)) {
			echo AHtml::checkBoxList($lists,'itinerarydest[]','',$dest,'id', 'title');
		}else{
			echo '<label class="control-label">'.JText::_('COM_BOOKPRO_NO_DESTINATION_CHILDREN').'</label>';
		}
    ...
    function saveorder()
    {
        ...
        ARequest::redirectList(CONTROLLER_ITINERARY);
    }
    ...
    function save($apply = false)
    {
        ...
        $itinerary = $input->post->get('itinerarydest', array(), 'array');
        
         
        
        $id = $this->_model->store($post);
        if ($id) {
        	$model = new BookProModelItineraryDest();
        	$model->deleteByItinerary($id);
        	
        	foreach ($itinerary as $dest){
        		$idest = array('id'=>0,'itinerary_id'=>$id,'dest_id'=>$dest);
        		$model->store($idest);
        	}
        }
        ...
        if ($apply) {
            ARequest::redirectEdit(CONTROLLER_ITINERARY, $id);
        } else {
            ARequest::redirectList(CONTROLLER_ITINERARY);
        }
