<?php
    /**
    * @package 	Bookpro
    * @author 		Nguyen Dinh Cuong
    * @link 		http://ibookingonline.com
    * @copyright 	Copyright (C) 2011 - 2012 Nguyen Dinh Cuong
    * @license 	GNU/GPLv3 http://www.gnu.org/licenses/gpl-3.0.html
    * @version 	$Id: transport.php 66 2012-07-31 23:46:01Z quannv $
    **/

    defined('_JEXEC') or die('Restricted access');

    //import needed JoomLIB helpers
    AImporter::helper('request', 'controller');
    AImporter::model("cartransportcar", "cartransportcars");

    class BookProControllerCarTransport extends AController
    {


        var $_model;

        function __construct($config = array())
        {
            parent::__construct($config);
            $this->_model = $this->getModel('cartransport');
            $this->_controllerName = CONTROLLER_CAR_TRANSPORT;
        }

        /**
        * Display default view - Airport list	
        */
        function display()
        {
            switch ($this->getTask()) {
                case 'publish':
                    $this->state($this->getTask());
                    break;
                case 'unpublish':
                    $this->state($this->getTask());
                    break;
                case 'trash':
                    $this->state($this->getTask());
                    break;
                default:
            }
            JRequest::setVar('view', 'cartransports');
            parent::display();
        }

        /**
        * Open editing form page
        */
        function editing()
        {
            parent::editing('cartransport');
        }

        /**
        * Cancel edit operation. Check in subject and redirect to subjects list. 
        */
        function cancel()
        {
            parent::cancel('Subject editing canceled');
        }

        /**
        * Save items ordering 
        */
        function saveorder()
        {
            JRequest::checkToken() or jexit('Invalid Token');

            $cids = ARequest::getCids();
            $order = ARequest::getIntArray('order');
            if (ARequest::controlCids($cids, 'save order')) {
                $mainframe = &JFactory::getApplication();
                if ($this->_model->saveorder($cids, $order)) {
                    $mainframe->enqueueMessage(JText::_('Successfully saved order'), 'message');
                } else {
                    $mainframe->enqueueMessage(JText::_('Order save failed'), 'error');
                }
            }
            ARequest::redirectList(CONTROLLER_CAR_TRANSPORT);
        }

        /**
        * Move item up in ordered list
        */
        function orderup()
        {
            $this->setOrder(- 1);
        }

        /**
        * Move item down in ordered list
        */
        function orderdown()
        {
            $this->setOrder(1);
        }

        /**
        * Set item order
        * 
        * @param int $direct move direction
        */
        function setOrder($direct)
        {
            JRequest::checkToken() or jexit('Invalid Token');
            $cid = ARequest::getCid();
            $mainframe = &JFactory::getApplication();
            if ($this->_model->move($cid, $direct)) {
                $mainframe->enqueueMessage(JText::_('Successfully moved item'), 'message');
            } else {
                $mainframe->enqueueMessage(JText::_('Item move failed'), 'error');
            }
            ARequest::redirectList(CONTROLLER_CAR_TRANSPORT);
        }


        /**
        * Save subject and state on edit page.
        */
        function apply()
        {
            $this->save(true);
        }


        /**
        * Save subject.
        * 
        * @param boolean $apply true state on edit page, false return to browse list
        */
        function save($apply = false)
        {
            JRequest::checkToken() or jexit('Invalid Token');


            $mainframe = &JFactory::getApplication();

            $post = JRequest::get('post');


            $post['id'] = ARequest::getCid();   

            $post['text'] = JRequest::getVar('text', '', 'post', 'string', JREQUEST_ALLOWRAW);

            $id = $this->_model->store($post);

            if ($id !== false) {
                $this->DeleteCarTransportCar($id);
                $this->SaveCarTransportCar($post, $id);
                $mainframe->enqueueMessage(JText::_('Successfully saved'), 'message');
            } else {
                $mainframe->enqueueMessage(JText::_('Save failed'), 'error');
            }
            if ($apply) {
                ARequest::redirectEdit(CONTROLLER_CAR_TRANSPORT, $id);
            } else {
                ARequest::redirectList(CONTROLLER_CAR_TRANSPORT);
            }
        }
        
        function SaveCarTransportCar($post, $transport_id)
        {          
            $cartransportcar='';     
            if(count($post['car_id'])>0){
                for($i=0; $i<count($post['car_id']) ;$i++){
                    $cartransportcar['car_id'] = $post['car_id'][$i];
                    if($cartransportcar['car_id']){
                        $cartransportcar['car_price'] = $post['car_price'][$i];   
                        $cartransportcar['car_transport_id']= $transport_id; 
                        $cartransportcarModel = new BookProModelCarTransportCar();             
                        $cartransportcar_id = $cartransportcarModel->store($cartransportcar);       
                    }

                }   
            }
        }

        function DeleteCarTransportCar($transport_id)
        {
            $lists = array('car_transport_id'=>$transport_id);
            $cartransportcarsModel = new BookProModelCarTransportCars();     
            $cartransportcarsModel->init($lists);
            $cartransportcars = $cartransportcarsModel->getData();  
            if(count($cartransportcars)>0)
            {
                $cid = array();
                for($i=0; $i<count($cartransportcars); $i++)
                {   
                    $cid[] = $cartransportcars[$i]->id;                                                              
                }
                $cartransportcarModel = new BookProModelCarTransportCar();     
                $cartransportcarModel->trash($cid);   

            }      
        }   
    }

?>