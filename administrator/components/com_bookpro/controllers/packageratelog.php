<?php
    ...
    AImporter::model('packageratelogs', 'packageratelog');

    class BookProControllerPackageRateLog extends AController
    {
        ...
        function __construct($config = array())
        {
            ...
            $this->_model = $this->getModel('packageratelog');
            $this->_controllerName = CONTROLLER_PACKAGE_RATE_LOG;
        }
        ...
        function display()
        {
            switch ($this->getTask()) {
                ...
                case 'trash':
                    ...
                    break;
                default:
                    JRequest::setVar('view', 'packageratelogs');
            }
        ...
        */
        function saveorder()
        {
            ...
            ARequest::redirectList(CONTROLLER_PACKAGE_RATE_LOG);
        }
...
/*            if($post['startdate']){
                $startdate = JFactory::getDate($post['startdate']); 
                $post['startdate'] = $startdate->toSlq();       
            }

            if($post['enddate']){
                $enddate   = JFactory::getDate($post['enddate']);           
                $post['enddate']   = $enddate->toSql();
            }*/
            ...
            if ($apply) {
                ARequest::redirectEdit(CONTROLLER_PACKAGE_RATE_LOG, $id);
            } else {
                ARequest::redirectList(CONTROLLER_PACKAGE_RATE_LOG);
            }
