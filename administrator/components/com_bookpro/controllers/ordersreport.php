...
class BookProControllerOrdersreport extends AController {

    var $_model;

    function __construct($config = array()) {
        parent::__construct($config);
        $this->_model = $this->getModel('ordersreport');
        $this->_controllerName = CONTROLLER_ORDERSREPORT;
    }
    ...
    function display() {
        ...
        switch ($this->getTask()) {
            ...
            case 'export_passenger':
                ...
                break;
            default:
                JRequest::setVar('view', 'ordersreport');
        }
    ...
    function updatestateorder() {
        ...
        $query->update($db->qn('#__bookpro_orders'))
                ->set($db->qn('pay_status') . ' = ' . $db->q($order_status))
                ->where('id = ' . $id);
        $db->setQuery($query);
        $db->execute();
        exit();
    ...
    function updatepaymentstatus() {
        ...
        $query = $db->getQuery(true);
        $query->update($db->qn('#__bookpro_orders'))
                ->set($db->qn('pay_method') . ' = ' . $db->q($pay_status))
                ->where('id = ' . $id);
        $db->setQuery($query);
        $db->execute();
        exit();
