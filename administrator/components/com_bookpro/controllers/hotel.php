...
class BookProControllerHotel extends AController
{
	...
	function __construct($config = array())
	{
		...
		$this->_controllerName = CONTROLLER_HOTEL;
	}
	...
	function saveorder()
	{
		...
		ARequest::redirectList(CONTROLLER_HOTEL);
	}
	...
	function save($apply = false)
	{
		...
		if ($id !== false) {
			...
			$db=JFactory::getDbo();
			$mainframe->enqueueMessage(JText::_('Save failed:').$db->getErrorMsg(), 'error');
		}
		if ($apply) {
			ARequest::redirectEdit(CONTROLLER_HOTEL, $id);
		} else {
			ARequest::redirectList(CONTROLLER_HOTEL);
		}
