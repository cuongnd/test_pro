    ...
    class BookProModeladdons extends JModelList
    {
        ...
        */
        protected function getListQuery()
        {
            ...
            if($type)
                $query->where('addon.type='.$db->q($type));
            if($object_id)
                $query->where('addon.object_id='.$object_id);
            if($this->getState('filter.search')){
            	$search = $db->quote('%' . $db->escape($this->getState('filter.search'), true) . '%');
            	$query->where('(addon.title LIKE ' . $search.')');
            }
            return $query;
        }
        ...
        function getaddonsByIds($Ids){
            $query = 'SELECT fac.* FROM #__bookpro_addon AS fac ';
            if (count($Ids)) {
        ...
        function getaddonsById($id){
            $db = JFactory::getDBO();
            $query = $db->getQuery(true);
            $query->select("a.*");
            $query->from("#__bookpro_addon as a");
            $query->where("a.id=".$id);
            $db->setQuery($query);
            return $db->loadObject();
        }

        function getFullAddons(){
            $db = JFactory::getDBO();
            $query = $db->getQuery(true);
            $query->select("a.*");
            $query->from("#__bookpro_addon as a");
        ...
        function getListQueryByFtypeandhotels($hotel_ids,$ftype='')
        {
            ...
            $query->from('#__bookpro_addon AS fac');
