...
class BookProModelOrderreceipt extends AModel
{
	var $_table;
	...
	{
		parent::__construct();
		if (! class_exists('ceceipt')) {
			AImporter::table('ceceipt');
		}
		$this->_table = $this->getTable('ceceipt');
	}
	...
	function trash($cids)
	{
		$table = $this->getTable('packageratelog');
		foreach ($cids as $id){
			if( !$table->delete($id))
			{
