scheduler.locale={
	date:{
		month_full:["Janu�r", "Febru�r", "M�rcius", "�prilis", "M�jus", "J�nius", "J�lius", "Augusztus", "Szeptember", "Okt�ber", "November", "December"],
		month_short:["Jan", "Feb", "M�r", "�pr", "M�j", "J�n", "J�l", "Aug", "Sep", "Okt", "Nov", "Dec"],
		day_full:["Vas�rnap", "H�tf�", "Kedd", "Szerda", "Cs�t�rt�k", "P�ntek", "szombat"],
    	day_short:["Va", "H�", "Ke", "Sze", "Cs�", "P�", "Szo"]
    },
    labels:{
    	dhx_cal_today_button:"Ma",
    	day_tab:"Nap",
    	week_tab:"H�t",
    	month_tab:"H�nap",
    	new_event:"�j esem�ny",
		icon_save:"Ment�s",
		icon_cancel:"M�gse",
		icon_details:"R�szletek",
		icon_edit:"Szerkeszt�s",
		icon_delete:"T�rl�s",
		confirm_closing:"",//A v�ltoz�sok elvesznek, biztosan folytatja? "
		confirm_deleting:"Az esem�ny t�r�lve lesz, biztosan folytatja?",
		section_description:"Le�r�s",
		section_time:"Id�szak",
		full_day:"Egesz napos",
		
		/*ism�tl�d� esem�nyek*/
		confirm_recurring:"Biztosan szerkeszteni akarod az �sszes ism�tl�d� esem�ny be�ll�t�s�t?",
		section_recurring:"Esem�ny ism�tl�se",
		button_recurring:"Tilt�s",
		button_recurring_open:"Enged�lyez�s",
		
		/*napirendi n�zet*/
		agenda_tab:"Napirend",
		date:"D�tum",
		description:"Le�r�s",
		
		/*�ves n�zet*/
		year_tab:"�v",

		/*week agenda view extension*/
		week_agenda_tab: "Napirend",

		/*grid view extension*/
		grid_tab:"Rács"
    }
};

