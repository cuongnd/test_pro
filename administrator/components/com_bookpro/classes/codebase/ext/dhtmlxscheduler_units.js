...
(function(){
	...
	var n = scheduler._append_mark_now;
	scheduler._append_mark_now = function(day_index, day_divs, now_div) {
		if (this._props && this._props[this._mode]) {
			var now = new Date();
			var next_day = scheduler.date.add(this._min_date, 1, "day");
			if (now < next_day && now > this._min_date && now.getHours() >= this.config.first_hour && now.getHours()<this.config.last_hour){
				for (var i = 0; i < day_divs.length-1; i++) {
					day_divs[i].appendChild(now_div.cloneNode(true));
				}
			}
		}
		else
