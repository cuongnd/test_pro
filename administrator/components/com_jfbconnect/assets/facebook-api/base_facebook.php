...
if (!function_exists('json_decode'))
{
    JFactory::getApplication()->enqueueMessage('Facebook needs the JSON PHP extension.', 'error');
}
 ...
 */
class JFBCFacebookApiException extends Exception
{
    /**
 ...
 * @author Naitik Shah <naitik@facebook.com>
 */
abstract class JFBCBaseFacebook
{
    /**
     * Version.
     */
    ...
    protected function establishCSRFTokenState()
    {
        if ($this->state === null)
        {
            // AA SourceCoast - Use the Joomla session ID instead of generating a new one.
            $this->state = JSession::getFormToken();
    ...
    protected function makeRequest($url, $params, $ch = null)
    {
        ...
        if ($errno == 60 || $errno == 77)
        {
            ...
            $app->enqueueMessage("SSL Verification Failed. Set 'Disable SSL Certificate Validation' to Yes in the JFBConnect Configuration", 'error');
            /*
        ...
        if ($result === false)
        {
            $e = new JFBCFacebookApiException(array(
     ...
     */
    // SC SourceCoast - Changing this to public - In the future, our FacebookApiLibrary class should derive from this class.
    ...
    {
        $e = new JFBCFacebookApiException($result);
        switch ($e->getType())
        {
            // OAuth 2.0 Draft 00 style
            case 'OAuthException':
