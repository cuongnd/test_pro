...
if (count($item)) {
	$type = JRequest::getInt('easyblog', 0);
	if ($type == 1) {
		$part_link = JURI::root()."index.php?option=com_easyblog&view=entry&id=";
	} elseif ($type == 2) {
		$part_link = JURI::root()."index.php?option=com_easyblog&view=categories&layout=listings&id=";
	}
}
...
window.addEvent('domready', function () {
	if (document.getElementById('easyblog').value == 0) {
	  document.getElementById('easyblog').value = 1;
...
<form action="index.php" method="post" name="adminForm" id="adminForm">
            	...
            	<table width="100%">
					...
					<tr>
						<td  style="float:right;" colspan="2">
							<?php 
								if (JRequest::getVar('easyblog') == '1') {
									echo "<select name='itemcats_easyblog' onchange='document.adminForm.submit();'>
										<option value='0'>--- " . JText::_('COM_IJOOMLA_SEO_MTREE_SELECT_CATEGORY') . " ---</option>
									";
									foreach ($this->categs as $categ) {
										if (JRequest::getVar('itemcats_easyblog') == $categ->id) { $selected = " selected='selected' "; }
										else { $selected = NULL; }
	...
	<table width="100%" cellspacing="0" cellpadding="0" border="0" class="adminlist">
		...
		$limistart	= $app->getUserStateFromRequest('com_ijoomla_seo.easyblog'.'.list.start', 'limitstart');
		$limit		= $app->getUserStateFromRequest('com_ijoomla_seo.easyblog'.'.list.limit', 'limit');
		$k = $limistart+1;
		...
		?>
			<tr class="row<?php echo $i%2; ?>">
				...
				<td>
					<a href="<?php echo $part_link . $item->id; ?>&tmpl=component" class="modal" rel="{handler: 'iframe', size: {x: 640, y: 480}}"><?php echo "view"; ?></a>
				</td>
	...
	<input type="hidden" name="controller" value="easyblog" />
