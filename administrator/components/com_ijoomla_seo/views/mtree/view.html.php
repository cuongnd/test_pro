...
class iJoomla_SeoViewMtree extends JViewLegacy {
	...
	function createCriterias(){				
		...
		if($mtree_option == "1"){
			$default = JRequest::getVar("filter_mtreecat", "-1");			
			$mtreecatop[] = JHTML::_('select.option', '-1',  "-- ".JText::_("COM_IJOOMLA_SEO_MTREE_SELECT_CATEGORY")." --");
			$mtre_categories = $this->get("MtreeCategories");				
			if(isset($mtre_categories) && count($mtre_categories) > 0){
				foreach($mtre_categories as $key=>$value){				
					$mtreecatop[] = JHTML::_('select.option', $value["cat_id"], $value["cat_name"]);
				}
			}
			$return .= JHTML::_('select.genericlist', $mtreecatop,  'filter_mtreecat',  'class = "inputbox" size = "1" onchange = "document.adminForm.submit();"',  'value',  'text', $default);
		}
