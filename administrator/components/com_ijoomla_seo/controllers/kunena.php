...
class iJoomla_SeoControllerKunena extends iJoomla_SeoController {
	
	function __construct() {	  
		parent::__construct();
		// Register Extra tasks		
		$this->registerTask('', 'kunena');
	...
	function genMetadesc(){
		...
		$link = "index.php?option=com_ijoomla_seo&controller=kunena".$and;
	...
	function copyArticleToTitle(){
		...
		$link = "index.php?option=com_ijoomla_seo&controller=kunena";
	...
	function kunena() {
		...
		if ($model->existsKunena()) {
			JRequest::setVar( 'view', 'kunena' );
			parent::display();
		} else {
			$link = "index.php?option=com_ijoomla_seo&controller=menus";
			$msg = JText::_("COM_IJOOMLA_SEO_KUNENA_ABSENT");
			$this->setRedirect($link, $msg, 'notice');		
		}
	}		
	...
	function save() {	
		...
		if($task == "apply"){
			$link = "index.php?option=com_ijoomla_seo&controller=kunena&kunena=".JRequest::getVar('kunena', '');
		}
		elseif($task == "save"){
