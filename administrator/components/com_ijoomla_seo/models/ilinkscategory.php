...
class iJoomla_SeoModelIlinkscategory extends JModelLegacy{

	protected $context = 'com_ijoomla_seo.ilinkscategory';
	var $_total = 0;
	...
	function publish(){
		...
		foreach($ids as $key=>$value){
			$query->clear();
			$query->update('#__ijseo_ilinks_category');
	...
	function remove(){
		...
		if(is_array($ids) && count($ids) > 0){
			foreach($ids as $key=>$value){
				...
				if(intval($total_links) > 0){
					$cant_delete = TRUE;
					$sql = "select `name` from #__ijseo_ilinks_category where `id`=".intval($value);
					$db->setQuery($sql);
					$db->query();
					$name = $db->loadColumn();
					$name = $name["0"];
		...
		$query->clear();
		$query->delete('#__ijseo_ilinks_category');
		...
		if($cant_delete){
			...
			$app->redirect("index.php?option=com_ijoomla_seo&controller=ilinkscategory", JText::_("COM_IJOOMLA_SEO_THOSE_CATEGS").": ".implode(", ", $categs)." ".JText::_("COM_IJOOMLA_SEO_CANT_BE_DELETED"), "error");
			return false;
		}
