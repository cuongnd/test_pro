...
class NewsRss
{
	...
	var $cache_class = 'NewsRss_Cache';
	var $locator_class = 'NewsRss_Locator';
	var $parser_class = 'NewsRss_Parser';
	var $file_class = 'NewsRss_File';
	...
	function NewsRss($feed_url = null, $cache_location = null, $cache_max_minutes = null)
	{
		...
		$this->sanitize = new NewsRss_Sanitize;
	...
	{
		$this->rss_url = NewsRss_Misc::fix_protocol($url, 1);
	}
	
	function set_file(&$file)
	{
		if (is_a($file, 'NewsRss_File'))
		{
	...
	function set_cache_class($class = 'NewsRss_Cache')
	{
		if (NewsRss_Misc::is_a_class($class, 'NewsRss_Cache'))
		{
	...
	function set_locator_class($class = 'NewsRss_Locator')
	{
		if (NewsRss_Misc::is_a_class($class, 'NewsRss_Locator'))
		{
	...
	function set_parser_class($class = 'NewsRss_Parser')
	{
		if (NewsRss_Misc::is_a_class($class, 'NewsRss_Parser'))
		{
	...
	function set_file_class($class = 'NewsRss_File')
	{
		if (NewsRss_Misc::is_a_class($class, 'NewsRss_File'))
		{
	...
	function set_sanitize_class($object = 'NewsRss_Sanitize')
	{
	...
	function set_item_class($class = 'NewsRss_Item')
	{
		return $this->sanitize->set_item_class($class);
	}
	
	function set_author_class($class = 'NewsRss_Author')
	{
		return $this->sanitize->set_author_class($class);
	}
	
	function set_enclosure_class($class = 'NewsRss_Enclosure')
	{
	...
	function init()
	{
		...
		if ($this->sanitize->bypass_image_hotlink && !empty($_GET[$this->sanitize->bypass_image_hotlink]))
		{
			...
			NewsRss_Misc::display_file($_GET[$this->sanitize->bypass_image_hotlink], 10, $this->useragent);
		}
...
EOT;
		...
		if (!empty($this->rss_url) || !empty($this->raw_data))
		{
			...
			if (!empty($this->rss_url))
			{
				...
				if (!isset($file))
				{
					if (is_a($this->file, 'NewsRss_File') && $this->file->url == $this->rss_url)
					{
						$file =& $this->file;
					}
					else
				...
				if (!$locate->is_feed($file))
				{
					...
					if ($feed)
					{
						if ($cache && !$cache->save(array('url' => $this->rss_url, 'feed_url' => $feed)))
						{
							...
							NewsRss_Misc::error($this->error, E_USER_WARNING, __FILE__, __LINE__);
						}
						$this->rss_url = $feed;
						return $this->init();
					}
			...
			$data = NewsRss_Misc::change_encoding($data, $encoding, 'UTF-8');
			...
			if (version_compare(phpversion(), '5', '<'))
			{
				...
				{
					$data = NewsRss_Misc::utf8_bad_replace($data);
				}
			}
	...
	function get_favicon($check = false, $alternate = null)
	{
		...
		{
			$favicon = NewsRss_Misc::absolutize_url('/favicon.ico', $this->get_feed_link());
	...
	function subscribe_feed()
	{
		if (!empty($this->rss_url))
		{
			return NewsRss_Misc::fix_protocol($this->rss_url, 2);
		}
		else
		{
			return false;
	...
	function subscribe_outlook()
	{
		if (!empty($this->rss_url))
		{
			return 'outlook' . NewsRss_Misc::fix_protocol($this->rss_url, 2);
		}
		else
		{
			return false;
	...
	function subscribe_podcast()
	{
		if (!empty($this->rss_url))
		{
			return NewsRss_Misc::fix_protocol($this->rss_url, 3);
		}
		else
		{
			return false;
...
class NewsRss_Author
{
	var $name;
	var $link;
	var $email;
	
	function NewsRss_Author($name, $link, $email)
	{
		$this->name = $name;
		$this->link = $link;
		$this->email = $email;
...
class NewsRss_Enclosure
{
	var $link;
	var $type;
	var $length;

	function NewsRss_Enclosure($link, $type, $length)
	{
		$this->link = $link;
		$this->type = $type;
		$this->length = $length;
...
class NewsRss_File
{
	...
	function NewsRss_File($url, $timeout = 10, $redirects = 5, $headers = null, $useragent = null, $force_fsockopen = false)
	{
		if (class_exists('idna_convert'))
		{
			$idn = new idna_convert;
		...
		if (preg_match('/^http(s)?:\/\//i', $url))
		{
			...
			if (extension_loaded('curl') && version_compare(NewsRss_Misc::get_curl_version(), '7.10.5', '>=') && !$force_fsockopen)
			{
				$this->method = 'curl';
				$fp = curl_init();
				$headers2 = array();
				...
				{
					$this->redirects++;
					return $this->NewsRss_File($this->headers['location'], $timeout, $redirects, $headers, $useragent, $force_fsockopen);
				}
			}
			else
			{
...
class NewsRss_Cache
{
	...
	function NewsRss_Cache($location, $filename, $extension)
	{
...
class NewsRss_Misc
{
	function absolutize_url($relative, $base)
	{
		...
		if (!empty($relative))
		{
			$relative = NewsRss_Misc::parse_url($relative, false);
			...
			{
				$base = NewsRss_Misc::parse_url($base, false);
				...
				else
				{
					...
					if (!empty($relative['path']))
					{
						...
						while (!empty($input))
						{
							...
							else if (strpos($input, '/.') === 0 && NewsRss_Misc::strendpos($input, '/.') === 0)
							{
	...
	{
		$file = new NewsRss_File($url, $timeout, 5, array('X-FORWARDED-FOR' => $_SERVER['REMOTE_ADDR']), $useragent);
		$headers = $file->headers();
		if ($file->body() !== false)
		{
	...
	{
		$parsed = NewsRss_Misc::parse_url($url);
		if (!empty($parsed['scheme']) && strtolower($parsed['scheme']) != 'http' && strtolower($parsed['scheme']) != 'https')
		{
			return NewsRss_Misc::fix_protocol("$parsed[authority]$parsed[path]$parsed[query]$parsed[fragment]", $http);
		}
		if (!file_exists($url) && empty($parsed['scheme']))
		{
			return NewsRss_Misc::fix_protocol("http://$url", $http);
		}
	...
	{
		$input = NewsRss_Misc::encoding($input);
		$output = NewsRss_Misc::encoding($output);
...
class NewsRss_Locator
{
	...
	function NewsRss_Locator(&$file, $timeout = 10, $useragent = null, $file_class = 'NewsRss_File')
	{
		if (!is_a($file, 'NewsRss_File'))
		{
	...
	{
		$links = NewsRss_Misc::get_element('link', $file->body());
		$done = array();
		foreach ($links as $link)
		{
			if (!empty($link['attribs']['TYPE']['data']) && !empty($link['attribs']['HREF']['data']) && !empty($link['attribs']['REL']['data']))
			{
				...
				$href = NewsRss_Misc::absolutize_url(trim($link['attribs']['HREF']['data']), $this->file->url);
	...
	{
		$links = NewsRss_Misc::get_element('a', $file->body());
		foreach ($links as $link)
		{
			if (!empty($link['attribs']['HREF']['data']))
			{
				$href = trim($link['attribs']['HREF']['data']);
				$parsed = NewsRss_Misc::parse_url($href);
				if (empty($parsed['scheme']) || $parsed['scheme'] != 'javascript')
				{
					$current = NewsRss_Misc::parse_url($this->file->url);
					if (empty($parsed['authority']) || $parsed['authority'] == $current['authority'])
					{
						$this->local[] = NewsRss_Misc::absolutize_url($href, $this->file->url);
					}
					else
					{
						$this->elsewhere[] = NewsRss_Misc::absolutize_url($href, $this->file->url);
					}
				}
			}
		}
	...
	function extension(&$array)
	{
		...
		{
			$value = NewsRss_Misc::absolutize_url($value, $this->file->url);
...
class NewsRss_Sanitize
{
	...
	var $item_class = 'NewsRss_Item';
	var $author_class = 'NewsRss_Author';
	var $enclosure_class = 'NewsRss_Enclosure';
	...
	function set_item_class($class = 'NewsRss_Item')
	{
		if (NewsRss_Misc::is_a_class($class, 'NewsRss_Item'))
		{
	...
	function set_author_class($class = 'NewsRss_Author')
	{
		if (NewsRss_Misc::is_a_class($class, 'NewsRss_Author'))
		{
	...
	function set_enclosure_class($class = 'NewsRss_Enclosure')
	{
		if (NewsRss_Misc::is_a_class($class, 'NewsRss_Enclosure'))
		{
	...
	{		
		...
		{
			$this->feed_xmlbase = NewsRss_Misc::parse_url($url);
		...
		if (!empty($data['items']))
		{
			foreach ($data['items'] as $key => $item)
			{
				...
				{
					$this->item_xmlbase = NewsRss_Misc::absolutize_url($item['attribs']['XML:BASE'], $this->feed_xmlbase);
				}
				else if (!empty($item['attribs']['HTTP://WWW.W3.ORG/XML/1998/NAMESPACE:BASE']))
				{
					$this->item_xmlbase = NewsRss_Misc::absolutize_url($item['attribs']['HTTP://WWW.W3.ORG/XML/1998/NAMESPACE:BASE'], $this->feed_xmlbase);
				}
				else
				{
	...
	function sanitize($data, $attribs, $is_url = false, $force_decode = false)
	{
		...
		{
			$images = NewsRss_Misc::get_element('img', $data);
			foreach ($images as $img)
			{
				if (!empty($img['attribs']['SRC']['data']))
				{
					...
					$data = str_replace($img['full'], NewsRss_Misc::element_implode($img), $data);
				}
			}
		}
		...
		$data = NewsRss_Misc::change_encoding($data, $this->input_encoding, $this->output_encoding);
	...
	function replace_urls($data, $raw_url = false)
	{
		...
		if (!empty($xmlbase))
		{
			...
			{
				$xmlbase = NewsRss_Misc::absolutize_url($xmlbase, $this->item_xmlbase);
			}
			else
			{
				$xmlbase = NewsRss_Misc::absolutize_url($xmlbase, $this->feed_xmlbase);
			}
		}
		...
		if ($raw_url)
		{
			return NewsRss_Misc::absolutize_url($data, $xmlbase);
		}
		else
		{
			$attributes = array(
			...
			foreach ($attributes as $attribute)
			{
				...
				{
					$new_tag = str_replace($attrib[1], NewsRss_Misc::absolutize_url($attrib[1], $xmlbase), $attrib[0]);
	...
	function do_entites_decode($data)
	{
		...
		{
			$return = NewsRss_Misc::change_encoding(html_entity_decode($data[0], ENT_QUOTES), 'ISO-8859-1', $this->input_encoding);
			if ($return == $data[0])
			{
				$return = NewsRss_Misc::change_encoding(preg_replace_callback('/&#([x]?[0-9a-f]+);/mi', array(&$this, 'replace_num_entity'), $data[0]), 'UTF-8', $this->input_encoding);
			}
