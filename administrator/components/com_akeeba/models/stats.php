 ...
 *
 */
class AkeebaModelStats extends F0FModel
{
	/**
	...
	public function checkAndFixCommonTables()
	{
		...
		$dbInstaller = new F0FDatabaseInstaller(array(
			'dbinstaller_directory' => JPATH_ADMINISTRATOR . '/components/com_akeeba/sql/common'
		));
    ...
    public function getSiteId()
    {
        ...
        if (empty($siteId) || (md5(JUri::root()) != $siteUrl))
        {
            $siteUrl = md5(JUri::root());
	...
	public function collectStatistics($useIframe)
	{
		...
		if (!file_exists(JPATH_ROOT . '/administrator/components/com_akeeba/assets/stats/usagestats.php'))
		{
			return false;
		}
		...
		{
			require_once JPATH_ROOT . '/administrator/components/com_akeeba/assets/stats/usagestats.php';
		}
