...
class VirtuemartViewCategory extends VmView
{
    ...
    public function display($tpl = null)
    {
        ...
        $productModel = VmModel::getModel('shortedproduct');
...
//        $list_products=array();
        ...
        foreach($products as $key=> $product)
        {
            ...
            $products[$key]->prices=$productModel->getPrice ($product, array(), 1);

           /* if(!JFile::exists(JPATH_BASE.'/'.$product->file_url_thumb)&&$product->virtuemart_media_id)
            {

                $fullSizeFilenamePath=$product->file_url;
                $width=250;
                $height=300;
                $saveFilenamePath=$file_url_folder_thumb.basename($product->file_url);

                $ch = curl_init($fullSizeFilenamePath);
                ...
                $newName='thumb_'.$textrandom.basename($product->file_url);


                $resizedFilenamePath=$file_url_folder_thumb.$newName;
                $createdImage = new Img2Thumb($saveFilenamePath, (int)$width, (int)$height, $resizedFilenamePath);
                JFile::delete($saveFilenamePath);
                $query=$db->getQuery(true);
                $query->update('#__virtuemart_medias');
                $file_path=VmConfig::get('forSale_path_thumb').$newName;

                $query->set('file_url_thumb='.$db->quote($file_path));
                $query->where('virtuemart_media_id='.$product->virtuemart_media_id);
                $db->setQuery($query);
                $db->execute();
                $products[$key]->file_url_thumb=$file_path;
                $products[$key]->file_url=JUri::root().$file_path;
            }

            $products[$key]->file_url=JUri::root().$products[$key]->file_url_thumb;
            $changeproduct=productHelper::saveFileUser($product->virtuemart_product_id);
            $products[$key]->download_free=$changeproduct->download_free;
            $products[$key]->link_download=$changeproduct->link_download;*/

            $products[$key]->virtuemart_media_id = $xrefTable_media->load ((int)$product->virtuemart_product_id);
            $productModel->addimages($products[$key]);
            $products[$key]->virtuemart_media_file_Upload_id = $xrefTable->load ((int)$product->virtuemart_product_id);

            //$products[$key]->existsFileTemplate=productHelper::checkExistsFileTemplate($product->virtuemart_product_id);
