...
function virtuemartBuildRoute(&$query) {
	...
	switch ($view) {
		...
		case 'productdetails';
            $config=JComponentHelper::getParams('com_virtuemart');
            $query['Itemid']=$config->get('ItemidDetailProduct',0);
			...
			if (isset($jmenu['virtuemart_product_id'][ $query['virtuemart_product_id'] ] ) ) {
                ...
                */
				...
				if($virtuemart_product_id)
                {
					...
					$segments[] = $helper->getProductName($virtuemart_product_id,$query['slug']);
                    unset($query['slug']);
                }
			}
...
// if joomla link joomlalink/vendorname/layout
	...
	if ( $helper->compareKey($view,'orders') || $helper->activeMenu->view == 'orders') {
        ...
        $vars['view'] = 'user';
        if (  $helper->compareKey($segments[1] ,'becomevendor') ) {
            $vars['controller'] = 'user';
            $vars['task'] = 'becomevendor';
            return $vars;
        }
        elseif ( $helper->compareKey($view,'user') ) {
			array_shift($segments);
		}
	...
	if ( (substr($last_elem, -(int)$helper->seo_sufix_size ) == $helper->seo_sufix)
        ...
        $config=JComponentHelper::getParams('com_virtuemart');
        $vars['Itemid']=$config->get('ItemidDetailProduct',0);
		if($segments[count($segments)-2]=='demo')
		{
			$vars['layout'] = 'demo';
		}
		if($last_elem=='notify') {
...
class vmrouterHelper {
    ...
    public $listChildParentProduct = null;
	/*
	  * $use_id type boolean
	...
	public function getCategoryNames($virtuemart_category_id,$catMenuId=0){
        ...
        $list_category_name_slug=array();
        $cache = JFactory::getCache('_virtuemart','');

        $modelcategory=VmModel::getModel('shortedcategory');
        $key='listcategory_name_'.$this->vmlang;
        $query=$db->getQuery(true);
        $query->from('#__virtuemart_categories_'.$this->vmlang.' AS cl');
        $query->select('cl.slug AS name,cl.virtuemart_category_id');
        $db->setQuery($query);

        if($config->get('caching'))
        {
            if (!($list_category_name_slug = $cache->get($key))) {

                $list_category_name_slug=$db->loadObjectList('virtuemart_category_id');
                $cache->store($list_category_name_slug, $key);
            }

        }
        else
        {
            if(!$modelcategory->list_category_name_slug)
            {
                $modelcategory->list_category_name_slug=$db->loadObjectList('virtuemart_category_id');
            }
            $list_category_name_slug=$modelcategory->list_category_name_slug;
        }

        foreach ($parents_id as $id ) {
            $strings[]=$list_category_name_slug[$id]->name;
	...
	public function getCategoryRecurse($virtuemart_category_id,$catMenuId,$first=true ) {
        ...
        $cache = JFactory::getCache('_virtuemart','');
        $key='listCategoryRoute';
        $modelcategory=VmModel::getModel('shortedcategory');
        $db			= JFactory::getDBO();
        $query=$db->getQuery(true);
        $query->from('#__virtuemart_category_categories AS cc');
        $query->select('cc.category_child_id AS child,cc.category_parent_id AS parent');
        $query->order('cc.category_parent_id');
        $db->setQuery($query);
        if($config->get('caching'))
        {
            if (!($listCategoryRoute = $cache->get($key))) {
                $listCategoryRoute=$db->loadObjectList('child');
                $cache->store($listCategoryRoute, $key);
            }

        }
        else
        {
            if(!$modelcategory->listCategoryRoute)
            {
                $modelcategory->listCategoryRoute=$db->loadObjectList('child');
            }
            $listCategoryRoute=$modelcategory->listCategoryRoute;
        }


         $object_cat=$listCategoryRoute[$virtuemart_category_id];

        if(isset($object_cat->child))
        {
            $idsArr[] = $object_cat->child;
            if($object_cat->parent != 0 and $catMenuId != $virtuemart_category_id and $catMenuId != $object_cat->parent) {
                $this->getCategoryRecurse($object_cat->parent,$catMenuId,false);
            }
	...
	public function getProductName($id,$slug){




		return $slug.$this->seo_sufix;
	}
	...
	public function getProductId($names,$virtuemart_category_id = NULL ){
		...
		$q = 'SELECT `p`.`virtuemart_product_id`
			...
			WHERE `p`.`slug` LIKE '.$db->quote($productName).' ';
	...
	public function getManufacturerId($slug ){
		$db = JFactory::getDBO();
		$query = "SELECT `virtuemart_manufacturer_id` FROM `#__virtuemart_manufacturers_".$this->vmlang."` WHERE `slug` LIKE ".$db->quote($slug);
		$db->setQuery($query);
	...
	public function getVendorId($slug ){
		$db = JFactory::getDBO();
		$query = "SELECT `virtuemart_vendor_id` FROM `#__virtuemart_vendors_".$this->vmlang."` WHERE `slug` LIKE '".$db->quote($slug)."' ";
		$db->setQuery($query);
