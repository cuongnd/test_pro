	...
	<?php if($this->config->get('unsub_dispoptions',1)){ ?>
		...
		<?php if(!empty($this->mailid)){ ?>
			<div id="unsublist_div" class="unsubdiv">
				...
				<label for="unsublist"><?php echo str_replace(array_keys($this->replace),$this->replace,JText::_('UNSUB_CURRENT')); ?></label>
			</div>
		<?php } ?>
			<div id="unsuball_div" class="unsubdiv">
				...
				<label for="unsuball"><?php echo str_replace(array_keys($this->replace),$this->replace,JText::_('UNSUB_ALL')); ?></label>
			</div>
			<div id="unsubfull_div" class="unsubdiv">
				<input type="checkbox" value="1" name="refuse" id="refuse" />
				<label for="refuse"><?php echo str_replace(array_keys($this->replace),$this->replace,JText::_('UNSUB_FULL')); ?></label>
			</div>
		</div>
	<?php }else{
	...
	if($this->config->get('unsub_survey',1)){ ?>
		<div class="unsubsurvey" >
			...
			foreach($reasons as $i => $oneReason){
				...
				echo '<div>';
				echo '<input type="checkbox" value="'.$oneReason.'" name="survey[]" id="reason'.$i.'" /> ';
				echo '<label for="reason'.$i.'">'.$trans.'</label>';
				echo '</div>';
			} ?>
			<div id="otherreasons">
