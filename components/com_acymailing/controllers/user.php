...
class UserController extends acymailingController{
	...
	function saveunsub(){
		...
		if($user->subid != $subscriber->subid){
			echo "<script type="text/javascript">alert('ERROR : You are not allowed to modify this user'); window.history.go(-1);</script>";
			exit;
		}
		...
		if($refusemails OR $unsuball){
			...
			if(empty($allLists)){
				echo "<script type="text/javascript">alert('ERROR : Could not get the list for the mailing $mailid'); window.history.go(-1);</script>";
				exit;
			}
