...
class SubController extends acymailingController{
	...
	function optin(){
		...
		if(empty($user->email) || !$userHelper->validEmail($user->email,true)){
			...
			else echo "<script type="text/javascript">alert('".JText::_('VALID_EMAIL',true)."'); window.history.go(-1);</script>";
			exit;
		}
		...
		if(empty($myuser->subid)){
			...
			else echo "<script type="text/javascript">alert('Could not save the user'); window.history.go(-1);</script>";
			exit;
		}
	...
	function _checkRedirectUrl($redirectUrl){
		...
		if($regex != 'all' && !empty($redirectUrl)){
			...
			if(!preg_match('#^'.$regex.'$#i',$domainredirect)){
				...
				echo "<script type="text/javascript">alert('This redirect url is not allowed, you should change the \"".JText::_('REDIRECTION_MODULE',true)."\" parameter from the AcyMailing configuration page to \"".$regex."\" to allow it or set it to \"all\" to allow all urls'); window.history.go(-1);</script>";
				exit;
			}
		}
	}
