...
require_once( dirname(__FILE__) . DIRECTORY_SEPARATOR . 'JSLikeHTMLElement.php');
...
class Readability
{
	...
	**/
	public $regexps = array(
		...
		'normalize' => '/\s{2,}/',
		'killBreaks' => '/(<br\s*\/?>(\s|&nbsp;?)*){1,}/',
		'video' => '/http:\/\/(.*)?(youtube|vimeo|liveleak|dailymotion|videolog)\.(.*)/i',
	...
	*/
	function __construct($html, $url=null)
	{
		...
		// *sam hack* $this->dom->preserveWhiteSpace = false;
	...
	**/
	public function init()
	{
		...
		$relaxParsing	= true;
		$articleContent = $relaxParsing ? $this->dom->createElement('div') : $this->grabArticle();
		if (!$articleContent->innerHTML) {
			for($i=0;$i<3;$i++) {
				if(!$articleContent->innerHTML) {
					if(($relaxParsing AND $relaxParsing == 1) OR (!$relaxParsing AND $this->flagIsActive(self::FLAG_STRIP_UNLIKELYS))) {
						$this->removeFlag(self::FLAG_STRIP_UNLIKELYS);
						$this->dbg('Reprocessing without parsing flag: FLAG_STRIP_UNLIKELYS');
					}
					elseif(($relaxParsing AND $relaxParsing == 2) OR (!$relaxParsing AND $this->flagIsActive(self::FLAG_WEIGHT_CLASSES))) {
						if($relaxParsing) $this->removeFlag(self::FLAG_STRIP_UNLIKELYS);
						$this->removeFlag(self::FLAG_WEIGHT_CLASSES);
						$this->dbg('Reprocessing without parsing flag: FLAG_WEIGHT_CLASSES');
					}
					elseif(($relaxParsing AND $relaxParsing == 3) OR (!$relaxParsing AND $this->flagIsActive(self::FLAG_CLEAN_CONDITIONALLY))) {
						if($relaxParsing) $this->removeFlag(self::FLAG_STRIP_UNLIKELYS);
						if($relaxParsing) $this->removeFlag(self::FLAG_WEIGHT_CLASSES);
						$this->removeFlag(self::FLAG_CLEAN_CONDITIONALLY);
						$this->dbg('Reprocessing without parsing flag: FLAG_CLEAN_CONDITIONALLY');
					}
