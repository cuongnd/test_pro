 ...
 * @since       1.5
 */
class BookproModelReset extends JModelForm
{
	/**
	...
	{
		// Get the form.
		$form = $this->loadForm('com_bookpro.reset_request', 'reset_request', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	{
		// Get the form.
		$form = $this->loadForm('com_bookpro.reset_complete', 'reset_complete', $options = array('control' => 'jform'));
		if (empty($form))
		{
			return false;
		}
	...
	{
		// Get the form.
		$form = $this->loadForm('com_bookpro.reset_confirm', 'reset_confirm', $options = array('control' => 'jform'));
		if (empty($form))
		{
			return false;
		}
	...
	protected function populateState()
	{
		...
		$params = JFactory::getApplication()->getParams('com_bookpro');
	...
	function processResetComplete($data)
	{
		...
		$token = $app->getUserState('com_bookpro.reset.token', null);
		$userId = $app->getUserState('com_bookpro.reset.user', null);
		...
		$app->setUserState('com_bookpro.reset.token', null);
		$app->setUserState('com_bookpro.reset.user', null);
	...
	function processResetConfirm($data)
	{
		...
		$app = JFactory::getApplication();
		$app->setUserState('com_bookpro.reset.token', $crypt . ':' . $salt);
		$app->setUserState('com_bookpro.reset.user', $user->id);
	...
	public function processResetRequest($data)
	{
		...
		$link = 'index.php?option=com_bookpro&view=reset&layout=confirm' . $itemid;
		...
		$body = str_replace('com_users', 'com_bookpro', $body);
