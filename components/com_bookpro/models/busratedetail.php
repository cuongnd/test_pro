...
class BookProModelbusRatedetail extends AModelFrontEnd
{
	var $_table;
	...
	{
		parent::__construct();
		if (! class_exists('TablebusRate')) {
			AImporter::table('busrate');
		}
    ...
    function getObjectBybusIdAndDate($bus_id, $date)
    {
                              
            $query = 'SELECT `obj`.* FROM `' . $this->_table->getTableName() . '` AS `obj` ';
            $query .= 'WHERE `obj`.`bustrip_id` = ' . (int) $bus_id . ' AND DATE_FORMAT(`obj`.`date`,"%d-%m-%Y")  =' .$this->_db->quote(JFactory::getDate($date)->format('d-m-Y',true));
