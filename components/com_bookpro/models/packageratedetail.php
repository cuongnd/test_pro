...
class BookProModelPackageRateDetail extends AModelFrontEnd
{
	var $_table;
	...
	function getObjectByTourPackageIdAndDate($tourpackage_id, $date)
	{

		$query = 'SELECT `obj`.* FROM `' . $this->_table->getTableName() . '` AS `obj` ';
		$query .= 'WHERE `obj`.`tourpackage_id` = ' . (int) $tourpackage_id . ' AND DATE_FORMAT(`obj`.`date`,"%d-%m-%Y")  =' .$this->_db->quote(JFactory::getDate($date)->format('d-m-Y',true));
	...
	function store($post)
	{
		...
		$db = JFactory::getDBO();
		$query = 'DELETE `obj`.* FROM `#__bookpro_packagerate` AS `obj` ';
		...
		if(count($date)>0){
			...
			{
				if($adult[$i] || $teen[$i] || $child1[$i] || $child2[$i] || $extra_bed[$i]){
					$data = array();
					...
					$tcTable=$this->getTable('packagerate');
					$tcTable->init();
					if(!$tcTable->check())
					return false;
					$tcTable->bind($data);
