...
class BookProModelFlightRate extends AModelFrontEnd
{
	var $_table;
	...
	function getRateFlight($flight_id,$date){
		$db = JFactory::getDbo();
		...
		$rdate = JFactory::getDate($date)->toSql();
		$query->where('rate.date ='.$db->quote($rdate));
	...
	function trash($cids)
	{                             
        $table = $this->getTable('flightratelog'); 
		foreach ($cids as $id){
