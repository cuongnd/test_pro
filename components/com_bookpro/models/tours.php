...
class BookProModelTours extends AModelFrontEnd
{
    ...
    function buildQuery()
    {
        $query=null;
        if(IS_ADMIN) {
           $query = 'SELECT `t`.*,`cat_id` as `tcat_id` ';
           //$query = 'SELECT `t`.*,`tour_cat`.`cat_id` as `tcat_id` ';
            $query .= 'FROM `' . $this->_table->getTableName() . '` AS `t` ';
            //$query .= 'JOIN `#__bookpro_tourcategory` AS `tour_cat` ON `tour_cat`.`tour_id` = `t`.`id`';
