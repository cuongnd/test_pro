...
class JFormFieldAirline extends JFormFieldList
{
	
	protected function getInput() {
		
		$db =& JFactory::getDBO();
		$sql = "SELECT id, title  FROM #__bookpro_airline WHERE state = 1 ORDER BY id ";
		$db->setQuery($sql);
		$options 	= array();
		$options[] 	= JHTML::_('select.option',  '', JText::_('Select Airline'), 'id', 'title');
