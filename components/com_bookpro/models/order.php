...
class BookProModelOrder extends AModelFrontEnd {
    ...
    function getInfotourById($id){
        ...
        $query->where('a.id='.$id);
        $query->join('INNER','#__bookpro_orderinfo as b ON a.id=b.order_id');
        $query->join('INNER','#__bookpro_packagerate as c ON b.obj_id=c.id');
        $query->join('INNER','#__bookpro_tour_package as d ON c.tourpackage_id=d.id');

        $query->join('INNER','#__bookpro_tour as e ON d.tour_id=e.id');
        $query->join('INNER','#__bookpro_packagetype as f ON d.packagetype_id=f.id');
        $query->join('INNER','#__bookpro_customer as g ON a.user_id=g.id');
        $query->join('INNER','#__users as h ON a.assigned_id=h.id');
        $query->join('INNER','#__bookpro_cpayorderstatus as i ON a.pay_status=i.id');
        $query->join('INNER','#__bookpro_cpayorderstatus as k ON a.order_status=k.id');
