...
class BookProModelFlightRateDetail extends AModelFrontEnd
{
	var $_table;
	...
	{
		parent::__construct();
		if (! class_exists('TableFlightRate')) {
			AImporter::table('flightrate');
		}
		$this->_table = $this->getTable('flightrate');
	}
    ...
    function getObjectByRoomIdAndDate($room_id, $date)
    {
    	
                              
            $query = 'SELECT `obj`.* FROM `' . $this->_table->getTableName() . '` AS `obj` ';
            $query .= 'WHERE `obj`.`room_id` = ' . (int) $room_id . ' AND DATE_FORMAT(`obj`.`date`,"%d-%m-%Y")  =' .$this->_db->quote(JFactory::getDate($date)->format('d-m-Y',true));
