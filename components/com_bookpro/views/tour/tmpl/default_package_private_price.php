...
foreach ($packagetypes as $key_packagetype => $packagetype) {
    ...
    foreach ($tourpackages as $key => $tourpackage) {
        ...
        $tourpackages[$key]->a_packagerate = $result;
...
//            print_r($packagetype);
            ...
            <tr class="private_price">
                ...
                foreach ($arr_person as $array_person_item) {
                    foreach ($packagetype->tourpackages as $tourpackage) {
                        ...
                        if ($tourpackage->min_person == $array_person_item) {
                            $price = $tourpackage->a_packagerate->adult;
                            break;
                        }
                        ?>
                ...
                <?php } ?>
                <td>
                    <input class="inputbox required checkin" type="hidden" name="checkin" value="<?php echo JRequest::getVar('checkin'); ?>" placeholder="<?php echo JText::_('COM_BOOKPRO_CHECKIN'); ?>" />
                    
                   
                    <?php //echo "<pre>"; print_r($packagetype->tourpackages) ?>
                </td>
                <td>
                     <div class="state">
                        <?php echo $packagetype->tourpackages[0]->a_packagerate->available ? '<span class="available"></span>' : '' ?>
                        <?php echo $packagetype->tourpackages[0]->a_packagerate->request ? '<span class="request"></span>' : '' ?>
                        <?php echo $packagetype->tourpackages[0]->a_packagerate->guaranteed ? '<span class="guaranteed"></span>' : '' ?>
                        <?php echo $packagetype->tourpackages[0]->a_packagerate->close ? '<span class="	close"></span>' : '' ?>
                        <?php echo $packagetype->tourpackages[0]->a_packagerate->adult_promo ? '<span class="promo"></span>' : '' ?>

                    </div>
                </td>

                <td><a class="bookingtourpackage" data="{&quot;packagetype_id&quot;:<?php echo $packagetype->id ?>,&quot;tour_id&quot;:<?php echo JRequest::getVar('id') ?>,&quot;stype&quot;:&quot;nonedaytripprivate&quot;}" href="javascript:void(0)"><?php echo JText::_('COM_BOOKPRO_BOOKNOW') ?></a> </td>
            </tr>
        <?php } ?>
