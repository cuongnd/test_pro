...
class BookProViewTour extends JViewLegacy
{
    ...
    var $document;
    var $blocks;
    public function get_header_column()
    {
        return array();
    }
    function display($tpl = null)
    {
        ...
        if($layout=='tourprice')
        {
            $this->blocks->prices->params->data->header_column=$this->get_header_column();
            parent::display();
            return;
        }
        ...
        //check it tour customtrip
        if ($layout == 'customtrip') {
            AImporter::model('countries');
            $modelCountry = new BookProModelCountries();
            $this->countries = $modelCountry->getItems();
            parent::display($tpl);
            return;
        }
        //end check it tour customtrip
   ...
   ,packagetype.title AS packagetype_title
        ...
        $query->group('tour_package.min_person');
        $db->setQuery($query);
