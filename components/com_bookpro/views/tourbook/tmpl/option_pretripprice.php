...
<?php
$a_pre_trip_acommodaton = $this->cart->pre_trip_acommodaton;
$a_total = 0;
?>
<h3 class="right_level1" style="text-transform: uppercase; color: #9A0000; text-align: left"><?php echo JText::_('COM_BOOKPRO_PRE_TRIP_HOTEL') ?></h3>
<?php foreach ($a_pre_trip_acommodaton as $pre_trip_acommodaton) { ?>
    <?php if (count($pre_trip_acommodaton->trip_acommodaton)) { ?>
        <div style="border-bottom: 1px dotted #738498">
            <div class="right_level">
                <div>
                    ...
                    <div class="right_bold">
                        <?php echo DateHelper::formatDate($pre_trip_acommodaton->checkin, 'd M Y'); ?> 
                    </div>
                </div>
                <div class="right_level1"><?php echo Jtext::_('COM_BOOKPRO_CHECKOUT') ?></div>
                <div class="right_bold">
                    <?php echo DateHelper::formatDate($pre_trip_acommodaton->checkout, 'd M Y'); ?> 
                </div>
            </div>
            <ul>
                <?php
                $k = 0;
                foreach ($pre_trip_acommodaton->trip_acommodaton as $trip_acommodaton) {
            ...
            <?php
            $a_total+=$pre_trip_acommodaton->total;
            ?>
            <div style="text-transform: uppercase; color: #9A0000; text-align: right"><?php echo JText::_("COM_BOOKPRO_SERVICE_FEE"); ?>:<?php echo CurrencyHelper::formatprice($pre_trip_acommodaton->total); ?></div>
        </div>
    <?php } ?>
<?php } ?>
