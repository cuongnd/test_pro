...
AImporter::model('airports','registerhotels');
//import custom icons
...
if (! defined('SESSION_PREFIX')) {
	if (IS_ADMIN) {
		...
		define('SESSION_PREFIX', 'bookpro_site_hotel_list_');
	}
}


class BookProViewRegisterHotels extends JViewLegacy
{
	/**
	...
	function display($tpl = null)
	{
        ...
        $this->lists['userid'] = AFactory::getCustomer()->id ? AFactory::getCustomer()->id : -1;        
	...
	function getCitySelectBox($select, $field = 'city_id', $autoSubmit = true)
	{
		...
		$lists = array('limit' => null , 'limitstart' => null , 'state' => null , 'order' => 'ordering' );
        $model->init($lists);
        $fullList = $model->getData();
        return AHtmlFrontEnd::getFilterSelect($field, JText::_('COM_BOOKPRO_SELECT_DESTINATION'), $fullList, $select, $autoSubmit, '', 'id', 'title');
    }
