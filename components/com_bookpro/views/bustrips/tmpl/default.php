...
function submitForm(){
	...
	if(jQuery("input:checkbox[name='bustrip_id']").is(":checked")==false)
	{
		alert("<?php echo JText::_('COM_BOOKPRO_SELECT_BUSTRIPS_WARN')?>");
	 		return false; 
	}
	if(jQuery("input:checkbox[name='return_bustrip_id']").is("*")){
		if(jQuery("input:checkbox[name='return_bustrip_id']").is(":checked")==false)
		{
			alert("<?php echo JText::_('COM_BOOKPRO_SELECT_BUSTRIPS_WARN')?>");
		 		return false; 
		}
