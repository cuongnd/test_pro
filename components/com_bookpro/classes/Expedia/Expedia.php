...
class Expedia
{
    ...
    public function __construct($account_id, $api_key,$currency_code,$minor_rev,$locale='en_US')
    {
        ...
        if($config->get('expediamode'))
        {
            $this->account_id = 55505;
            $this->api_key = 'zcp4upmc4npsm3cg82spzzq2';
        }
        $cart = JModelLegacy::getInstance('ExpediaHotelCart', 'bookpro');
        $cart->load();
        if($cart->language_code)
            $locale=$cart->language_code;
        else
        {
            $cart->language_code=$locale;
        }
        ...
        $this->set_customer_session_id( session_id() );
        $this->set_customer_ip_address($_SERVER['REMOTE_ADDR']);
        $this->set_customer_user_agent($_SERVER['HTTP_USER_AGENT']);
        $this->set_currency_code($currency_code);
        $this->set_minor_rev($minor_rev);
    ...
    {
        JLog::addLogger(array('text_file' =>'booking_flow1.txt','text_file_path'=>'logs'),JLog::ALL,array('com_bookpro'));
        $url = $this->protocol . $this->hotel_api_url . $name;
        $url = 'http://dev.api.ean.com/ean-services/rs/hotel/v3/'. $name;
        ...
        if (count($args)) {
            ...
            //echo "<pre>";
			//print_r($args[0]);
            JLog::add(JText::_($name.': param:').print_r($data,true), JLog::DEBUG);
        ...
        JLog::add(JText::_('Response:').print_r($response,true), JLog::DEBUG);
        return $response;
    }
