...
foreach ($displayData as $deal){
	$link = JRoute::_('index.php?option=com_bookpro&controller=tour&view=tour&id=' . $deal -> id . '&Itemid=' . JRequest::getVar('Itemid'));
	if($deal->image){
	...
	$price = TourHelper::getMinPriceTour($deal->id, $from_date, $to_date);
	$duration = TourHelper::getDuration($deal->days);
	$itis = TourHelper::getDestinationList($deal->id);
	$first = reset($itis);
	$end = end($itis);
?>
	...
	<div class="span7 content-bestdeals" >
		<div class="contair-bestdeals">
			...
			<div class="bestdeal-desc">
				...
				<div>
					<?php echo JText::sprintf('COM_BOOKPRO_BESTDEAL_FROM',$first->dest_name,$first->country_name); ?>
					
				</div>
				<div>
					<?php echo JText::sprintf('COM_BOOKPRO_BESTDEAL_TO',$end->dest_name,$end->country_name); ?>
