...
class AModelFrontEnd extends JModelLegacy
{
	/**
	 * Loaded data
	 *
	 ...
	 * @param array $data
	 */
	function store(&$db, &$table, $foreignKeys, $dataPrefix, &$data)
	{
		$vars = &get_object_vars($table);
		$items = array();
		foreach ($vars as $var => $value) {
			if (isset($data[$dataPrefix . $var])) {
				$items[$var] = &ARequest::getArray($dataPrefix . $var);
			}
		}
		if (count($items)) {
			$where = array();
