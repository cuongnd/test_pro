 ...
 * @since       1.6
 */
class gianhangControllergianhang extends JControllerAdmin
{
	/**
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'component', $prefix = 'gianhangModel', $config = array('ignore_request' => true))
	{
    ...
    public function duplicate()
    {
        ...
        try {
            if (empty($pks))
            {
                throw new Exception(JText::_('com_gianhang_ERROR_NO_gianhang_SELECTED'));
            }
            $model = $this->getModel();
            $model->duplicate($pks);
            $this->setMessage(JText::plural('com_gianhang_N_gianhang_DUPLICATED', count($pks)));
        } catch (Exception $e)
        {
            JError::raiseWarning(500, $e->getMessage());
        }

        $this->setRedirect('index.php?option=com_gianhang&view=gianhang');
    }
