...
$doc->addScript(JUri::root().'/gianhang/com_gianhang/views/gianhang/tmpl/assets/js/loadgianhang.js');
jimport('joomla.filesystem.folder');
$website=JFactory::getWebsite();
$db=JFactory::getDbo();
$query = $db->getQuery(true);
$query->from('#__gianhang as component')
    ->select('component.*')
    ->leftJoin('#__gioithieu AS gioithieu ON gioithieu.id=component.extension_id')
    ->where('component.type='.$query->q('component'))
    ->where('gioithieu.website_id=' . (int)$website->website_id)
...
foreach ($listComponent as $com) {
    ...
    $folder_view=JPATH_ROOT . '/gianhang/website/website_'.$website->website_id.'/' . $com->element . '/views';
    if(!JFolder::exists($folder_view))
    {
        $is_private_component=false;
        $folder_view=JPATH_ROOT . '/gianhang/' . $com->element . '/views';
    }
...
<script type="text/javascript">
    jQuery(document).ready(function ($) {
        $('ul.list-component').load_gianhang({
                            ...
                            <?php foreach ($item_layout->paths as $layout) { ?>
                                ...
                                <li  data-component-id="<?php echo $item_layout->component_id ?>" data-component="<?php echo $com ?>" data-view="<?php echo $view ?>"
                                    ...
                                    <a href="javascript:void(0)"><i
                                            class="ec-pencil2 layout-config" data-id="<?php echo $item_layout->component_id ?>" data-element-type="extension_component" data-element_path="<?php echo $item_layout->is_private_component?"gianhang/website/website_$website->website_id/$com/views/$view/tmpl/$layout":"gianhang/$com/views/$view/tmpl/$layout" ?>" ></i><?php echo JString::sub_string(JText::_($layout), 7) ?>
                                    </a>
                                </li>
                            <?php } ?>
                        </ul>
