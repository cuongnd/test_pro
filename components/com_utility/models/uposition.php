 ...
 * @since       1.6
 */
class UtilityModelUposition extends JModelAdmin
{
    /**
     ...
     * @since  1.6
     */
    protected $helpKey = 'JHELP_EXTENSIONS_position_MANAGER_EDIT';
    ...
    protected function populateState()
    {
        ...
        // Load the User state.
        $pk = $app->input->getInt('id')||$app->getUserState('position.id');
        if (!$pk)
        {
    ...
    public function duplicateAndAssign(&$pks,$website_id=0)
    {
        ...
        */
        ...
        foreach ($pks as $pk)
        {
            if ($table->load($pk, true))
            {
                ...
                $query	= $db->getQuery(true)
                    ...
                    ->from($db->quoteName('#__positions_menu'))
                    ->where($db->quoteName('positionid') . ' = ' . (int) $pk);
        ...
        if (!empty($tuples))
        {
            // position-Menu Mapping: Do it in one query
            $query = $db->getQuery(true)
                ->insert($db->quoteName('#__positions_menu'))
                ->columns($db->quoteName(array('positionid', 'menuid')))
                ->values($tuples);
    ...
    /**
     * Method to perform batch operations on a set of positions.
     *
    ...
    /**
     * Batch copy positions to a new position or current.
     *
     * @param   integer  $value     The new value matching a position position.
    ...
    protected function batchCopy($value, $pks, $contexts)
    {
        ...
        foreach ($pks as $pk)
        {
            if ($user->authorise('core.create', 'com_utility'))
            {
                ...
                // Now we need to handle the position assignments
                $db = $this->getDbo();
                $query = $db->getQuery(true)
                    ->select($db->quoteName('menuid'))
                    ->from($db->quoteName('#__positions_menu'))
                    ->where($db->quoteName('positionid') . ' = ' . $pk);
                $db->setQuery($query);
                $menus = $db->loadColumn();
                ...
                foreach ($menus as $menu)
                {
                    $query->clear()
                        ->insert($db->quoteName('#__positions_menu'))
                        ->columns(array($db->quoteName('positionid'), $db->quoteName('menuid')))
                        ->values($newId . ', ' . $menu);
                    $db->setQuery($query);
                    $db->execute();
                }
    ...
    /**
     * Batch move positions to a new position or current.
     *
     * @param   integer  $value     The new value matching a position position.
     * @param   array    $pks       An array of row IDs.
     * @param   array    $contexts  An array of item contexts.
    ...
    protected function canEditState($record)
    {
        ...
        // Default to component settings if position not known.
        else
        {
        ...
        foreach ($pks as $pk)
        {
            if ($table->load($pk))
            {
                ...
                else
                {
                    ...
                    $db    = $this->getDbo();
                    $query = $db->getQuery(true)
                        ->delete('#__positions_menu')
                        ->where('positionid=' . (int) $pk);
                    $db->setQuery($query);
                    $db->execute();
                }

                // Clear position cache
                parent::cleanCache($table->position, $table->client_id);
            }
            else
            {
                throw new Exception($table->getError());
    ...
    public function getForm($data = array(), $loadData = true)
    {
        ...
        ob_start();
        UtilityModelUPosition::render_to_xml($fields);
        ...
        $this->setState('item.position', $position);

        // Get the form.
        $form = $this->loadForm('com_utility.position', 'position', array('control' => 'jform', 'load_data' => $loadData));
        if (empty($form))
        {
            return false;
        }
    ...
    public function save($data)
    {
        ...
        // Include the content positions for the onSave events.
        ...
        // Delete old position to menu item associations
        $db    = $this->getDbo();
        $query = $db->getQuery(true)
            ->delete('#__positions_menu')
            ->where('positionid = ' . (int) $table->id);
        $db->setQuery($query);
        ...
        if (is_numeric($assignment))
        {
            ...
            // Logic check: if no position excluded then convert to display on all.
            if ($assignment == -1 && empty($data['assigned']))
            {
                $assignment = 0;
            }

            // Check needed to stop a position being assigned to `All`
            // and other menu items resulting in a position being displayed twice.
            if ($assignment === 0)
            {
                // Assign new position to `all` menu item associations.
                $query->clear()
                    ->insert('#__positions_menu')
                    ->columns(array($db->quoteName('positionid'), $db->quoteName('menuid')))
            ...
            elseif (!empty($data['assigned']))
            {
                ...
                $this->_db->setQuery(
                    'INSERT INTO #__positions_menu (positionid, menuid) VALUES ' .
                    implode(',', $tuples)
                );
        ...
        // Compute the extension id of this position in case the controller wants it.
        $query	= $db->getQuery(true)
            ->select('e.id as id')
            ->from('#__extensions AS e')
            ->join('LEFT', '#__positions AS m ON e.element = m.position')
