...
class JblanceControllerMembership extends JControllerLegacy {
	...
	function fillProjectInfo(){
		...
		$query = "SELECT p.*,b.amount bidamount FROM #__jblance_project p ".
				 ...
				 "WHERE p.id=$projectId AND b.status='COM_JBLANCE_ACCEPTED'";
		$db->setQuery($query);
		$row = $db->loadObject();
		...
		if($row){
			...
			$response['proj_balance_html'] = JText::_('COM_JBLANCE_PROJECT_BALANCE_IS').' '.$currencysym.number_format($proj_balance, 2);
	...
	function upgradeSubscription(){
		...
		$query = "SELECT * FROM #__jblance_plan_subscr WHERE id = ".$id_max;
		$db->setQuery($query);
		...
		if($id_max && $last_subscr->approved == 0){	//if he has some plan and not approved
			...
			$link	= JRoute::_('index.php?option=com_jblance&view=membership&layout=planhistory&Itemid='.$Itemid, false);
			$this->setRedirect($link, $msg);
			return false;
		}
		...
		if($count->time_limit > 0){
			if($count->plan_count >= $count->time_limit){
				...
				$link	= JRoute::_('index.php?option=com_jblance&view=membership&layout=planadd&Itemid='.$Itemid, false);
		...
		$link = JRoute::_('index.php?option=com_jblance&view=membership&layout=check_out&type=plan&Itemid='.$Itemid, false);
		$this->setRedirect($link, $msg);
	}
	...
	function addSubscription($userid){
		...
		//calculate the price
		$sql = "SELECT id, days, days_type, price, discount, bonusFund, name FROM #__jblance_plan WHERE id =".$db->quote($planid);
		$db->setQuery($sql);
		$plan = $db->loadObject();
		...
		$row->checkin();
		JRequest :: setVar('returnrowid', $row->id, 'POST');
	}
	...
	function processPaymentMethod(){
		...
		if(empty($price)){
			if($buy == 'plan')
				$link	= JRoute::_('index.php?option=com_jblance&view=membership&layout=planhistory&Itemid='.$Itemid, false);
			elseif($buy == 'deposit')
				$link	= JRoute::_('index.php?option=com_jblance&view=membership&layout=transaction&Itemid='.$Itemid, false);
		...
		if($paymode == 'banktransfer'){ // Regular Bank Transfer
			if($buy == 'plan')
				$link	= JRoute::_('index.php?option=com_jblance&view=membership&layout=bank_transfer&id='.$id.'&type=plan&Itemid='.$Itemid, false);
			elseif($buy == 'deposit')
				$link	= JRoute::_('index.php?option=com_jblance&view=membership&layout=bank_transfer&id='.$id.'&type=deposit&Itemid='.$Itemid, false);
			$this->setRedirect($link);
		}
		else {
	...
	function saveDepositFund(){
		...
		$link = JRoute::_('index.php?option=com_jblance&view=membership&layout=check_out&id='.$row->id.'&type=deposit&Itemid='.$Itemid, false);
		$this->setRedirect($link, $msg);
	}
	...
	function saveEscrow(){
		...
		if(empty($recipientInfo)){
			...
			$link	= JRoute::_('index.php?option=com_jblance&view=membership&layout=escrow&Itemid='.$Itemid, false);
			$this->setRedirect($link, $msg, 'error');
			return false;
		}
	...
	function releaseEscrow(){
		...
		$link	= JRoute::_('index.php?option=com_jblance&view=membership&layout=managepay&Itemid='.$Itemid, false);
		$this->setRedirect($link, $msg);
		return false;
	}
	...
	function saveWithdrawFund(){
		...
		if($totalFund < $post['amount']){
			$msg = JText::_('COM_JBLANCE_BALANCE_INSUFFICIENT_TO_MAKE_PAYMENT');
			$link	= JRoute::_('index.php?option=com_jblance&view=membership&layout=withdrawfund&step=2&gateway='.$post['gateway'].'&Itemid='.$Itemid, false);
			$this->setRedirect($link, $msg, 'error');
			return false;
		}
