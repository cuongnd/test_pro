...
class JblanceControllerProject extends JControllerLegacy {
	...
	function saveProject(){
		...
		if($isNew){
			...
			if($chargePerProject > 0){
				...
				$msg_debit = JText::sprintf('COM_JBLANCE_YOUR_ACCOUNT_DEBITED_WITH_CURRENCY_FOR_POSTING_PROJECT', $currencysym, $chargePerProject);
		...
		$return	= JRoute::_('index.php?option=com_jblance&view=project&layout=showmyproject&Itemid='.$Itemid, false);
	...
	function saveBid(){
		...
		if($isNew){
			...
			if($chargePerBid > 0){
				...
				$msg_debit = JText::sprintf('COM_JBLANCE_YOUR_ACCOUNT_DEBITED_WITH_CURRENCY_FOR_BIDDING_PROJECT', $currencysym, $chargePerBid);
				$app->enqueueMessage($msg_debit);
			}
		}
		...
		$return	= JRoute::_('index.php?option=com_jblance&view=project&layout=showmybid&Itemid='.$Itemid, false);
		$this->setRedirect($return, $msg);
	}
	...
	function acceptBid(){
		...
		//update transaction table for buyer
		$buyLastPlan = JblanceHelper::whichPlan($project->publisher_userid);		//get the current active plan of the publisher/buyer
		$buyProjFeeAmt = $buyLastPlan->buyFeeAmtPerProject;
		$buyProjFeePer = $buyLastPlan->buyFeePercentPerProject;
		
		//calculate the project fee from buyer, from amt & perc, whichever is higher and debit him
		$fee_bper = round((($buyProjFeePer /100) * $bid->amount), 2);
		if($fee_bper >= $buyProjFeeAmt)
			$fee_from_buyer = $fee_bper;
		else
			$fee_from_buyer = $buyProjFeeAmt;
		...
		$flLastPlan = JblanceHelper::whichPlan($project->assigned_userid);		//get the current active plan of the publisher/buyer
		$flProjFeeAmt = $flLastPlan->flFeeAmtPerProject;
		$flProjFeePer = $flLastPlan->flFeePercentPerProject;
		
		//calculate the project fee from freelancer, from amt & perc, whichever is higher and debit him
		$fee_fper = round((($flProjFeePer /100) * $bid->amount), 2);
		if($fee_fper >= $flProjFeeAmt)
			$fee_from_lancer = $fee_fper;
		else
			$fee_from_lancer = $flProjFeeAmt;
	...
	function saveRateUser(){
		...
		elseif($rate_type == 'COM_JBLANCE_FREELANCER')
			...
			$return	= JRoute::_('index.php?option=com_jblance&view=project&layout=listproject&Itemid='.$Itemid, false);
