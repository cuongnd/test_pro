...
class websiteHelperFrontEnd
{
    ...
    function treeRecurse($id, &$html, &$children, $maxLevel = 9999, $level = 0, $enableEditWebsite=true)
    {
        ...
        if ($enableEditWebsite){
            if (@$children[$id] && $level <= $maxLevel) {
                ...
                for($j=0;$j<count($items);$j++){
                    ...
                    if ($v->type == 'row')
                        $html .= '
                        <div class="row-content block-item   show-grid-stack-item '.$setClass.' '.$css_class.'" style="'.($level==0?'display:none;':'').'" data-screensize="' . $v->screensize . '" data-ordering="'.$v->ordering.'" data-block-parent-id="'.$v->parent_id .'" data-bootstrap-type="' . $v->type . '" data-block-id="' . $v->id . '" element-type="'.$v->type.'">
                    ...
                    elseif ($v->type == 'column') {
                        ...
                        <div class="grid-stack-item   show-grid-stack-item block-item grid-stack-item_' . $v->parent_id .'" data-ordering="' . $v->ordering . ' '.$css_class.'" data-block-parent-id="' . $v->parent_id . '" data-position="' . $v->position . '"  data-bootstrap-type="' . $v->type . '" data-screensize="' . $v->screensize . '"  data-block-id="' . $v->id . '" data-gs-x="' . $v->gs_x . '" data-gs-y="' . $v->gs_y . '" data-gs-width="' . $v->width . '" data-gs-height="' . $v->height . '" element-type="'.$v->type.'">
                            <div class="grid-stack-item-content edit-style allow-edit-style" data-block-parent-id="' . $v->parent_id . '" data-block-id="' . $v->id . '">
                                ...
                                <a href="javascript:void(0)" class="menu label label-danger menu-list config-block" data-block-parent-id="'.$v->parent_id .'" data-block-id="'.$v->id .'"><i class="im-menu2"></i></a>
                                <div id="position_content_' . $v->id . '" data-block-parent-id="' . $v->parent_id . '" data-axis="'.$axis.'" data-block-id="' . $v->id . '" class="position-content'. $setClass.' block-item '.($v->position == 'position-component'?'position-component':'').' '.($enableEditWebsite?' control-element ':'').' "  element-type="'.$v->type.'">';
            ...
            if (@$children[$id] && $level <= $maxLevel) {
                ...
                for($j=0;$j<count($items);$j++){
                    ...
                    if($v->type=='row'||$v->type=='column') {
                        ...
                        $html .= '<div data-screensize="' . $v->screensize . '" class=" block-item ' . ($v->type == 'row' ? $classRow : $classColumn) . $setClass . ' '.$css_class.'" data-block-id="' . $v->id . '" data-block-parent-id="'.$v->parent_id.'" data-column-type="' . $bootstrapColumnType . '" data-block-type="' . ($v->position == 'position-component' ? 'block-component' : '') . '" >';
                        if($v->position=='position-component')
                        {
    ...
    public  function getBlocksCss($listPositionsSetting=array())
    {
        ...
        foreach($listPositionsSetting as $position)
        {
            ...
            $cssBlocks.= $css;
            $css3_gradient_generator=$params->get('css3_gradient_generator','');
            ...
            $style_controller=$params->get('style_controller','');
            $css_controller='';
        ...
        <div data-bootstrap-class="container">
            <div class="main-container">
                <div class="row-content">
                    ...
                    <a class="add label label-danger add-column-in-row" href="javascript:void(0)"><i
                            class="glyphicon glyphicon-plus"></i> add column</a>
