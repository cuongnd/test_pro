...
abstract class JFBCOAuth1Client
{
    ...
    public function resetConnection()
    {
        $this->token = null;
        $this->version = null;
        $this->input->set('oauth_token', null);
    }
	...
	private function _generateRequestToken()
	{
        ...
        $data = array();
        if ($this->getOption('x_auth_access_type', null))
        {
            $scope = $this->getOption('x_auth_access_type');
            if (isset($scope[0]))
                $data['x_auth_access_type'] = $scope;
        }
		// Make an OAuth request for the Request Token.
		$response = $this->oauthRequest($this->getOption('requestTokenURL'), 'POST', $parameters, $data);
	...
	public function oauthRequest($url, $method, $parameters, $data = array(), $headers = array())
	{
        ...
        if ($this->validateResponse($url, $response))
    		return $response;
        else
            return false;
	}
