...
class TemplatesModelStyles extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				'a.id, a.template,a.website_id, a.title, a.home, a.client_id, l.title AS language_title, l.image as image'
			)
		);
		...
		$query->select('COUNT(m.template_style_id) AS assigned')
			...
			->group('a.id, a.template, a.title, a.home, a.client_id, l.title, l.image');

		// Join over the language
		$query->join('LEFT', '#__languages AS l ON l.lang_code = a.home');

		// Filter by extension enabled
		$query->select('e.id AS e_id')
