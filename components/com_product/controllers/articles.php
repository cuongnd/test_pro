...
class productControllerproducts extends JControllerAdmin
{
	 ...
	 * @return  productControllerproducts
	 * @see     JController
	 * @since   1.6
	 */
    ...
    public function aJaxGetOptionsCategory()
    {
        ...
        $options= '<option value="">' . JText::_('JSELECT') . '</option>';
        $options.=JHtml::_('select.options', JHtml::_('category.options', $extension,$config  ) );
        echo $options;
        die;
    }
	...
	public function featured()
	{
		...
		foreach ($ids as $i => $id)
		{
			if (!$user->authorise('core.edit.state', 'com_product.article.'.(int) $id))
			{
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Article', $prefix = 'productModel', $config = array('ignore_request' => true))
	{
