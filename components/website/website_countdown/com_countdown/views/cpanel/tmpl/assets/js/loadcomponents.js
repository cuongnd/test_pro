//huong dan su dung
/*
 $('.load_cpanel').load_cpanel();

 load_cpanel=$('.load_cpanel').data('load_cpanel');
 console.log(load_cpanel);
 */

// jQuery Plugin for SprFlat admin load_cpanel
// Control options and basic function of load_cpanel
...
(function($) {
    ...
    // here we go!
    $.load_cpanel = function(element, options) {

        // plugin's default options
        var defaults = {
            //main color scheme for load_cpanel
        ...
        plugin.layout_component_config=function(e_taget){
            ...
            if(show_popup_control)
            {
                $.open_popup_window({
                    ...
                    windowURL:'index.php?enable_load_component=1&option=com_cpanel&view=component&layout=config&id='+id+'&element_path='+element_path+'&tmpl=field&hide_panel_component=1',
                    centerBrowser:1,
                    width:'800',
                    menubar:0,
                    scrollbars:1,
    ...
    $.fn.load_cpanel = function(options) {

        // iterate through the DOM elements we are attaching the plugin to
        return this.each(function() {

            // if plugin has not already been attached to the element
            if (undefined == $(this).data('load_cpanel')) {
                var plugin = new $.load_cpanel(this, options);

                $(this).data('load_cpanel', plugin);
