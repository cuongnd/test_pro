 ...
 * @author jseros
 */
class VirtueMartModelCategories extends VmModel {
    ...
    function getListQuery()
    {
        ...
        $db = JFactory::getDbo();
        $query=$db->getQuery(true);
        $query->select('categories.virtuemart_category_id,categories.ordering,categories_en_gb.category_name,category_categories.category_parent_id')
            ->from('#__virtuemart_categories AS categories')
            ->leftJoin('#__virtuemart_categories_en_gb AS categories_en_gb  USING(virtuemart_category_id)')
            ->leftJoin('#__virtuemart_category_categories AS category_categories ON category_categories.category_child_id=categories.virtuemart_category_id')
            ->where('categories.website_id='.(int)$website->website_id)
        ...
        if ($search) {
            ...
            $query->where('categories_en_gb.category_name LIKE '.$search);
        }
...
// 			$childList = $this->_getList( $query );
			...
			if(!empty($childList)){
				if(!class_exists('TableCategory_medias'))require(JPATH_VM_SITE.DS.'tables'.DS.'category_medias.php');
