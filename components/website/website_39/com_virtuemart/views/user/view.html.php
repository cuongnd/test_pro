...
class VirtuemartViewUser extends VmView
{
    ...
    function display($tpl = null)
    {
        ...
        if ($user->id == 0) {
            $return=JUri::root().'index.php?option=com_virtuemart&view=user&layout=profile';
            $return=base64_encode($return);
            $app->redirect(JUri::root().'index.php?option=com_users&view=login&return='.$return);
            return;
	}
...
// 			vmdebug('Try to get $virtuemart_userinfo_id by type BT', $virtuemart_userinfo_id);
    ...
    function shopper($userFields)
    {
        ...
        if (!class_exists('VirtueMartModelShopperGroup'))
            require(JPATH_VM_SITE . DS . 'models' . DS . 'shoppergroup.php');
