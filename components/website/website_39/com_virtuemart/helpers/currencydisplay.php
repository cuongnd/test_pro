...
class CurrencyDisplay {
	...
	private function __construct ($vendorId = 0){
		...
		if (file_exists( JPATH_VM_SITE.DS.'plugins'.DS.'currency_converter'.DS.$converterFile ) and !is_dir(JPATH_VM_SITE.DS.'plugins'.DS.'currency_converter'.DS.$converterFile)) {
			$module_filename=substr($converterFile, 0, -4);
			require_once(JPATH_VM_SITE.DS.'plugins'.DS.'currency_converter'.DS.$converterFile);
			if( class_exists( $module_filename )) {
				$this->_currencyConverter = new $module_filename();
			}
		} else {

			if(!class_exists('convertECB')) require(JPATH_VM_SITE.DS.'plugins'.DS.'currency_converter'.DS.'convertECB.php');
	...
	function convertCurrencyTo($currency,$price,$shop=true){
		...
		if(is_Object($currency)){
            ...
            static $_listCurrencies = array();
            if(empty($_listCurrencies))
            {
                $db=JFactory::getDbo();
                $query=$db->getQuery(true);
                $query->from('#__virtuemart_currencies AS cur');
                $query->select('cur.currency_exchange_rate,cur.virtuemart_currency_id');
                $db->setQuery($query);
                $_listCurrencies=$db->loadObjectList('virtuemart_currency_id');
            }
			if(!empty($_listCurrencies)){
				$exchangeRate = $_listCurrencies[(int)$currency]->currency_exchange_rate;
			} else {
				$exchangeRate = 0;
			}
		}
