...
$doc->addScript(JUri::root().'/websitetemplatepro/com_websitetemplatepro/views/websitetemplatepro/tmpl/assets/js/loadwebsitetemplatepro.js');
jimport('joomla.filesystem.folder');
$website=JFactory::getWebsite();
$db=JFactory::getDbo();
$query = $db->getQuery(true);
$query->from('#__websitetemplatepro as component')
    ->select('component.*')
    ->leftJoin('#__domains AS domains ON domains.id=component.extension_id')
    ->where('component.type='.$query->q('component'))
    ->where('domains.website_id=' . (int)$website->website_id)
...
foreach ($listComponent as $com) {
    ...
    $folder_view=JPATH_ROOT . '/websitetemplatepro/website/website_'.$website->website_id.'/' . $com->element . '/views';
    if(!JFolder::exists($folder_view))
    {
        $is_private_component=false;
        $folder_view=JPATH_ROOT . '/websitetemplatepro/' . $com->element . '/views';
    }
...
<script type="text/javascript">
    jQuery(document).ready(function ($) {
        $('ul.list-component').load_websitetemplatepro({
                            ...
                            <?php foreach ($item_layout->paths as $layout) { ?>
                                ...
                                <li  data-component-id="<?php echo $item_layout->component_id ?>" data-component="<?php echo $com ?>" data-view="<?php echo $view ?>"
                                    ...
                                    <a href="javascript:void(0)"><i
                                            class="ec-pencil2 layout-config" data-id="<?php echo $item_layout->component_id ?>" data-element-type="extension_component" data-element_path="<?php echo $item_layout->is_private_component?"websitetemplatepro/website/website_$website->website_id/$com/views/$view/tmpl/$layout":"websitetemplatepro/$com/views/$view/tmpl/$layout" ?>" ></i><?php echo JString::sub_string(JText::_($layout), 7) ?>
                                    </a>
                                </li>
                            <?php } ?>
                        </ul>
