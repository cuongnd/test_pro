 ...
 * @since       1.6
 */
class supperadminModelplugins extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select(
			...
			->from($db->quoteName('#__plugins') . ' AS a');
		...
		$query->leftJoin('#__extensions AS extension ON extension.id=a.extension_id');
		$query->select('website.title as website_name')
			->join('LEFT', '#__website AS website ON website.id = extension.website_id');
		...
		if ($website_id)
		{
			$query->where('plugins.website_id = ' . $website_id);
		}
