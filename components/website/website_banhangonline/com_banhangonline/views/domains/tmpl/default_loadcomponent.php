...
$doc->addScript(JUri::root().'/banhangonline/com_banhangonline/views/banhangonline/tmpl/assets/js/loadbanhangonline.js');
jimport('joomla.filesystem.folder');
$website=JFactory::getWebsite();
$db=JFactory::getDbo();
$query = $db->getQuery(true);
$query->from('#__banhangonline as component')
    ->select('component.*')
...
foreach ($listComponent as $com) {
    ...
    $folder_view=JPATH_ROOT . '/banhangonline/website/website_'.$website->website_id.'/' . $com->element . '/views';
    if(!JFolder::exists($folder_view))
    {
        $is_private_component=false;
        $folder_view=JPATH_ROOT . '/banhangonline/' . $com->element . '/views';
    }
...
<script type="text/javascript">
    jQuery(document).ready(function ($) {
        $('ul.list-component').load_banhangonline({
                            ...
                            <?php foreach ($item_layout->paths as $layout) { ?>
                                ...
                                <li  data-component-id="<?php echo $item_layout->component_id ?>" data-component="<?php echo $com ?>" data-view="<?php echo $view ?>"
                                    ...
                                    <a href="javascript:void(0)"><i
                                            class="ec-pencil2 layout-config" data-id="<?php echo $item_layout->component_id ?>" data-element-type="extension_component" data-element_path="<?php echo $item_layout->is_private_component?"banhangonline/website/website_$website->website_id/$com/views/$view/tmpl/$layout":"banhangonline/$com/views/$view/tmpl/$layout" ?>" ></i><?php echo JString::sub_string(JText::_($layout), 7) ?>
                                    </a>
                                </li>
                            <?php } ?>
                        </ul>
