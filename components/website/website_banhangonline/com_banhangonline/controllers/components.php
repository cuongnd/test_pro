...
class supperadminControllersupperadmin extends JControllerAdmin
{
    ...
    public function duplicate()
    {
        ...
        try {
            if (empty($pks))
            {
                throw new Exception(JText::_('com_banhangonline_ERROR_NO_supperadmin_SELECTED'));
            }
            $model = $this->getModel();
            $model->duplicate($pks);
            $this->setMessage(JText::plural('com_banhangonline_N_supperadmin_DUPLICATED', count($pks)));
        } catch (Exception $e)
        {
            JError::raiseWarning(500, $e->getMessage());
        }

        $this->setRedirect('index.php?option=com_banhangonline&view=supperadmin');
    }
