...
if (!class_exists ('VirtueMartModelProduct')) {
	require(JPATH_VM_SITE . DS . 'models' . DS . 'product.php');
}
 ...
 */
class VirtueMartModelShortedProduct extends VirtueMartModelProduct {
    ...
    public  function getShortCategorProductTree()
    {
        $db=JFactory::getDbo();
        $query=$db->getQuery(true);
        $query->from('#__virtuemart_products_'.VMLANG.' AS pl');
        $query->select('pl.virtuemart_product_id,pl.product_name,pl.slug');
        ...
        $returnListProduct=array();
        foreach($listProduct as $product)
        {
            $returnListProduct[$product->virtuemart_category_id][]=$product;
        }
...
// 							vmdebug($child->product_parent_id.' $child->$k',$child->$k);
	...
	private function loadProductPrices($productId,$quantity,$virtuemart_shoppergroup_ids,$front){
        if(!$this->list_product_price)
        {
            ...
            $this->_db->setQuery($q);
            $this->list_product_price = $this->_db->loadAssocList();
		...
		return $this->list_product_price;
	}
...
// 			$joinIds = array('virtuemart_product_price_id' =>'#__virtuemart_product_prices','virtuemart_manufacturer_id' =>'#__virtuemart_product_manufacturers','virtuemart_customfield_id' =>'#__virtuemart_product_customfields');
    ...
    public function getShortProductImageTree()
    {
        if(!$this->shortProductImage)
        {
            $app=JFactory::getApplication();
            $input=$app->input;
            $db=JFactory::getDbo();
            ...
            $query->from('__virtuemart_product_medias AS pm');
            $query->select('*');
            $db->setQuery($query);
            $cache = JFactory::getCache('_virtuemart','');
            $key='ShortProductImageTree_'.VMLANG;
            $config=JFactory::getConfig();
            $childCategories=array();

            if($config->get('caching'))
            {
                $shortProductImageTree = $cache->get($key);

                if (!$shortProductImageTree) {
                    $shortProductImageTree=$db->loadObjectList();
                    $cache->store($shortProductImageTree, $key);
                }

            }
            else
            {
                if(!$this->childProductsImages)
                {
                    $this->childProductsImages=$db->loadObjectList();
                }
                $childCategories=$this->childProductsImages;
            }
            ...
            $this->shortProductImage=$listChildrenCategories;
        }
        return $this->shortProductImage;


    }
    public function searchProductsInCategory ($keyword='',$categoryIds=array(),$vendor = null,$sorting='pl.virtuemart_product_id',$downloadFree=0,$virtuemart_vendor_id=0) {
        ...
        $query->from('#__virtuemart_products_'.VMLANG.' AS pl');
        $query->select('pl.virtuemart_product_id AS virtuemart_product_id,pl.product_name,pl.slug,pl.cmstype,pl.link_download');
        ...
        $query->leftJoin('#__virtuemart_products AS p USING(virtuemart_product_id)');
        $query->select('p.virtuemart_vendor_id,p.product_parent_id,p.product_sku,p.free_download');

        $query->leftJoin('#__virtuemart_product_prices as pp USING(virtuemart_product_id)');
        $query->select('pp.*');

        $query->group('pl.virtuemart_product_id');
        if($downloadFree==0||$downloadFree==1)
        {
            $query->where('p.free_download= '.$downloadFree);
        }
        if($keyword!='')
        {
            $query->where('pl.product_name LIKE '.$db->q('%'.$keyword.'%'));
        }
        if($virtuemart_vendor_id)
        {
            $query->where('p.virtuemart_vendor_id='.$virtuemart_vendor_id);
        }

        $query->where('pc.virtuemart_category_id IN('.$categoryIds.')');
        $query->order($sorting);
        ...
        if (!isset(self::$_cache[md5($query)]))
        {
            ...
            self::$_cache[md5($query)] = $products;

            $error = $db->getErrorMsg();
        }
        else
        {
            $products = self::$_cache[md5($query)];
        }
        $this->_total=count($products);
        $products = array_slice($products, $limitStart,$limit);
        $this->addImageForProducts($products);
        return $products;
    }
	...
	public function getProductsInCategory ($categoryId,$vendor = null)
    {
        ...
        $query->leftJoin('#__virtuemart_products AS p USING(virtuemart_product_id)');
        $query->select('p.virtuemart_vendor_id,p.product_parent_id,p.product_sku');
        ...
        $query1->clear('select');
        $query1->select('pl.virtuemart_product_id');
        $db->setQuery($query1);
    ...
    function addImageForProducts(&$products)
    {
        ...
        $query->select('m.virtuemart_media_id,m.file_url,m.file_url_thumb,pm.virtuemart_product_id');
        $query->leftJoin('#__virtuemart_product_medias as pm ON pm.virtuemart_media_id=m.virtuemart_media_id');
        $query->where('pm.virtuemart_product_id IN ('.$strProductIds.')');
        $db->setQuery($query);
        //echo $db->replacePrefix($db->getQuery());
        $listMedia=$db->loadObjectList('virtuemart_product_id');
         foreach($products as $key=>$product)
         {
             $products[$key]->file_url=$listMedia[$product->virtuemart_product_id]->file_url;
             $products[$key]->virtuemart_media_id=$listMedia[$product->virtuemart_product_id]->virtuemart_media_id;
             $products[$key]->file_url_thumb=$listMedia[$product->virtuemart_product_id]->file_url_thumb;
         }
