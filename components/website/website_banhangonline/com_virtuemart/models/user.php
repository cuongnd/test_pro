...
class VirtueMartModelUser extends VmModel {
	...
	*/
	public function validateUserData($data,$type='BT') {
		...
		require(JPATH_VM_SITE . DS . 'models' . DS . 'userfields.php');
		...
		foreach ($neededFields as $field) {
			...
			//This is a special test for the virtuemart_state_id. There is the speciality that the virtuemart_state_id could be 0 but is valid.
				...
				if(!empty($data['virtuemart_country_id']) && !empty($data['virtuemart_state_id']) ){
					if (!class_exists('VirtueMartModelState')) require(JPATH_VM_SITE . DS . 'models' . DS . 'state.php');
	...
	function getUserList() {
		...
		if ($search) {
			...
			if($tableToUse!='juser'){
				...
				if(!class_exists('TableUserinfos'))require(JPATH_VM_SITE.DS.'tables'.DS.'userinfos.php');
				$db = JFactory::getDbo();
...
//DISTINCT get removed.
		...
		$this->_data = $this->exeSortSearchListQuery(0,$select,$joinedTables,$this->_getFilter(),' GROUP BY ju.id',$this->_getOrdering());
        return $this->_data;
