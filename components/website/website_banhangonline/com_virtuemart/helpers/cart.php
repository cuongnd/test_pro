...
// 		JTable::addIncludePath(JPATH_VM_SITE . DS . 'tables');
        if(empty(self::$_cart)){
            ...
            if (!empty($cartSession)) {
                ...
                self::$_cart->array_search						    = $sessionCart->array_search;
            }
...
// 		$sessionCart->prices 								= $this->prices;
        ...
        $sessionCart->array_search						    = $this->array_search;
...
// 		$pr = serialize($sessionCart->pricesUnformatted);
    ...
    public function add($virtuemart_product_ids=null,&$errorMsg='') {
        ...
        foreach ($virtuemart_product_ids as $p_key => $virtuemart_product_id) {
            ...
            vmdebug('$tmpProduct->prices',$tmpProduct->prices);
...
// 			$product -> product_price_edate = $tmpProduct -> product_price_edate;
            ...
            if ($product) {
                ...
                if (isset($post['customPrice'])) {
                    ...
                    if (isset($post['customPlugin'])){
                        ...
                        //if(!class_exists('vmFilter'))require(JPATH_VM_SITE.DS.'helpers'.DS.'vmfilter.php');
                ...
                if (array_key_exists($productKey, $this->products) && (empty($product->customPlugin)) ) {
                    ...
                    if ($this->checkForQuantities($product, $quantityPost,$errorMsg)) {
                        vmdebug('$product',$product->prices);
    ...
    private function checkoutData($redirect = true) {
        ...
        $menu_ID_editaddresscheckoutBT = $Config->get( 'menu_ID_editaddresscheckoutBT',0 );
        $menu_ID_editaddresscheckoutBT=$menu_ID_editaddresscheckoutBT?'&Itemid='.$menu_ID_editaddresscheckoutBT:'';

        if (empty($this->BT)) {
            $redirectMsg = '';
            return $this->redirecter('index.php?option=com_virtuemart&view=user&task=editaddresscheckout&addrtype=BT'.$menu_ID_editaddresscheckoutBT , $redirectMsg);
        } else {
            $redirectMsg = self::validateUserData();
            if (!$redirectMsg) {

                return $this->redirecter('index.php?option=com_virtuemart&view=user&task=editaddresscheckout&addrtype=BT'.$menu_ID_editaddresscheckoutBT , '');
            }
        }
        ...
        if($this->STsameAsBT!==0){
            ...
            if (!empty($this->ST)) {
                ...
                if (!$redirectMsg) {
                    return $this->redirecter('index.php?option=com_virtuemart&view=user&task=editaddresscheckout&addrtype=ST'.$menu_ID_editaddresscheckoutBT , '');
                }
            }
        }
        ...
        if ($store->vendor_min_pov > 0) {
            ...
            if ($prices['salesPrice'] < $store->vendor_min_pov) {
                if (!class_exists('CurrencyDisplay'))
                    require(JPATH_VM_SITE . DS . 'helpers' . DS . 'currencydisplay.php');
    ...
    function saveAddressInCart($data, $type, $putIntoSession = true) {
        ...
        if(!class_exists('VirtueMartModelUserfields')) require(JPATH_VM_SITE.DS.'models'.DS.'userfields.php' );
        $userFieldsModel = VmModel::getModel('userfields');
        $prefix = '';
    ...
    private function prepareCartPrice( ){
        ...
        foreach ($this->products as $cart_item_id=>&$product){
            ...
            if(!empty($product->customfieldsCart)){
                if(!class_exists('VirtueMartModelCustomfields'))require(JPATH_VM_SITE.DS.'models'.DS.'customfields.php');
