...
// 			vmdebug('Get standard model of the view');
		...
		if(empty(self::$_vmmodels[strtolower($className)])){
			if( !class_exists($className) ){
				...
				$modelPath = JPATH_VM_SITE.DS."models".DS.$name.".php";
    ...
    function getTable($name)
    {
        if (!class_exists ('VmTable')) {
            require_once(JPATH_VM_SITE . DS . 'helpers' . DS . 'vmtable.php');
        }
        return VmTable::getInstance($name,'Table');
    }
	...
	public function setMainTable($maintablename,$maintable=0){
        ...
        $defaultTable =VmTable::getInstance($this->_maintablename,'Table');
