...
// 		$result = $this->portProducts();
	...
	public function portMedia(){
		...
		require(JPATH_VM_SITE . DS . 'models' . DS . 'media.php');
...
// 					vmdebug('$dir',$dir);
	...
	private function portUsers(){
		...
		if(!class_exists('VirtueMartModelUser')) require(JPATH_VM_SITE . DS . 'models' . DS . 'user.php');
...
// 				$category['modified_on'] = $oldcategory['mdate'];
	...
	private function portManufacturerCategories(){
		...
		if(!class_exists('TableManufacturercategories')) require(JPATH_VM_SITE . DS . 'tables' . DS . 'manufacturercategories.php');
	...
	private function portManufacturers(){
		...
		foreach($oldManus as $oldmanu){
			if(!array_key_exists($oldmanu['manufacturer_id'],$alreadyKnownIds)){
				...
				require(JPATH_VM_SITE . DS . 'tables' . DS . 'manufacturers.php');
...
// 					$oldProducts = array();
	...
	function portOrders(){
		...
		require(JPATH_VM_SITE . DS . 'models' . DS . 'orderstatus.php');
