...
//set redirect when is not vendor in frontend
//$params=JComponentHelper::getParams('com_virtuemart');
//$menu=JMenuSite::getInstance();
//$menuItem=$menu->getItem($params->get('becomeAnAuthor'));
//JFactory::getApplication()->redirect(JRoute::_($menuItem->link),'Please press bottom "sign in to become an author"');
...
$treemenu = array(
    ...
    'COM_VIRTUEMART_ADD' => array(
        'product' => '<i class="icon-stack"></i> ' . jtext::_('COM_VIRTUEMART_PRODUCT'),
        'category' => '<i class="icon-folder"></i> ' . jtext::_('COM_VIRTUEMART_CATEGORY'),
        'manufacturer' => '<i class="icon-briefcase"></i> ' . jtext::_('COM_VIRTUEMART_MANUFACTURER'),
        'coupon' => '<i class="icon-scissors"></i> ' . jtext::_('COM_VIRTUEMART_COUPON')
    )
);
        ...
        <div class = "container-fluid">
            ...
            <div class = "nav-collapse collapse" id = "mainvmnav">
                <ul class = "nav navbar-nav" id = "menu">
                    <li>
                        <a href = "<?php echo jRoute::_('index.php?option=com_virtuemart&view=virtuemart&tmpl=component') ?>"><i class = "icon icon-dashboard"></i><span class = "desktop"><?php echo jtext::_('COM_VIRTUEMART_SHOP_HOME') ?></span></a>
                    </li>
                    <?php foreach ($treemenu as $topname => $menus) {
                        if ($topname == 'COM_VIRTUEMART_ADD') $task = 'add';
                        $hasLink = false; ?>
                        <li class = "dropdown">
                            <a href = "#" data-toggle = "dropdown" class = "dropdown-toggle"><i class = "icon icon-<?php echo $mainIcons[$topname] ?>"></i><span class = "desktop"><?php echo jtext::_($topname) ?></span><span class = "caret"></span></a>
                            <ul class = "dropdown-menu">

                                <?php foreach ($menus as $link => $name) {
                                    //echo '<div style="color: red">@'.$task.'@'.$link.'@'.'</div>';
                                    if (!ShopFunctions::can($task, $link)) continue;
                                    if ($topname == 'COM_VIRTUEMART_ADD') $link.='&task=add';
                                    $hasLink = true;
                                    ?>
                                    <li>
                ...
                <ul class = "nav pull-right">
                    <li class = "dropdown">
                        ...
                        <ul class = "dropdown-menu">
                            <li class = "">
                                <a href = "<?php echo jRoute::_('index.php?option=com_virtuemart&view=user') ?>"><i class = "icon icon-user"></i> <?php echo jText::_('COM_VIRTUEMART_YOUR_ACCOUNT_DETAILS') ?></a>
                            </li>
                            <li class = "divider"></li>
                            <li class = "">
                                <a href = "<?php echo jRoute::_('index.php?option=com_users&task=user.logout&' . jSession::getFormToken() . '=1&return=' . base64_encode('index.php?option=com_virtuemart')) ?>"><i class = "icon icon-exit"></i> <?php echo jText::_('COM_VIRTUEMART_BUTTON_LOGOUT') ?></a>
                            </li>
                        </ul>
                    </li>
                </ul>
