...
class ShopFunctions {
	...
	*/
	 ...
	 */
	static public function renderGuiList ($fieldnameXref, $tableXref, $fieldIdXref, $idXref, $fieldname, $table, $fieldId, $view, $quantity = 4, $translate = 1,$wherein=array()) {

		if (!class_exists( 'VmConfig' )) require(JPATH_COMPONENT_ADMINISTRATOR.DS.'helpers'.DS.'config.php');
		VmConfig::loadConfig();
        static $_datatable=array();
		...
		$db = JFactory::getDBO ();
        $listFieldnameXref=array();
		$q = 'SELECT ' . $db->quoteName ($fieldnameXref) . ' FROM ' . $db->quoteName ($tableXref) . ' WHERE ' . $db->quoteName ($fieldIdXref) . ' = "' . (int)$idXref . '"';
        if(!empty($wherein))
        {
            if(empty($_datatable))
            {
                $str_wherein=implode(',',$wherein);
                $q = 'SELECT ' . $db->quoteName ($fieldnameXref) . ','.$db->quoteName ($fieldIdXref).' FROM ' . $db->quoteName ($tableXref) . ' WHERE ' . $db->quoteName ($fieldIdXref) . ' IN('.$str_wherein.')';
                $db->setQuery($q);
                ...
                {
                    $_datatable[$item->$fieldIdXref][]=$item->$fieldnameXref;
                    $listFieldnameXref[]=$item->$fieldnameXref;
                }
            }
            $tempArray=$_datatable[$idXref];
		...
		if (isset($tempArray)) {
            ...
            if(is_null($_tmp))
            {
                if ($translate) {
                    ...
                    $q = 'SELECT ' . $db->quoteName ($fieldname) . ','.$db->quoteName ($fieldId).' FROM ' . $db->quoteName ($mainTable) . ' JOIN ' . $db->quoteName ($table) . ' using (`' . $fieldnameXref . '`)';
                } else {
                    $q = 'SELECT ' . $db->quoteName ($fieldname) . ','.$db->quoteName ($fieldId).' FROM ' . $db->quoteName ($table) ;
                }
                if(!empty($listFieldnameXref))
                {
                    $strFieldNameXref=implode(',',$listFieldnameXref);
                    $q.=' WHERE ' . $db->quoteName ($fieldId) . ' IN('.$strFieldNameXref.')' ;
                    //' WHERE ' . $db->quoteName ($fieldId) . ' = "' . (int)$value . '"';
                }
                $db->setQuery ($q);
                $_tmp = $db->loadObjectList($fieldId);
            }
	...
	static public function renderShopperGroupList ($shopperGroupId = 0, $multiple = TRUE,$name='virtuemart_shoppergroup_id',$id=null) {
		...
		$listHTML = JHTML::_ ('select.genericlist', $shoppergrps, $name, $attrs, 'virtuemart_shoppergroup_id', 'shopper_group_name', $shopperGroupId, $id);
		return $listHTML;
	}
	 ...
	 * @return string HTML containing the <select />
	 */
	static public function renderStateList ($stateId = '0', $prefix = '', $multiple = FALSE, $required = 0) {
	 ...
	 * $id because in multiple price same id brake javascript
	 * @author Max Milbers
	 */
	static function renderTaxList ($selected, $name = 'product_tax_id', $class = '',$id = null) {
		static $taxrates = null;
		if ($id ===null) $id = $name ;
		if ($taxrates === null) {
			if (!class_exists ('VirtueMartModelCalc')) {
						require(JPATH_VM_SITE . DS . 'models' . DS . 'calc.php');
					}
		...
		$listHTML = JHTML::_ ('Select.genericlist', $taxrates, $name, $class, $name, 'text', $selected, $id);
		return $listHTML;
	}
	 ...
	 * @author Valérie Isaksen
	 */
	static function renderWeightUnitList ($name, $selected,$extra = 'class="input-medium"') {
		...
		$listHTML = JHTML::_ ('Select.genericlist', $wu_list, $name, $extra, $name, 'text', $selected);
		return $listHTML;
		/*
		if (!class_exists('VmHTML')) require(JPATH_VM_SITE.DS.'helpers'.DS.'html.php');
	...
	static function renderLWHUnitList ($name, $selected) {
		...
		return VmHTML::selectList ($name, $selected, $lwh_unit_default,1, '', '','input-medium');
	...
	static function writePriceConfigLine ($obj, $name, $langkey) {
		...
		$html =
			'<tr>
				...
				<td align="center">
				<input type="text" value="' . $obj->get ($name . 'Rounding', 2) . '" class="inputbox input-mini" size="4" name="' . $name . 'Rounding">
				</td>
			</tr>';
		return $html;
	}
...
// 			vmTime('Start with categoryListTree');
			...
			$cache->setCaching (1);
            self::$categoryTree=ShopFunctions::categoryListTreeLoop($selectedCategories, $cid, $level, $disabledFields);
			//self::$categoryTree = $cache->call (array('ShopFunctions', 'categoryListTreeLoop'), );
...
// 		vmSetStartTime('getCategories');
	...
	static public function getCountryByID ($id, $fld = 'country_name') {
		...
		$q = 'SELECT ' . $db->quoteName ($fld) . ' AS fld FROM `#__virtuemart_countries` WHERE virtuemart_country_id = ' . (int)$id;
		$db->setQuery ($q);
	...
	static public function getCountryIDByName ($name) {
		...
		$q = 'SELECT `virtuemart_country_id` FROM `#__virtuemart_countries` WHERE `' . $db->quoteName( $fieldname ). '` = "' . $db->quote ($name) . '"';
		$db->setQuery ($q);
		$r = $db->loadResult ();
		return $r;
	}
	...
	static public function getStateByID ($id, $fld = 'state_name') {
		...
		$db = JFactory::getDBO ();
		$q = 'SELECT ' . $db->quoteName ($fld) . ' AS fld FROM `#__virtuemart_states` WHERE virtuemart_state_id = "' . (int)$id . '"';
		$db->setQuery ($q);
		$r = $db->loadObject ();
		return $r->fld;
	}
	...
	static public function getStateIDByName ($name) {
		...
		$q = 'SELECT `virtuemart_state_id` FROM `#__virtuemart_states` WHERE `' . $db->quoteName( $fieldname ). '` = "' . $db->quote ($name) . '"';
		$db->setQuery ($q);
		$r = $db->loadResult ();
		return $r;
	}
	...
	*/
	...
	static public function getCurrencyByID ($id, $fld = 'currency_name') {
		...
		$q = 'SELECT ' . $db->quoteName ($fld) . ' AS fld FROM `#__virtuemart_currencies` WHERE virtuemart_currency_id = ' . (int)$id;
		$db->setQuery ($q);
		return $db->loadResult ();
	}
	...
	static public function getCurrencyIDByName ($name) {
		...
		$q = 'SELECT `virtuemart_currency_id` FROM `#__virtuemart_currencies` WHERE `' . $db->quoteName( $fieldname ). '` = '.$db->quote ($name) ;
		$db->setQuery ($q);
		$r = $db->loadResult ();
	...
	static public function getOrderStatusName ($_code) {
		...
		$_q = 'SELECT `order_status_name` FROM `#__virtuemart_orderstates` WHERE `order_status_code` = ' . $db->quote ($_code) ;
		$db->setQuery ($_q);
		$_r = $db->loadObject ();
		if (empty($_r->order_status_name)) {
			vmError ('getOrderStatusName: couldnt find order_status_name for ' . $_code.' '.$db->quote ($_code));
	 ...
	 * or non existing eg. COM_VIRTUEMART_ORDER_STATUS_+sold out
	 * or custom format eg. JARCHIVED in the same loop
	 * @param   string   $string   The string to translate.
	 * @param   string   $prefix   The prefix to search with the string in Jtext
	 * @return  string  a translated String
	 */
	static public function altText($string ,$prefix ='com_virtuemart') {
		$lang = JFactory::getLanguage();
	...
	static public function can($task,$className){
        ...
        static $params = null;
        if ($params === null) $params = JComponentHelper::getParams('com_virtuemart', false);
