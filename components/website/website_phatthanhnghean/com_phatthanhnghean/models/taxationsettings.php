 ...
 * @license     GNU taxationsettings Public License version 2 or later; see LICENSE.txt
 */
 ...
 * @since       1.6
 */
class productsModelTaxationSettings extends JModelAdmin
{
	/**
	 * @var     string  The help screen key for the module.
	 * @since   1.6
	 */
	protected $helpKey = 'JHELP_taxationsettingsS_component_MANAGER_EDIT';
	 ...
	 * @since   1.6
	 */
	protected $event_after_save = 'ontaxationsettingsAfterSave';

	/**
	 * @var     string  The event to trigger after before the data.
	 * @since   1.6
	 */
	protected $event_before_save = 'ontaxationsettingsBeforeSave';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_phatthanhnghean.taxationsettings', 'taxationsettings', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_phatthanhnghean.edit.taxationsettings.data', array());
		...
		$this->preprocessData('com_phatthanhnghean.taxationsettings', $data);
    ...
    public function delete(&$pks)
    {
        ...
        foreach ($pks as $pk)
        {
            if ($table->load($pk))
            {
                ...
                if($table->issystem)
                {
                    $this->setError('you cannot delete taxationsettings system');
                    return false;
                }
                if (!$table->delete($pk))
                {
    ...
    public function duplicateAndAssign(&$pks,$website_id=0)
    {
        ...
        $tuples=array();
/*        // Access checks.
        if (!$user->authorise('core.create', 'com_taxationsettings'))
        {
            throw new Exception(JText::_('JERROR_CORE_CREATE_NOT_PERMITTED'));
        }*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('taxationsettings.id');
	...
	*/
	public function getTable($type = 'taxationsettings', $prefix = 'JTable', $config = array())
	{
	...
	protected function populateState()
	{
		...
		$this->setState('taxationsettings.id', $pk);
	}
