 ...
 * @since       1.6
 */
class productsModelMetadata extends JModelAdmin
{
	/**
	 * @var     string  The help screen key for the module.
	 * @since   1.6
	 */
	protected $helpKey = 'JHELP_metadataS_component_MANAGER_EDIT';
	 ...
	 * @since   1.6
	 */
	protected $event_after_save = 'onmetadataAfterSave';

	/**
	 * @var     string  The event to trigger after before the data.
	 * @since   1.6
	 */
	protected $event_before_save = 'onmetadataBeforeSave';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_phatthanhnghean.metadata', 'metadata', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_phatthanhnghean.edit.metadata.data', array());
		...
		$this->preprocessData('com_phatthanhnghean.metadata', $data);
    ...
    public function delete(&$pks)
    {
        ...
        foreach ($pks as $pk)
        {
            if ($table->load($pk))
            {
                ...
                if($table->issystem)
                {
                    $this->setError('you cannot delete metadata system');
                    return false;
                }
                if (!$table->delete($pk))
                {
    ...
    public function duplicateAndAssign(&$pks,$website_id=0)
    {
        ...
        $tuples=array();
/*        // Access checks.
        if (!$user->authorise('core.create', 'com_metadata'))
        {
            throw new Exception(JText::_('JERROR_CORE_CREATE_NOT_PERMITTED'));
        }*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('metadata.id');
	...
	*/
	public function getTable($type = 'metadata', $prefix = 'JTable', $config = array())
	{
