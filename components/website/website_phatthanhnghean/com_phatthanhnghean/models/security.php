 ...
 * @license     GNU security Public License version 2 or later; see LICENSE.txt
 */
 ...
 * @since       1.6
 */
class productsModelSecurity extends JModelAdmin
{
	/**
	 * @var     string  The help screen key for the module.
	 * @since   1.6
	 */
	protected $helpKey = 'JHELP_securityS_component_MANAGER_EDIT';
	 ...
	 * @since   1.6
	 */
	protected $event_after_save = 'onsecurityAfterSave';

	/**
	 * @var     string  The event to trigger after before the data.
	 * @since   1.6
	 */
	protected $event_before_save = 'onsecurityBeforeSave';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_phatthanhnghean.security', 'security', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_phatthanhnghean.edit.security.data', array());
		...
		$this->preprocessData('com_phatthanhnghean.security', $data);
    ...
    public function delete(&$pks)
    {
        ...
        foreach ($pks as $pk)
        {
            if ($table->load($pk))
            {
                ...
                if($table->issystem)
                {
                    $this->setError('you cannot delete security system');
                    return false;
                }
                if (!$table->delete($pk))
                {
    ...
    public function duplicateAndAssign(&$pks,$website_id=0)
    {
        ...
        $tuples=array();
/*        // Access checks.
        if (!$user->authorise('core.create', 'com_security'))
        {
            throw new Exception(JText::_('JERROR_CORE_CREATE_NOT_PERMITTED'));
        }*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('security.id');
	...
	*/
	public function getTable($type = 'security', $prefix = 'JTable', $config = array())
	{
