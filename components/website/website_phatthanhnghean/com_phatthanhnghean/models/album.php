 ...
 * @since       1.6
 */
class PhatThanhNgheAnModelalbum extends JModelAdmin
{
	/**
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_phatthanhnghean.album', 'album', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_phatthanhnghean.edit.album.data', array());
		...
		$this->preprocessData('com_phatthanhnghean.albums', $data);
    ...
    public function delete(&$pks)
    {
        ...
        foreach ($pks as $pk)
        {
            if ($table->load($pk))
            {
                ...
                if($table->issystem)
                {
                    $this->setError('you cannot delete album system');
                    return false;
                }
                if (!$table->delete($pk))
                {
	...
	public function duplicateAndAssign(&$pks,$website_id=0)
    {
        ...
        $tuples=array();
/*        // Access checks.
        if (!$user->authorise('core.create', 'com_album'))
        {
            throw new Exception(JText::_('JERROR_CORE_CREATE_NOT_PERMITTED'));
        }*/
	...
	*/
	public function getTable($type = 'album', $prefix = 'JTable', $config = array())
	{
