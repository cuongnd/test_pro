...
class JFormFieldCurrency extends JFormField
{
    ...
    protected function getInput()
    {
        ...
        $query->clear()
            ->select('currency.id,currency.currency_name AS text')
            ->from('#__ecommerce_currency AS currency')
            ->where('website_id='.(int)$website->website_id)
        ;
        $list_currency=$db->setQuery($query)->loadObjectList();
        ...
        $doc->addScript(JUri::root().'/components/website/website_template5532788/com_phatthanhnghean/models/fields/jquery.currency.js');
        $script_id = "script_field_parent_product_category_" . $this->id;
        ob_start();
        ?>
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                $('#<?php echo $this->id; ?>').field_currency({
                    list_currency:<?php echo json_encode($list_currency)?>,
                    field:{
