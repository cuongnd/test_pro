...
$query->from('#__products as component')
    ->select('component.*')
    ->leftJoin('#__albums AS albums ON albums.id=component.album_id')
    ->where('component.type='.$query->q('component'))
    ->where('albums.website_id=' . (int)$website->website_id)
