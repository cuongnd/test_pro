 ...
 * @since       1.6
 */
class supperadminModelusergroup extends JModelAdmin
{
	/**
	 * @var     string  The help screen key for the usergroup.
	 * @since   1.6
	 */
	protected $helpKey = 'JHELP_usergroupS_component_MANAGER_EDIT';

	/**
	 * @var     string  The help screen base URL for the usergroup.
	 * @since   1.6
	 */
	protected $helpURL;
	 ...
	 * @since   1.6
	 */
	protected $event_after_save = 'onusergroupAfterSave';

	/**
	 * @var     string  The event to trigger after before the data.
	 * @since   1.6
	 */
	protected $event_before_save = 'onusergroupBeforeSave';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_supperadmin.usergroup', 'usergroup', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_supperadmin.edit.usergroup.data', array());
		...
		$this->preprocessData('com_supperadmin.usergroup', $data);

		return $data;
	}
    /**
     * Method to duplicate usergroups.
     *
    ...
    public function delete(&$pks)
    {
        ...
        foreach ($pks as $pk)
        {
            if ($table->load($pk))
            {
                ...
                if($table->issystem)
                {
                    $this->setError('you cannot delete usergroup system');
                    return false;
                }
                if (!$table->delete($pk))
                {
    ...
    public function duplicateAndAssign(&$pks,$website_id=0)
    {
        ...
        $tuples=array();
/*        // Access checks.
        if (!$user->authorise('core.create', 'com_usergroup'))
        {
            throw new Exception(JText::_('JERROR_CORE_CREATE_NOT_PERMITTED'));
        }*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('usergroup.id');
