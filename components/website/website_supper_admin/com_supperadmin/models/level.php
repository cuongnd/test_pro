 ...
 * @since       1.6
 */
class supperadminModellevel extends JModelAdmin
{
	/**
	 * @var     string  The help screen key for the level.
	 * @since   1.6
	 */
	protected $helpKey = 'JHELP_levelS_component_MANAGER_EDIT';

	/**
	 * @var     string  The help screen base URL for the level.
	 * @since   1.6
	 */
	protected $helpURL;
	 ...
	 * @since   1.6
	 */
	protected $event_after_save = 'onlevelAfterSave';

	/**
	 * @var     string  The event to trigger after before the data.
	 * @since   1.6
	 */
	protected $event_before_save = 'onlevelBeforeSave';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_supperadmin.level', 'level', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_supperadmin.edit.level.data', array());
		...
		$this->preprocessData('com_supperadmin.level', $data);

		return $data;
	}
    /**
     * Method to duplicate levels.
     *
    ...
    public function delete(&$pks)
    {
        ...
        foreach ($pks as $pk)
        {
            if ($table->load($pk))
            {
                ...
                if($table->issystem)
                {
                    $this->setError('you cannot delete level system');
                    return false;
                }
                if (!$table->delete($pk))
                {
    ...
    public function duplicateAndAssign(&$pks,$website_id=0)
    {
        ...
        $tuples=array();
/*        // Access checks.
        if (!$user->authorise('core.create', 'com_level'))
        {
            throw new Exception(JText::_('JERROR_CORE_CREATE_NOT_PERMITTED'));
        }*/
