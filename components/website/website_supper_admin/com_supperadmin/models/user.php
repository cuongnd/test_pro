 ...
 * @since       1.6
 */
class supperadminModeluser extends JModelAdmin
{
	/**
	 * @var     string  The help screen key for the user.
	 * @since   1.6
	 */
	protected $helpKey = 'JHELP_userS_component_MANAGER_EDIT';

	/**
	 * @var     string  The help screen base URL for the user.
	 * @since   1.6
	 */
	protected $helpURL;
	 ...
	 * @since   1.6
	 */
	protected $event_after_save = 'onuserAfterSave';

	/**
	 * @var     string  The event to trigger after before the data.
	 * @since   1.6
	 */
	protected $event_before_save = 'onuserBeforeSave';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_supperadmin.user', 'user', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
	...
	protected function loadFormData()
	{
		...
		$this->preprocessData('com_supperadmin.user', $data);

		return $data;
	}
    /**
     * Method to duplicate users.
     *
    ...
    public function delete(&$pks)
    {
        ...
        foreach ($pks as $pk)
        {
            if ($table->load($pk))
            {
                ...
                if($table->issystem)
                {
                    $this->setError('you cannot delete user system');
                    return false;
                }
                if (!$table->delete($pk))
                {
    ...
    public function duplicateAndAssign(&$pks,$website_id=0)
    {
        ...
        $tuples=array();
/*        // Access checks.
        if (!$user->authorise('core.create', 'com_user'))
        {
            throw new Exception(JText::_('JERROR_CORE_CREATE_NOT_PERMITTED'));
        }*/
