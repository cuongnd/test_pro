...
(function($) {
    ...
    $.load_supperadmin = function(element, options) {
                    ...
                    success: function (response) {
                        ...
                        if(!$('.extension-plugin-config').length) {
                            html = $('<div class="panel plugin panel-primary extension-plugin-config  panelMove toggle panelRefresh panelClose"  >' +
                                ...
                                '</div>' +
                                '<div class="panel-body plugin"></div>' +
                                '<div class="panel-footer plugin-handle-footer">' +
                                '<button class="btn btn-danger save-block-property pull-right" onclick="view_plugin_config.save_and_close(self)" ><i class="fa-save"></i>Save&close</button>&nbsp;&nbsp;' +
                                '<button class="btn btn-danger apply-block-property pull-right" onclick="view_plugin_config.save(self)" ><i class="fa-save"></i>Save</button>&nbsp;&nbsp;' +
                                '<button class="btn btn-danger cancel-block-property pull-right" onclick="view_plugin_config.cancel(self)"><i class="fa-save"></i>Cancel</button>' +
                                '</div>'+
                                '</div>'
                            );
                            $('body').prepend(html);
                            ...
                            html.draggable({
                                handle: '.plugin-handle,.plugin-handle-footer'
                            });
                        }
        ...
        plugin.init = function() {
            ...
            $element.find(".list-plugin .item-element").draggable({
                appendTo: 'body',
                helper: "clone"
                /* revert:true,
                 proxy:'clone'*/
