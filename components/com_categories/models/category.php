...
class CategoriesModelCategory extends JModelAdmin
{
	...
	{
        require_once JPATH_ROOT.'/administrator/components/com_categories/tables/category.php';
	 ...
	 * @since   1.6
	 */
	protected function batchCopy($category_id, $pks, $contexts)
	{
		...
		$query = $db->getQuery(true)
            ...
            ->leftJoin('#__categories AS c2 ON c2.website_id=c1.website_id')
            ->where('c1.extension!='.$db->quote('system'))
            ->where('c2.id='.(int)reset($pks))
        ;
		...
		while (!empty($pks) && $count > 0)
		{
            ...
            $this->table->copy_from = $this->table->id;
			$this->table->id = 0;
            $this->table->website_id = $website_id;
			// If we a copying children, the Old ID will turn up in the parents list
			// otherwise it's a new top level item
			$this->table->parent_id = isset($parents[$oldParentId]) ? $parents[$oldParentId] : $category_id;
