...
class CategoriesModelCategories extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$website_id = $this->getUserStateFromRequest($context . '.filter.website_id', 'filter_website_id', 0);
	...
	protected function getListQuery()
	{
		...
		{
			$query->where('(a.extension = ' . $db->quote($extension).' OR  a.extension ='.$db->quote('root').')');
		}
        ...
        $query->select('CONCAT(dw.domain,"[",w.title,"]") AS website');
        $query->leftJoin('#__website AS w ON w.id=a.website_id');
        $query->leftJoin('#__domain_website AS dw ON dw.website_id=w.id');
