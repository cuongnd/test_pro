...
$query->from('#__menu_types AS menuType');
$query->select('menu.parent_id, menu.id,menu.binding_source,menu.binding_source_key,menu.binding_source_value,menu.access,menu.level,menu.icon,menu.title,menu.link,menu.alias');
$query->leftJoin('#__menu As menu ON menu.menu_type_id=menuType.id');
...
foreach ($list_menu as $item) {
    if(!is_array($list_menu_item[$item->menu_type_id]))
    {
        $list_menu_item[$item->menu_type_id]=array();
    }
    if($item->id)
    {
