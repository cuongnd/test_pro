...
class MenusModelMenutypes extends JModelLegacy
{
	 ...
	 * @since   1.6
	 */
	public function getReverseLookup($menu_type_id)
	{
		if (empty($this->rlu))
		{
			$this->getTypeOptions($menu_type_id);
		}
		return $this->rlu;
	}
	public function getTypeOptionsByWebsiteId($website_id)
	{
		...
		$query->select('com.id,com.name, com.element AS ' . $db->quoteName('option'))
			->from('#__components AS com')
			->where('com.enabled = 1')
			->where('com.website_id='.(int)$website_id)
			->order('name ASC');
		$db->setQuery($query);
		...
		foreach ($components as $component)
		{
			if ($options = $this->getTypeOptionsByComponent($component->option))
			{
				...
				foreach($options as $key=>$listOption) {
	 ...
	 * @since   1.6
	 */
	public function getTypeOptions($menu_type_id)
	{
		jimport('joomla.filesystem.file');
        $app=JFactory::getApplication();
		$lang = JFactory::getLanguage();
	...
	protected function getTypeOptionsByComponent($component)
	{
        ...
        foreach($component1 as $key=> $path) {
            ...
            $mainXML = $path . '/components/' . $component . '/metadata.xml';

            if (is_file($mainXML)) {
                $options[$key] = $this->getTypeOptionsFromXML($mainXML, $component);
            }

            if (empty($options[$key])) {

                $options[$key] = $this->getTypeOptionsFromMVC($component,$path);
	...
	protected function getTypeOptionsFromMVC($component,$a_path)
	{
		$options = array();

		// Get the views for this component.
		if (is_dir($a_path . '/components/' . $component))
		{
			$folders = JFolder::folders($a_path . '/components/' . $component, '^view[s]?$', false, true);
		}
		$path = '';
				...
				if (is_file($file))
				{
					...
					if ($xml = simplexml_load_file($file))
					{
						...
						if ($menu = $xml->xpath('view[1]'))
						{
							...
							{
								$options = array_merge($options, (array) $this->getTypeOptionsFromLayouts($component, $view,$a_path));
							}
						}
	...
	protected function getTypeOptionsFromLayouts($component, $view,$a_path)
	{
		$options = array();
		$layouts = array();
		$layoutNames = array();
		...
		$folders = JFolder::folders($a_path . '/templates', '', false, true);
		...
		foreach ($folders as $folder)
		{
			if (is_dir($folder . '/html/' . $component . '/' . $view))
			{
				...
				$lang->load('tpl_' . $template . '.sys', $a_path, null, false, true)
				||	$lang->load('tpl_' . $template . '.sys', $a_path . '/templates/' . $template, null, false, true);
