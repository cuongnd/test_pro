...
class JFormFieldMenuParent extends JFormFieldList
{
	...
	protected function getOptions()
	{
		...
		if ($menu_type_id = $this->form->getValue('menu_type_id'))
		{
			$query->where('a.menu_type_id = ' .(int) $menu_type_id);
		}else
        {
            $menusModelItems=JModelLegacy::getInstance('items','menusModel');
            $menu_type_id=$menusModelItems->getDefaultMenuTypeId();
            $query->where('a.menu_type_id = ' .(int) $menu_type_id);
        }
