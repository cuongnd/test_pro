...
class MenusModelUitem extends JModelAdmin
{
	 ...
	 * @since   1.6
	 */
	public  function batchCopy($value, $pks, $website_id)
	{

        $supperAdmin=JFactory::isSupperAdmin();
		// $value comes as {menu_type_id}.{parent_id}
		$parts = explode('.', $value);
		$menu_type_id = $parts[0];
		$parent_id = (int) JArrayHelper::getValue($parts, 1, 0);
		...
		if (empty($parent_id))
		{
            $table->menu_type_id=$menu_type_id;

			if (!$parent_id = $table->getRootId())
			{
...
/*		if (!$user->authorise('core.create', 'com_menus'))
		...
		$query->select('menu1.*')
			->from('#__menu AS menu1')
            ->leftJoin('#__menu AS menu2 ON menu2.menu_type_id=menu1.menu_type_id')
            ->where('menu2.id='.(int)reset($pks))
            ->where('menu1.alias!='.$query->q('root'))
        ;
		...
		while (!empty($pks) && $count > 0)
		{
			...
			$query->clear()
                ...
                ->where('menu_type_id='.(int)$table->menu_type_id)
            ;
			...
			$table->parent_id = isset($parents[$oldParentId]) ? $parents[$oldParentId] : $parent_id;
			$table->menu_type_id = $menu_type_id;
	 ...
	 * @param   integer  $value     The new menu or sub-item.
	 * @param   array    $pks       An array of row IDs.
	 * @param   array    $website_id  website_id.
	 *
	...
	protected function batchMove($value, $pks)
	{
		...
		foreach ($pks as $pk)
		{
			...
			if ($menu_type_id != $table->menu_type_id)
			{
			...
			if ($table->load($pk))
			{
				if (!array_key_exists($table->language, $languages))
				{
					...
					else
					{
                        ...
                        $menuTypeTable=$this->getTable('MenuType','JTable');
                        $menuTypeTable->load($table->menu_type_id);
                        $table->website_id=$menuTypeTable->website_id;
    ...
    function ajaxSaveForm($pks)
    {
        ...
        $alias=$input->get('alias',array(),'array');
        $table = $this->getTable();
        $pks = (array) $pks;
	...
	public function publish(&$pks, $value = 1)
	{
		...
		if ($value != 1)
		{
			...
			foreach ($pks as $i => $pk)
			{
				...
				if (!$superAdmin && $table->load($pk) && $table->home && $table->language == '*')
				{
					// Prune items that you can't change.
