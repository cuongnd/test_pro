 ...
 * @since       1.6
 */
class MenusModelMenu extends JModelAdmin
{
	/**
...
*/
        $table = $this->getTable();
        $menuModelItem=JModelLegacy::getInstance('Item','menusModel');
        foreach ($pks as $pk)
        {
            ...
            $query->select('id');
            $query->from('#__menu');
            $query->where('menu_type_id='.$pk);
            $query->where('alias!='.$query->q('root'));
            $db->setQuery($query);
            $listMenuItemId=$db->loadColumn();

            //cope menu item to new menu type

            $return= $menuModelItem->batchCopy($table->id.'.1',$listMenuItemId,$website_id);
            if(!$return)
            {
                $errors=$menuModelItem->getErrors();
                $errors=implode(',',$errors);
                throw new Exception(JText::_('JERROR_CORE_CREATE_NOT_BATCH_COPY').':'.$errors);
            }
	...
	public function quick_assign_website(&$pks,$website_id)
    {
        ...
        foreach ($pks as $pk)
        {
            ...
            $db->setQuery($query);
            $listMenuItemId=$db->loadColumn();
            $return= $menuModelItem->batchCopy($table->id.'.1',$listMenuItemId);
            if(!$return)
            {
                throw new Exception(JText::_('JERROR_CORE_CREATE_NOT_BATCH_COPY'));
            }        }
	...
	public function &getModules()
	{
		...
		foreach ($modules as &$module)
		{
			...
			if (!isset($result[$menu_type_id]))
			{
				$result[$menu_type_id] = array();
			}
			$result[$menu_type_id][] = & $module;
		}
