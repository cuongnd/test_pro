...
class tocontents
{
	...
	function TOCpagebreak($tocfont = '', $tocfontsize = '', $tocindent = '', $TOCusePaging = true, $TOCuseLinking = '', $toc_orientation = '', $toc_mgl = '', $toc_mgr = '', $toc_mgt = '', $toc_mgb = '', $toc_mgh = '', $toc_mgf = '', $toc_ohname = '', $toc_ehname = '', $toc_ofname = '', $toc_efname = '', $toc_ohvalue = 0, $toc_ehvalue = 0, $toc_ofvalue = 0, $toc_efvalue = 0, $toc_preHTML = '', $toc_postHTML = '', $toc_bookmarkText = '', $resetpagenum = '', $pagenumstyle = '', $suppress = '', $orientation = '', $mgl = '', $mgr = '', $mgt = '', $mgb = '', $mgh = '', $mgf = '', $ohname = '', $ehname = '', $ofname = '', $efname = '', $ohvalue = 0, $ehvalue = 0, $ofvalue = 0, $efvalue = 0, $toc_id = 0, $pagesel = '', $toc_pagesel = '', $sheetsize = '', $toc_sheetsize = '')
	{
		...
		if (!$tocfont) {
			$tocfont = $this->mpdf->default_font;
		}
		if (!$tocfontsize) {
			$tocfontsize = $this->mpdf->default_font_size;
		}
		if (!$tocindent && $tocindent !== 0) {
			$tocindent = 5;
		}
...
// Initiate, and Mark a place for the Table of Contents to be inserted
	...
	function insertTOC()
	{
		...
		for ($toci = 0; $toci < $notocs; $toci++) {
			...
			if ($this->TOCheader) {
				$this->mpdf->SetHeader($this->TOCheader);
			}
			else if ($this->TOCheader !== false) {
				$this->mpdf->SetHeader();
			}
			...
			foreach ($this->_toc as $t) {
				if ($t['toc_id'] === '_mpdf_all' || $t['toc_id'] === $toc_id) {
					...
					// LTR
						...
						$fullstr = $str;
						$this->mpdf->SetFont($tocfont, $weight, $tocfontsize, true, true);
						if ($level > 0 && $tocindent) {
							$this->mpdf->Cell($level * $tocindent, $this->mpdf->FontSize + $lineheightcorr);
						}
						...
						if ($TOCusePaging) {
							...
							if ($nb > 0) {
								...
								$this->mpdf->y += $this->mpdf->lineheight;
								$dots = str_repeat('.', 5);
							} // ..... 5 dots?
	...
	function openTagTOC($attr)
	{
		if (isset($attr['FONT-SIZE']) && $attr['FONT-SIZE']) {
			$tocfontsize = $attr['FONT-SIZE'];
		} else {
			$tocfontsize = '';
		}
		if (isset($attr['FONT']) && $attr['FONT']) {
			$tocfont = $attr['FONT'];
		} else {
			$tocfont = '';
		}
		if (isset($attr['INDENT']) && $attr['INDENT']) {
			$tocindent = $attr['INDENT'];
		} else {
			$tocindent = '';
		}
	...
	function openTagTOCPAGEBREAK($attr)
	{
		...
		if ($toc_id) {
			if (isset($attr['FONT-SIZE'])) {
				$this->m_TOC[$toc_id]['TOCfontsize'] = $attr['FONT-SIZE'];
			} else {
				$this->m_TOC[$toc_id]['TOCfontsize'] = $this->mpdf->default_font_size;
			}
			if (isset($attr['FONT'])) {
				$this->m_TOC[$toc_id]['TOCfont'] = $attr['FONT'];
			} else {
				$this->m_TOC[$toc_id]['TOCfont'] = $this->mpdf->default_font;
			}
			if (isset($attr['INDENT']) && $attr['INDENT']) {
				$this->m_TOC[$toc_id]['TOCindent'] = $attr['INDENT'];
			} else {
				$this->m_TOC[$toc_id]['TOCindent'] = '';
			}
			if (isset($attr['TOC-ORIENTATION']) && $attr['TOC-ORIENTATION']) {
			...
			if (isset($attr['FONT-SIZE'])) {
				$this->TOCfontsize = $attr['FONT-SIZE'];
			} else {
				$this->TOCfontsize = $this->mpdf->default_font_size;
			}
			if (isset($attr['FONT'])) {
				$this->TOCfont = $attr['FONT'];
			} else {
				$this->TOCfont = $this->mpdf->default_font;
			}
			if (isset($attr['INDENT']) && $attr['INDENT']) {
				$this->TOCindent = $attr['INDENT'];
			} else {
				$this->TOCindent = '';
			}
			if (isset($attr['TOC-ORIENTATION']) && $attr['TOC-ORIENTATION']) {
