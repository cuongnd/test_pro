...
// FORMS
	...
	function print_ob_text($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
	{
		...
		if ($this->mpdf->useActiveForms) {
			...
			$flags = array();
			if ($objattr['disabled'] || $objattr['readonly']) {
				$flags[] = 1;
			} // readonly
			if ($objattr['disabled']) {
				$flags[] = 3;
			...
			if (!$objattr['spellcheck']) {
				$flags[] = 23;
			} // DoNotSpellCheck
			if ($objattr['subtype'] == 'PASSWORD') {
				$flags[] = 14;
			}
			$this->mpdf->SetTColor($objattr['color']);
			$fieldalign = $rtlalign;
			if ($objattr['text_align']) {
			...
			// mPDF 5.3.25
			$js = array();
			if ($objattr['onCalculate']) {
				$js[] = array('C', $objattr['onCalculate']);
			}
			if ($objattr['onValidate']) {
	...
	function print_ob_select($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
	{
		...
		if ($this->mpdf->useActiveForms) {
			...
			if ($objattr['multiple'] && $objattr['size'] > 1) {
				$flags[] = 22;
			...
			if (!$objattr['spellcheck'] || $objattr['size'] > 1 || !$objattr['editable']) {
				$flags[] = 23;
			} // DoNotSpellCheck
			if ($objattr['subtype'] == 'PASSWORD') {
				$flags[] = 14;
