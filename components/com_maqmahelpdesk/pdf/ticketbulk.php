...
for ($ti = 0; $ti < count($ids); $ti++) {
    ...
    if ((int)$id) {
        ...
        // Must verify access permission to ticket
        if ($user->id == $row->id_user || $is_support) {
            ...
            $database->setQuery("SELECT name FROM #__support_category WHERE id='" . $row->id_category . "'");
