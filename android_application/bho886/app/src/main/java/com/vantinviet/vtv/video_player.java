...
public class video_player extends Activity {
    ...
    private void initPlayer(int position) {
        ...
        ExtractorSampleSource sampleSource = new ExtractorSampleSource( Uri.parse(get_link_video()), dataSource, allocator,BUFFER_SEGMENT_COUNT * BUFFER_SEGMENT_SIZE);

        MediaCodecVideoTrackRenderer videoRenderer = new
                MediaCodecVideoTrackRenderer(this, sampleSource, MediaCodecSelector.DEFAULT,MediaCodec.VIDEO_SCALING_MODE_SCALE_TO_FIT);
    ...
    private String get_link_video() {
        Intent intent = getIntent();
        Bundle b = intent.getBundleExtra("personBdl");
        String link = b.getString("link");
        String mp4URL=this.mp4URL;
        if(!link.equals(""))
        {
            mp4URL=link;
        }
    ...
    private void initHLSPlayer(int position) {
        ...
        playlistFetcher.singleLoad(mHandler.getLooper(), new ManifestFetcher.ManifestCallback<HlsPlaylist>() {
            ...
            public void onSingleManifest(HlsPlaylist manifest) {
                ...
                DataSource dataSource = new DefaultUriDataSource(video_player.this, bandwidthMeter, userAgent);
                HlsChunkSource chunkSource = new HlsChunkSource(true , dataSource, HLSurl, playlistFetcher.getManifest(),
                        DefaultHlsTrackSelector.newDefaultInstance(video_player.this), bandwidthMeter, timestampAdjusterProvider,
                        HlsChunkSource.ADAPTIVE_MODE_SPLICE);
                HlsSampleSource sampleSource = new HlsSampleSource(chunkSource, loadControl,
                        BUFFER_SEGMENT_COUNT * BUFFER_SEGMENT_SIZE);
                MediaCodecVideoTrackRenderer videoRenderer = new MediaCodecVideoTrackRenderer(video_player.this, sampleSource,
